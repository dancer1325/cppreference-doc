...
[std::semiregular](../concepts/semiregular.html)<S> &&  
[std::input_or_output_iterator](input_or_output_iterator.html)<I> &&  
__WeaklyEqualityComparableWith<S, I>; |  |  (since C++20)  
| |   
...
Let `s` and `i` be values of type `S` and `I`, respectively, such that `[`i`, `s`)` denotes a [range](../iterator.html#Ranges "cpp/iterator"). `sentinel_for<S, I>` is modeled only if: 
  ...
  * [std::assignable_from](../concepts/assignable_from.html)<I&, S> is either modeled or not satisfied. 
