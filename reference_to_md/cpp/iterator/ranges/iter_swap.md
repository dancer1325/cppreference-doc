...
---  
[ranges::distance](distance.html "cpp/iterator/ranges/distance")(C++20)  
...
constexpr void iter_swap( I1&& i1, I2&& i2 ) noexcept(/* see below */); |  |  (since C++20)  
Helper function |  |   
template< class X, class Y >  
constexpr [std::iter_value_t](../iter_t.html)<X>  
iter-exchange-move( X&& x, Y&& y )  
noexcept(noexcept([std::iter_value_t](../iter_t.html)<X>(std::[ranges::iter_move](iter_move.html)(x))) &&  
noexcept(*x = std::[ranges::iter_move](iter_move.html)(y))); |  |  (exposition only*)  
| |   
...
The effect of the exposition-only helper function `_iter-exchange-move_` is equivalent to 
    ...
    [std::iter_value_t](../iter_t.html)<X> old(std::[ranges::iter_move](iter_move.html)(x));
    *x = std::[ranges::iter_move](iter_move.html)(y);
    return old;

ranges::iter_swap(i1, i2) is [expression-equivalent](../../language/expressions.html#Expression-equivalence "cpp/language/expressions") to: 

  1. (void)iter_swap(i1, i2), if i1 or i2 has a class or enumeration type and the expression is well-formed, where the [overload resolution](../../language/overload_resolution.html "cpp/language/overload resolution") of `iter_swap` is performed with the additional candidate void iter_swap(auto, auto) = delete;[[1]](iter_swap.html#cite_note-1), excluding `std::ranges::iter_swap` itself. 
  ...
  2. Otherwise, [ranges::swap](http://en.cppreference.com/w/cpp/ranges-utility-placeholder/swap)(*i1, *i2) if both `I1` and `I2` model [`indirectly_readable`](../indirectly_readable.html "cpp/iterator/indirectly readable") and if [std::iter_reference_t](../iter_t.html)<I1> and [std::iter_reference_t](../iter_t.html)<I2> model [`swappable_with`](../../concepts/swappable.html "cpp/concepts/swappable"). 
  3. Otherwise, (void)(*i1 =` `` _iter-exchange-move_`(i2, i1)), if [std::indirectly_movable_storable](../indirectly_movable_storable.html)<I1, I2> and [std::indirectly_movable_storable](../indirectly_movable_storable.html)<I2, I1> are both modeled, except that i1 is only evaluated once. 
  4. Otherwise, ranges::iter_swap(i1, i2) is ill-formed, which can result in [substitution failure](../../language/sfinae.html "cpp/language/sfinae") when ranges::iter_swap(i1, i2) appears in the immediate context of a template instantiation. 


  1. [↑](iter_swap.html#cite_ref-1) This precludes calling unconstrained [std::iter_swap](../../algorithm/iter_swap.html "cpp/algorithm/iter swap").



###  Customization point objects

The name `ranges::iter_swap` denotes a _customization point object_ , which is a const [function object](../../named_req/FunctionObject.html "cpp/named req/FunctionObject") of a [literal](../../named_req/LiteralType.html "cpp/named req/LiteralType") [`semiregular`](../../concepts/semiregular.html "cpp/concepts/semiregular") class type. See [CustomizationPointObject](../../named_req/CustomizationPointObject.html "cpp/named req/CustomizationPointObject") for details. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/iterator/ranges/iter_swap&action=edit&section=1 "Edit section: Example")] Example

| This section is incomplete  
Reason: no example   
---|---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/iterator/ranges/iter_swap&action=edit&section=2 "Edit section: See also")] See also
