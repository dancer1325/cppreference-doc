...
constexpr common_iterator( const common_iterator<I2, S2>& x ); |  (4)  |  (since C++20)  
| |   
  
Constructs a new iterator adaptor, effectively initializes the underlying [std::variant](../../utility/variant.html)<I, S> member object `_var_` to hold an `I` (iterator) or `S` (sentinel) object. 

1) Default constructor. Default-initializes `_var_`. After construction, `_var_` holds a value-initialized `I` object. Operations on the resulting iterator adaptor have defined behavior if and only if the corresponding operations on a value-initialized `I` also have defined behavior.

2) After construction, `_var_` holds an `I` object move-constructed from i.

3) After construction, `_var_` holds an `S` object move-constructed from s.
...
Run this code
    ...
    int main()
    {
        [std::vector](../../container/vector.html) v{3, 1, 4, 1, 5, 9, 2};
     
        ...
        CI unused; // (1)
        CI start{[std::counted_iterator](../counted_iterator.html){[std::next](../next.html)(begin(v)), ssize(v) - 2}}; // (2)
        CI finish{[std::default_sentinel](../default_sentinel.html)}; // (3)
        CI first{start}; // (4)
        CI last{finish}; // (4)
     
    ...
     
        [std::common_iterator](../common_iterator.html)<
            [std::counted_iterator](../counted_iterator.html)<
                [std::ostream_iterator](../ostream_iterator.html)<double>>,
                [std::default_sentinel_t](../default_sentinel.html)>
                beg{[std::counted_iterator](../counted_iterator.html){[std::ostream_iterator](../ostream_iterator.html)<double>{[std::cout](../../io/cout.html),";  "}, 5}},
                end{[std::default_sentinel](../default_sentinel.html)};
        [std::iota](../../algorithm/iota.html)(beg, end, 3.1);
        [std::cout](../../io/cout.html) << '\n';
    }
