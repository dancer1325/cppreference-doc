...
---|---|---  
template< class I >  
struct incrementable_traits {}; |  (1)  |  (since C++20)  
template< class T >  
requires [std::is_object_v](../types/is_object.html)<T>  
struct incrementable_traits<T*>; |  (2)  |  (since C++20)  
template< class T >  
...
template< class T >  
requires (!requires { typename T::difference_type; }) &&  
requires(const T& a, const T& b) { { a - b } -> [std::integral](../concepts/integral.html); }  
struct incrementable_traits<T>; |  (5)  |  (since C++20)  
| |   
  
Computes the associated difference type of the type `I`, if any. A program may specialize `incrementable_traits` for a [program-defined type](../language/type-id.html#Program-defined_type "cpp/language/type"). 

1) Primary template is an empty struct.

2) Specialization for pointers.

Provides a member type `difference_type` same as [std::ptrdiff_t](../types/ptrdiff_t.html "cpp/types/ptrdiff t").
...
Provides a member type `difference_type` same as [std::make_signed_t](../types/make_signed.html)<decltype([std::declval](../utility/declval.html)<T>() - [std::declval](../utility/declval.html)<T>())>. The implicit expression variations rule (see below) applies to the expression a - b.
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/iterator/incrementable_traits&action=edit&section=1 "Edit section: Example")] Example

| This section is incomplete  
Reason: no example   
---|---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/iterator/incrementable_traits&action=edit&section=2 "Edit section: See also")] See also
