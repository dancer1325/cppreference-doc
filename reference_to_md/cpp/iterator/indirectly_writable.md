...
const_cast<const [std::iter_reference_t](iter_t.html)<Out>&&>(*o) = [std::forward](../utility/forward.html)<T>(t);  
const_cast<const [std::iter_reference_t](iter_t.html)<Out>&&>(*[std::forward](../utility/forward.html)<Out>(o)) =  
[std::forward](../utility/forward.html)<T>(t);  
};  
/* none of the four expressions above are required to be equality-preserving */ |  |  (since C++20)  
| |   
...
Let `e` be an expression such that decltype((e)) is `T`, and `o` be a dereferenceable object of type `Out`, then indirectly_writable<Out, T> is modeled only if: 
  ...
  * If [std::indirectly_readable](indirectly_readable.html)<Out> is modeled and [std::iter_value_t](iter_t.html)<Out> is the same type as [std::decay_t](../types/decay.html)<T>, then *o after any above assignment is equal to the value of `e` before the assignment. 
