...
**default_sentinel_t default_sentinel**(C++20)(C++20)  
...
1) `default_sentinel_t` is an empty class type used to denote the end of a range. It can be used together with iterator types that know the bound of their range (e.g., [std::counted_iterator](counted_iterator.html "cpp/iterator/counted iterator")).
...
Run this code
    ...
     
        const [std::regex](../regex/basic_regex.html) words_regex("[^\\s]+");
        const std::[ranges::subrange](../ranges/subrange.html) words(
            [std::sregex_iterator](../regex/regex_iterator.html)(s.begin(), s.end(), words_regex), std::default_sentinel);
     
        [std::println](../io/println.html)("Found {} words:", std::[ranges::distance](ranges/distance.html)(words));
        for (const [std::smatch](../regex/match_results.html)& match : words)
            [std::println](../io/println.html)("{}", match.str());
    }
