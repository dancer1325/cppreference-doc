...
typename [std::iterator_traits](iterator_traits.html)<InputIt>::difference_type   
distance( InputIt first, InputIt last ); |  | (constexpr since C++17)  
| |   
  
Returns the number of hops from first to last. 

If `InputIt` is not [LegacyRandomAccessIterator](../named_req/RandomAccessIterator.html "cpp/named req/RandomAccessIterator"), the behavior is undefined if last is not [reachable](../iterator.html#Ranges "cpp/iterator") from first. 

If `InputIt` is [LegacyRandomAccessIterator](../named_req/RandomAccessIterator.html "cpp/named req/RandomAccessIterator"), the behavior is undefined if first and last are neither reachable from each other. 
...
---  
    ...
    namespace detail
    {
        ...
        typename [std::iterator_traits](iterator_traits.html)<It>::difference_type 
            do_distance(It first, It last, [std::input_iterator_tag](iterator_tags.html))
        {
            typename [std::iterator_traits](iterator_traits.html)<It>::difference_type result = 0;
            while (first != last)
            {
                ++first;
                ++result;
    ...
     
        ...
        constexpr // required since C++17
        typename [std::iterator_traits](iterator_traits.html)<It>::difference_type 
            do_distance(It first, It last, [std::random_access_iterator_tag](iterator_tags.html))
        {
            return last - first;
        }
    } // namespace detail
        ...
        distance(It first, It last)
    {
        return detail::do_distance(first, last,
                                   typename [std::iterator_traits](iterator_traits.html)<It>::iterator_category());
    }  
...
C++17 implementation via if constexpr  
    ...
    template<class It>
    constexpr typename [std::iterator_traits](iterator_traits.html)<It>::difference_type
        distance(It first, It last)
    {
        using category = typename [std::iterator_traits](iterator_traits.html)<It>::iterator_category;
        static_assert([std::is_base_of_v](../types/is_base_of.html)<[std::input_iterator_tag](iterator_tags.html), category>);
     
        if constexpr ([std::is_base_of_v](../types/is_base_of.html)<[std::random_access_iterator_tag](iterator_tags.html), category>)
            return last - first;
        else
        {
            typename [std::iterator_traits](iterator_traits.html)<It>::difference_type result = 0;
...
Run this code
    ...
    int main() 
    {
        ...
        [std::cout](../io/cout.html) << "distance(first, last) = "
