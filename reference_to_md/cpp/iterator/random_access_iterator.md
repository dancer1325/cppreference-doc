...
[std::bidirectional_iterator](bidirectional_iterator.html)<I> &&  
[std::derived_from](../concepts/derived_from.html)</*ITER_CONCEPT*/<I>, [std::random_access_iterator_tag](iterator_tags.html)> &&  
[std::totally_ordered](../concepts/totally_ordered.html)<I> &&  
[std::sized_sentinel_for](sized_sentinel_for.html)<I, I> &&  
requires(I i, const I j, const [std::iter_difference_t](iter_t.html)<I> n) {  
{ i += n } -> [std::same_as](../concepts/same_as.html)<I&>;  
{ j + n } -> [std::same_as](../concepts/same_as.html)<I>;  
{ n + j } -> [std::same_as](../concepts/same_as.html)<I>;  
{ i -= n } -> [std::same_as](../concepts/same_as.html)<I&>;  
{ j - n } -> [std::same_as](../concepts/same_as.html)<I>;  
{ j[n] } -> [std::same_as](../concepts/same_as.html)<[std::iter_reference_t](iter_t.html)<I>>;  
}; |  |  (since C++20)  
| |   
  
The concept `random_access_iterator` refines [`bidirectional_iterator`](bidirectional_iterator.html "cpp/iterator/bidirectional iterator") by adding support for constant time advancement with the `**+=**`, `**+**`, `**-=**`, and `**-**` operators, constant time computation of distance with `**-**`, and array notation with subscripting `**[]**`. 

## Contents

  * [1 Iterator concept determination](random_access_iterator.html#Iterator_concept_determination)
  * [2 Semantic requirements](random_access_iterator.html#Semantic_requirements)
  * [3 Equality preservation](random_access_iterator.html#Equality_preservation)
  * [4 Implicit expression variations](random_access_iterator.html#Implicit_expression_variations)
  * [5 Notes](random_access_iterator.html#Notes)
  * [6 Example](random_access_iterator.html#Example)
  * [7 See also](random_access_iterator.html#See_also)
...
(That is, [std::derived_from](../concepts/derived_from.html)</*ITER_CONCEPT*/<I>, [std::random_access_iterator_tag](iterator_tags.html)> is assumed to be true.) 
...
Let a and b be valid iterators of type `I` such that b is reachable from a, and let n be a value of type [std::iter_difference_t](iter_t.html)<I> equal to b - a. std::random_access_iterator<I> is modeled only if all the concepts it subsumes are modeled and: 
  ...
  * (a += n) is equal to b. 
  * [std::addressof](../memory/addressof.html)(a += n) is equal to [std::addressof](../memory/addressof.html)(a). [[1]](random_access_iterator.html#addressof_note)
  * (a + n) is equal to (a += n). 
  * (a + n) is equal to (n + a). 
  ...
  * [std::addressof](../memory/addressof.html)(b -= n) is equal to [std::addressof](../memory/addressof.html)(b). [[1]](random_access_iterator.html#addressof_note)
  * (b - n) is equal to (b -= n). 
...
Run this code
    ...
    namespace cxx20
    {
        template<[std::input_or_output_iterator](input_or_output_iterator.html) Iter>
        constexpr [std::iter_difference_t](iter_t.html)<Iter> distance(Iter first, Iter last)
        {
            if constexpr(std::random_access_iterator<Iter>)
                return last - first;
            else
            {
                [std::iter_difference_t](iter_t.html)<Iter> result{};
