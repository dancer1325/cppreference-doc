...
[std::indirectly_copyable](indirectly_copyable.html)<In, Out> &&  
[std::indirectly_writable](indirectly_writable.html)<Out, [std::iter_value_t](iter_t.html)<In>&> &&  
[std::indirectly_writable](indirectly_writable.html)<Out, const [std::iter_value_t](iter_t.html)<In>&> &&  
[std::indirectly_writable](indirectly_writable.html)<Out, [std::iter_value_t](iter_t.html)<In>&&> &&  
[std::indirectly_writable](indirectly_writable.html)<Out, const [std::iter_value_t](iter_t.html)<In>&&> &&  
[std::copyable](../concepts/copyable.html)<[std::iter_value_t](iter_t.html)<In>> &&  
[std::constructible_from](../concepts/constructible_from.html)<[std::iter_value_t](iter_t.html)<In>, [std::iter_reference_t](iter_t.html)<In>> &&  
[std::assignable_from](../concepts/assignable_from.html)<[std::iter_value_t](iter_t.html)<In>&, [std::iter_reference_t](iter_t.html)<In>>; |  |  (since C++20)  
| |   
...
`In` and `Out` model std::indirectly_copyable_storable<In, Out> only if given a dereferenceable value `i` of type `In`: 
  ...
  * After the definition [std::iter_value_t](iter_t.html)<In> obj(*i);, `obj` is equal to the value previously denoted by *i, and 
  * if [std::iter_reference_t](iter_t.html)<In> is an rvalue reference type, *i is placed in a valid but unspecified state after the initialization of `obj`. 
