...
[std::movable](../concepts/movable.html)<I> &&  
requires(I i) {  
typename [std::iter_difference_t](iter_t.html)<I>;  
requires /*is-signed-integer-like*/<[std::iter_difference_t](iter_t.html)<I>>;  
{ ++i } -> [std::same_as](../concepts/same_as.html)<I&>; // not required to be equality-preserving  
i++; // not required to be equality-preserving  
}; |  |  (since C++20)  
| |   
  
For the definition of /*is-signed-integer-like*/, see [`_is-integer-like_`](is-integer-like.html "cpp/iterator/is-integer-like") ﻿. 

This concept specifies requirements on types that can be incremented with the pre- and post-increment operators, but those increment operations are not necessarily [equality-preserving](../concepts.html#Equality_preservation "cpp/concepts"), and the type itself is not required to be [std::equality_comparable](../concepts/equality_comparable.html "cpp/concepts/equality comparable"). 
...
For an object i of type `I`, `I` models `std::weakly_incrementable` only if all following conditions are satisfied: 
  ...
  * If i is incrementable, then [std::addressof](../memory/addressof.html)(++i) == [std::addressof](../memory/addressof.html)(i). 
