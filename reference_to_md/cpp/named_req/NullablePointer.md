...
## Contents
  ...
  * [1 Requirements](NullablePointer.html#Requirements)
  * [2 Notes](NullablePointer.html#Notes)
  * [3 Standard library](NullablePointer.html#Standard_library)
  * [4 Defect reports](NullablePointer.html#Defect_reports)
...
In addition, a value-initialized object of the type must produce a null value of that type. This null value shall only be equivalent to itself. Default initialization of the type may have an [indeterminate](../language/default_initialization.html#Indeterminate_and_erroneous_values "cpp/language/default initialization") or erroneous(since C++26) value. 

A value of the type must be [contextually convertible](../language/implicit_cast.html#Contextual_conversions "cpp/language/implicit conversion") to bool. The effect of this conversion returns false if the value is equivalent to its null value and true otherwise. 

None of the operations that this type performs may throw exceptions. 

The type must satisfy the following additional expressions, given two values p and q that are of the type, and that np is a value of [std::nullptr_t](../types/nullptr_t.html "cpp/types/nullptr t") type (possibly const-qualified): 
...
p != q |    
|  Type and value meet the [BooleanTestable](BooleanTestable.html "cpp/named req/BooleanTestable") requirements  | (until C++20)  
---|---  
decltype(p != q) models [`_boolean-testable_`](../concepts/boolean-testable.html "cpp/concepts/boolean-testable") | (since C++20)  
  
The effect is !(p == q)  
  
p == np np == p |    
|  Type and value of both expressions the [BooleanTestable](BooleanTestable.html "cpp/named req/BooleanTestable") requirements  | (until C++20)  
---|---  
decltype(p == np) and decltype(np == p) each model [`_boolean-testable_`](../concepts/boolean-testable.html "cpp/concepts/boolean-testable") | (since C++20)  
...
Note that dereferencing (operator* or operator->) is not required for a NullablePointer type. A minimalistic type that satisfies these requirements is 
    ...
    public:
        handle() = default;
        handle([std::nullptr_t](../types/nullptr_t.html)) {}
...
The following types must satisfy NullablePointer in order to communicate with standard library components: 
  ...
  * The member types `X::pointer`, `X::const_pointer`, `X::void_pointer` and `X::const_void_pointer` of every [Allocator](Allocator.html "cpp/named req/Allocator") type `X`. 
  * The member type `pointer` of [std::unique_ptr](../memory/unique_ptr.html "cpp/memory/unique ptr"). 



  * The adapted pointer type of [`std::inout_ptr_t`](../memory/inout_ptr_t.html "cpp/memory/inout ptr t") and [`std::out_ptr_t`](../memory/out_ptr_t.html "cpp/memory/out ptr t"). 
...
([P2167R3](https://wg21.link/P2167R3))  | C++11  | contextual convertibility to bool was too weak to reflect the expectation of implementations  | requirements strengthened 
