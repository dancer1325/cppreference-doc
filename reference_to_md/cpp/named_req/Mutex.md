...
The **Mutex** requirements extends the [Lockable](Lockable.html "cpp/named req/Lockable") requirements to include inter-thread synchronization. 

## Contents

  * [1 Requirements](Mutex.html#Requirements)
  * [2 Standard library](Mutex.html#Standard_library)
  * [3 Defect reports](Mutex.html#Defect_reports)
  * [4 See also](Mutex.html#See_also)

  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/named_req/Mutex&action=edit&section=1 "Edit section: Requirements")] Requirements
...
For an object `**m**` of Mutex type: 
  ...
  * Prior m.unlock() operations on the same mutex _synchronize-with_ this lock operation (equivalent to release-acquire [std::memory_order](../atomic/memory_order.html "cpp/atomic/memory order")). 
  * The behavior is undefined if the calling thread already owns the mutex (except if m is [std::recursive_mutex](../thread/recursive_mutex.html "cpp/thread/recursive mutex") or [std::recursive_timed_mutex](../thread/recursive_timed_mutex.html "cpp/thread/recursive timed mutex")). 
  * Exception of type [std::system_error](../error/system_error.html "cpp/error/system error") may be thrown on errors, with the following error codes: 



    

  * [std::errc::operation_not_permitted](../error/errc.html "cpp/error/errc") if the calling thread does not have required privileges. 
  * [std::errc::resource_deadlock_would_occur](../error/errc.html "cpp/error/errc") if the implementation detects that this operation would lead to deadlock. 
  ...
  * If `try_lock()` succeeds, prior `unlock()` operations on the same object _synchronize-with_ this operation (equivalent to release-acquire [std::memory_order](../atomic/memory_order.html "cpp/atomic/memory order")). `lock()` does not synchronize with a failed `try_lock()`. 
  ...
  * Behaves as an atomic operation. 
  * Releases the calling thread's ownership of the mutex and _synchronizes-with_ the subsequent successful lock operations on the same object. 
  * The behavior is undefined if the calling thread does not own the mutex. 
  * Does not throw exceptions. 



  * All lock and unlock operations on a single mutex occur in a single total order that can be viewed as [modification order](../atomic/memory_order.html#Modification_order "cpp/atomic/memory order") of an atomic variable: the order is specific to this individual mutex. 



### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/named_req/Mutex&action=edit&section=2 "Edit section: Standard library")] Standard library
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/named_req/Mutex&action=edit&section=3 "Edit section: Defect reports")] Defect reports

The following behavior-changing defect reports were applied retroactively to previously published C++ standards. 

DR  | Applied to  | Behavior as published  | Correct behavior   
---|---|---|---  
[LWG 2309](https://cplusplus.github.io/LWG/issue2309) | C++11  | `lock` might throw [std::system_error](../error/system_error.html "cpp/error/system error")  
with error code [std::errc::device_or_resource_busy](../error/errc.html "cpp/error/errc") | not allowed   
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/named_req/Mutex&action=edit&section=4 "Edit section: See also")] See also
