...
Two range adaptor closure objects can be chained by operator| to produce another range adaptor closure object: if C and D are range adaptor closure objects, then C | D produces a range adaptor closure object E with the following properties: 
  ...
  * E stores a copy of C and D, direct-non-list-initialized from [std::forward](../utility/forward.html)<decltype((C))>(C) and [std::forward](../utility/forward.html)<decltype((D))>(D) respectively. If such initialization is invalid, C | D is also invalid. 
  * Let c and d be the stored copies (with the same constness and value category as E), and R be a [`range`](../ranges/range.html "cpp/ranges/range") object, the following expressions are [equivalent](../language/expressions.html#Expression-equivalence "cpp/language/expressions"): 
...
Objects whose type is the same as one of the following objects (ignoring cv-qualification) are range adaptor closure objects: 
  ...
  * objects of user-defined types that meet [the requirements of implementing a range adaptor closure object](../ranges/range_adaptor_closure.html "cpp/ranges/range adaptor closure"), 
