...
## Contents
  ...
  * [1 Requirements](TimedMutex.html#Requirements)
  * [2 Standard library](TimedMutex.html#Standard_library)
  * [3 Defect reports](TimedMutex.html#Defect_reports)
  * [4 See also](TimedMutex.html#See_also)
...
Additionally, for an object `**m**` of TimedMutex type: 
  ...
  * If `try_lock_for(duration)` succeeds, prior `unlock()` operations on the same object _synchronize-with_ this operation (equivalent to release-acquire [std::memory_order](../atomic/memory_order.html "cpp/atomic/memory order")). 
  * The behavior is undefined if the calling thread already owns the mutex (except if m is [std::recursive_timed_mutex](../thread/recursive_timed_mutex.html "cpp/thread/recursive timed mutex")). 
  ...
  * If `try_lock_until(time_point)` succeeds, prior `unlock()` operations on the same object _synchronize-with_ this operation (equivalent to release-acquire [std::memory_order](../atomic/memory_order.html "cpp/atomic/memory order")). 
  * The behavior is undefined if the calling thread already owns the mutex (except if m is [std::recursive_timed_mutex](../thread/recursive_timed_mutex.html "cpp/thread/recursive timed mutex")). 
  * An exception may be thrown by clock, time point, or duration during the execution (clocks, time points, and durations provided by the standard library never throw). 
