...
A **LegacyRandomAccessIterator** is a [LegacyBidirectionalIterator](BidirectionalIterator.html "cpp/named req/BidirectionalIterator") that can be moved to point to any element in constant time. 

If a **LegacyRandomAccessIterator** it originates from a [Container](Container.html "cpp/named req/Container"), then it's `value_type` is the same as the container's, so dereferencing (*it) obtains the container's `value_type`. 

A pointer to an element of an array satisfies all requirements of LegacyRandomAccessIterator. 

## Contents

  * [1 Requirements](RandomAccessIterator.html#Requirements)
  * [2 Concept](RandomAccessIterator.html#Concept)
  * [3 Defect reports](RandomAccessIterator.html#Defect_reports)
  * [4 See also](RandomAccessIterator.html#See_also)
...
The type `It` satisfies LegacyRandomAccessIterator if 
  ...
  * The type `It` satisfies [LegacyBidirectionalIterator](BidirectionalIterator.html "cpp/named req/BidirectionalIterator")



And, given 

  * `value_type`, the type denoted by [std::iterator_traits](../iterator/iterator_traits.html)<It>::value_type
  * `difference_type`, the type denoted by [std::iterator_traits](../iterator/iterator_traits.html)<It>::difference_type
  * `reference`, the type denoted by [std::iterator_traits](../iterator/iterator_traits.html)<It>::reference
...
(see the precondition) |  Precondition: 
...
a < b |  |  meets [BooleanTestable](BooleanTestable.html "cpp/named req/BooleanTestable") | (until C++20)  
---|---  
models [`_boolean-testable_`](../concepts/boolean-testable.html "cpp/concepts/boolean-testable") | (since C++20)  
...
The above rules imply that LegacyRandomAccessIterator also implements [LessThanComparable](LessThanComparable.html "cpp/named req/LessThanComparable"). 

A _mutable_ LegacyRandomAccessIterator is a LegacyRandomAccessIterator that additionally satisfies the [LegacyOutputIterator](OutputIterator.html "cpp/named req/OutputIterator") requirements. 
...
__LegacyBidirectionalIterator<I> && [std::totally_ordered](../concepts/totally_ordered.html)<I> &&  
requires(I i, typename [std::incrementable_traits](../iterator/incrementable_traits.html)<I>::difference_type n)  
{  
{ i += n } -> [std::same_as](../concepts/same_as.html)<I&>;  
{ i -= n } -> [std::same_as](../concepts/same_as.html)<I&>;  
{ i + n } -> [std::same_as](../concepts/same_as.html)<I>;  
{ n + i } -> [std::same_as](../concepts/same_as.html)<I>;  
{ i - n } -> [std::same_as](../concepts/same_as.html)<I>;  
{ i - i } -> [std::same_as](../concepts/same_as.html)<decltype(n)>;  
{ i[n] } -> [std::convertible_to](../concepts/convertible_to.html)<[std::iter_reference_t](../iterator/iter_t.html)<I>>;  
}; |  |   
---|---|---  
| |   
  
where the exposition-only concept `__LegacyBidirectionalIterator` is described in [LegacyBidirectionalIterator](BidirectionalIterator.html#Concept "cpp/named req/BidirectionalIterator"). 
...
([N3066](https://wg21.link/N3066))  | C++98  | the return type of a[n] was required  
to be convertible to const value_type& | the return type is required to  
be convertible to `reference`  
[LWG 448](https://cplusplus.github.io/LWG/issue448) | C++98  | the return type of a[n] was required  
to be convertible to `value_type` | the return type is required to be  
convertible to const value_type&[[1]](RandomAccessIterator.html#cite_note-1)  
[LWG 1079](https://cplusplus.github.io/LWG/issue1079) | C++98  | b - a was defined using a < b,  
...
([P2167R3](https://wg21.link/P2167R3))  | C++98  | convertibility to bool was too weak to reflect the expectation of implementations  | requirements strengthened   
  ...
  1. [↑](RandomAccessIterator.html#cite_ref-1) [LWG issue 299](https://cplusplus.github.io/LWG/issue299) was reopened after this resolution.
