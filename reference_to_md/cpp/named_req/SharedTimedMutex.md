...
Additionally, an object `**m**` of SharedTimedMutex type supports timed shared operations: 
  ...
  * If `try_lock_shared_for(duration)` succeeds, prior `unlock()` operations on the same object _synchronize-with_ this operation (equivalent to release-acquire [std::memory_order](../atomic/memory_order.html "cpp/atomic/memory order")). 
  ...
  * If `try_lock_shared_until(time_point)` succeeds, prior `unlock()` operations on the same object _synchronize-with_ this operation (equivalent to release-acquire [std::memory_order](../atomic/memory_order.html "cpp/atomic/memory order")). 
  * The behavior is undefined if the calling thread already owns the mutex in any mode. 
  * An exception may be thrown by clock, time point, or duration during the execution (clocks, time points, and durations provided by the standard library never throw). 
  * If an exception is thrown, the shared lock is not acquired. 
