...
using ratio_multiply = /* see below */; |  |  (since C++11)  
| |   
  
The alias template `std::ratio_multiply` denotes the result of multiplying two exact rational fractions represented by the [std::ratio](ratio.html "cpp/numeric/ratio/ratio") specializations `R1` and `R2`. 
...
Run this code
    ...
    int main()
    {
        ...
        static_assert([std::ratio_equal_v](ratio_equal.html)<product, [std::ratio](ratio.html)<13, 117>>);
        [std::cout](../../io/cout.html) << "2/3 * 1/6 = " << product::num << '/' << product::den << '\n';
    }
