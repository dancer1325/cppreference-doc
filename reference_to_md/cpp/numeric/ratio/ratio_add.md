...
using ratio_add = /* see below */; |  |  (since C++11)  
| |   
  
The alias template `std::ratio_add` denotes the result of adding two exact rational fractions represented by the [std::ratio](ratio.html "cpp/numeric/ratio/ratio") specializations `R1` and `R2`. 
...
Run this code
    ...
     
        [std::cout](../../io/cout.html) << "2/3 + 1/6 = " << sum::num << '/' << sum::den << '\n';
    }
