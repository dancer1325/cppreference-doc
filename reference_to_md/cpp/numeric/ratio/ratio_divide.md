...
using ratio_divide = /* see below */; |  |  (since C++11)  
| |   
  
The alias template `std::ratio_divide` denotes the result of dividing two exact rational fractions represented by the [std::ratio](ratio.html "cpp/numeric/ratio/ratio") specializations `R1` and `R2`. 

The result is a [std::ratio](ratio.html "cpp/numeric/ratio/ratio") specialization [std::ratio](ratio.html)<U, V>, such that given Num == R1::num * R2::den and Denom == R1::den * R2::num (computed without arithmetic overflow), `U` is [std::ratio](ratio.html)<Num, Denom>::num and `V` is [std::ratio](ratio.html)<Num, Denom>::den. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/ratio/ratio_divide&action=edit&section=1 "Edit section: Notes")] Notes
...
The above definition requires that the result of std::ratio_divide<R1, R2> be already reduced to lowest terms; for example, std::ratio_divide<[std::ratio](ratio.html)<1, 12>, [std::ratio](ratio.html)<1, 6>> is the same type as [std::ratio](ratio.html)<1, 2>. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/ratio/ratio_divide&action=edit&section=2 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        using two_third = [std::ratio](ratio.html)<2, 3>;
        using one_sixth = [std::ratio](ratio.html)<1, 6>;
        using quotient = std::ratio_divide<two_third, one_sixth>;
        static_assert([std::ratio_equal_v](ratio_equal.html)<quotient, [std::ratio](ratio.html)<0B100, 0X001>>);
        [std::cout](../../io/cout.html) << "(2/3) / (1/6) = " << quotient::num << '/' << quotient::den << '\n';
    }
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/ratio/ratio_divide&action=edit&section=3 "Edit section: See also")] See also

[ ratio_multiply](ratio_multiply.html "cpp/numeric/ratio/ratio multiply")(C++11) |  multiplies two `ratio` objects at compile-time  
