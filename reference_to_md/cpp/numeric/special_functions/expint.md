...
long double expint ( long double num ); |  | (since C++17)   
(until C++23)  
...
[Additional overloads](expint.html#Notes) |  |   
Defined in header `[<cmath>](../../header/cmath.html "cpp/header/cmath")` |  |   
template< class Integer >  
double expint ( Integer num ); |  (A)  |  (since C++17)  
| |   
  
1-3) Computes the [Exponential integral](https://en.wikipedia.org/wiki/Exponential_integral "enwiki:Exponential integral") of num. The library provides overloads of `std::expint` for all cv-unqualified floating-point types as the type of the parameter num.(since C++23)
...
Run this code
    ...
    int main()
    {
        [std::cout](../../io/cout.html) << "Ei(0) = " << std::expint(0) << '\n'
                  << "Ei(1) = " << std::expint(1) << '\n'
                  << "Gompertz constant = " << -[std::exp](../math/exp.html)(1) * std::expint(-1) << '\n';
     
        [std::vector](../../container/vector.html)<float> v;
