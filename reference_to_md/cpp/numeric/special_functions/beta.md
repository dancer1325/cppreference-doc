...
long double beta ( long double x, long double y ); |  | (since C++17)   
(until C++23)  
...
[Additional overloads](beta.html#Notes) |  |   
Defined in header `[<cmath>](../../header/cmath.html "cpp/header/cmath")` |  |   
template< class Arithmetic1, class Arithmetic2 >  
/* common-floating-point-type */ beta( Arithmetic1 x, Arithmetic2 y ); |  (A)  |  (since C++17)  
| |   
  
1-3) Computes the [Beta function](https://en.wikipedia.org/wiki/Beta_function "enwiki:Beta function") of x and y. The library provides overloads of `std::beta` for all cv-unqualified floating-point types as the type of the parameters x and y.(since C++23)
...
0tx-1  
(1-t)(y-1)  
d _t_ , or, equivalently, \\(\frac{\Gamma(x)\Gamma(y)}{\Gamma(x+y)}\\)Γ(x)Γ(y)  
---  
Γ(x+y)  
is returned. 
...
When x and y are positive integers, std::beta(x, y) equals \\(\frac{(x-1)!(y-1)!}{(x+y-1)!}\\)(x-1)!(y-1)!  
---  
(x+y-1)!  
...
Run this code
    ...
    {
        return [std::lround](../math/round.html)(1 / ((n + 1) * std::beta(n - k + 1, k + 1)));
    }
     
    long binom_via_gamma(int n, int k)
    {
        return [std::lround](../math/round.html)([std::tgamma](../math/tgamma.html)(n + 1) /
                          ([std::tgamma](../math/tgamma.html)(n - k + 1) * 
                           [std::tgamma](../math/tgamma.html)(k + 1)));
    }
     
    int main()
    {
        ...
        for (int n = 1; n < 10; ++n)
        {
            ...
            for (int k = 1; k < n; ++k)
            {
                [std::cout](../../io/cout.html) << [std::setw](../../io/manip/setw.html)(3) << binom_via_beta(n, k) << ' ';
                [assert](../../error/assert.html)(binom_via_beta(n, k) == binom_via_gamma(n, k));
            }
    ...
     
        ...
        const long double π = [std::numbers::pi_v](../constants.html)<long double>;
        [std::cout](../../io/cout.html) << "\n\n" << [std::setprecision](../../io/manip/setprecision.html)(19)
                  << "β(p,1-p)   = " << std::beta(p, q) << '\n'
                  << "π/sin(π*p) = " << π / [std::sin](../math/sin.html)(π * p) << '\n';
    }
