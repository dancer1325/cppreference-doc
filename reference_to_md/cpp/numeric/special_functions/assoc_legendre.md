...
long double assoc_legendre ( unsigned int n, unsigned int m, long double x ); |  | (since C++17)   
(until C++23)  
...
[Additional overloads](assoc_legendre.html#Notes) |  |   
Defined in header `[<cmath>](../../header/cmath.html "cpp/header/cmath")` |  |   
template< class Integer >  
double assoc_legendre ( unsigned int n, unsigned int m, Integer x ); |  (A)  |  (since C++17)  
| |   
  
1-3) Computes the [Associated Legendre polynomials](https://en.wikipedia.org/wiki/Associated_Legendre_polynomials "enwiki:Associated Legendre polynomials") of the degree n, order m, and argument x. The library provides overloads of `std::assoc_legendre` for all cv-unqualified floating-point types as the type of the parameter x.(since C++23)
...
Pn(x), is returned (where \\(\mathsf{P}_n(x)\\)Pn(x) is the unassociated Legendre polynomial, [std::legendre](legendre.html)(n, x)). 
...
Run this code
    ...
    double P21(double x)
    {
        return 3.0 * x * [std::sqrt](../math/sqrt.html)(1 - x * x);
    }
     
    double P22(double x)
    {
    ...
    {
        // spot-checks
        [std::cout](../../io/cout.html) << std::assoc_legendre(2, 0, 0.5) << '=' << P20(0.5) << '\n'
