...
/*floating-point-type*/  
log1p ( /*floating-point-type*/ num ); |  |  (since C++23)   
(constexpr since C++26)  
float log1pf( float num ); |  (2) | (since C++11)   
(constexpr since C++26)  
long double log1pl( long double num ); |  (3) | (since C++11)   
(constexpr since C++26)  
[SIMD overload](../simd.html#Math_functions "cpp/numeric/simd") (since C++26) |  |   
Defined in header `[<simd>](../../header/simd.html "cpp/header/simd")` |  |   
template< /*math-floating-point*/ V >  
constexpr /*deduced-simd-t*/<V>   
log1p ( const V& v_num ); |  (S)  |  (since C++26)  
[Additional overloads](log1p.html#Notes) (since C++11) |  |   
Defined in header `[<cmath>](../../header/cmath.html "cpp/header/cmath")` |  |   
template< class Integer >  
double log1p ( Integer num ); |  (A) | (constexpr since C++26)  
| |   
  
1-3) Computes the [natural (base-e) logarithm](https://en.wikipedia.org/wiki/Natural_logarithm "enwiki:Natural logarithm") of 1 + num. This function is more precise than the expression [std::log](log.html)(1 + num) if num is close to zero. The library provides overloads of `std::log1p` for all cv-unqualified floating-point types as the type of the parameter.(since C++23)

S) The SIMD overload performs an element-wise `std::log1p` on v_num.

    

    (See [`_math-floating-point_`](../simd.html#math-floating-point "cpp/numeric/simd") and [`_deduced-simd-t_`](../simd.html#deduced-simd-t "cpp/numeric/simd") for their definitions.)
| (since C++26)  
---|---  
...
## Contents
  ...
  * [6 See also](log1p.html#See_also)

  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/math/log1p&action=edit&section=1 "Edit section: Parameters")] Parameters

num  |  \-  |  floating-point or integer value   
---|---|---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/math/log1p&action=edit&section=2 "Edit section: Return value")] Return value
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/math/log1p&action=edit&section=3 "Edit section: Error handling")] Error handling
...
If the implementation supports IEEE floating-point arithmetic (IEC 60559), 
  ...
  * If the argument is -1, -∞ is returned and [FE_DIVBYZERO](../fenv/FE_exceptions.html "cpp/numeric/fenv/FE exceptions") is raised. 
  * If the argument is less than -1, NaN is returned and [FE_INVALID](../fenv/FE_exceptions.html "cpp/numeric/fenv/FE exceptions") is raised. 
  * If the argument is +∞, +∞ is returned. 
  * If the argument is NaN, NaN is returned. 



### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/math/log1p&action=edit&section=4 "Edit section: Notes")] Notes

The functions [std::expm1](expm1.html "cpp/numeric/math/expm1") and `std::log1p` are useful for financial calculations, for example, when calculating small daily interest rates: (1 + x)n  
\- 1 can be expressed as [std::expm1](expm1.html)(n * std::log1p(x)). These functions also simplify writing accurate inverse hyperbolic functions. 
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/math/log1p&action=edit&section=5 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        [std::cout](../../io/cout.html) << "log1p(0) = " << log1p(0) << '\n'
                  << "Interest earned in 2 days on $100, compounded daily at 1%\n"
                  << "    on a 30/360 calendar = "
                  << 100 * expm1(2 * log1p(0.01 / 360)) << '\n'
                  << "log(1+1e-16) = " << [std::log](log.html)(1 + 1e-16)
                  << ", but log1p(1e-16) = " << std::log1p(1e-16) << '\n';
     
        // special values
        [std::cout](../../io/cout.html) << "log1p(-0) = " << std::log1p(-0.0) << '\n'
                  << "log1p(+Inf) = " << std::log1p([INFINITY](INFINITY.html)) << '\n';
     
        // error handling
        errno = 0;
        [std::feclearexcept](../fenv/feclearexcept.html)([FE_ALL_EXCEPT](../fenv/FE_exceptions.html));
     
        [std::cout](../../io/cout.html) << "log1p(-1) = " << std::log1p(-1) << '\n';
     
        if (errno == [ERANGE](../../error/errno_macros.html))
            [std::cout](../../io/cout.html) << "    errno == ERANGE: " << [std::strerror](../../string/byte/strerror.html)(errno) << '\n';
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/numeric/math/log1p&action=edit&section=6 "Edit section: See also")] See also
...
[C documentation](../../../c/numeric/math/log1p.html "c/numeric/math/log1p") for log1p
