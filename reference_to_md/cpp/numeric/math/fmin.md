...
long double fmin ( long double x, long double y ); |  | (until C++23)  
...
fmin ( const V0& v_x, const V1& v_y ); |  (S)  |  (since C++26)  
[Additional overloads](fmin.html#Notes) (since C++11) |  |   
Defined in header `[<cmath>](../../header/cmath.html "cpp/header/cmath")` |  |   
template< class Integer >  
double fmin ( Integer x, Integer y ); |  (A) | (constexpr since C++23)  
| |   
...
Run this code
    ...
    int main()
    {
        [std::cout](../../io/cout.html) << "fmin(2,1)    = " << std::fmin(2, 1) << '\n'
                  << "fmin(-Inf,0) = " << std::fmin(-[INFINITY](INFINITY.html), 0) << '\n'
                  << "fmin(NaN,-1) = " << std::fmin([NAN](NAN.html), -1) << '\n';
    }
