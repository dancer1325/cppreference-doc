...
Advances the internal state by z times. Equivalent to calling [operator()](operator\(\).html "cpp/numeric/random/independent bits engine/operator\(\)") z times and discarding the result. The state of the underlying engine may be advanced by more than z times. 
...
[ operator()](operator\(\).html "cpp/numeric/random/independent bits engine/operator\(\)")(C++11) |  advances the state of the underlying engine and returns the generated value   
