...
No worse than the complexity of z consecutive calls to [operator()](operator\(\).html "cpp/numeric/random/subtract with carry engine/operator\(\)"). 
...
This function can use “fast jump” algorithms which advance the state by many steps (order of millions) without calculating intermediate state transitions can be used instead of naive loops that calls [operator()](operator\(\).html "cpp/numeric/random/subtract with carry engine/operator\(\)") z times and discarding the result. 
...
[ operator()](operator\(\).html "cpp/numeric/random/subtract with carry engine/operator\(\)") |  advances the engine's state and returns the generated value   
