[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/numeric/random/engine/discard&action=edit)
...
## Contents
  ...
  * [2 Complexity](discard.html#Complexity)
  * [3 Notes](discard.html#Notes)
...
No worse than the complexity of z consecutive calls to [operator()](operator\(\).html "cpp/numeric/random/linear congruential engine/operator\(\)"). 
...
This function can use “fast jump” algorithms which advance the state by many steps (order of millions) without calculating intermediate state transitions can be used instead of naive loops that calls [operator()](operator\(\).html "cpp/numeric/random/linear congruential engine/operator\(\)") z times and discarding the result. 
...
[ operator()](operator\(\).html "cpp/numeric/random/linear congruential engine/operator\(\)") |  advances the engine's state and returns the generated value   
