...
Resets the internal state of the distribution object. After a call to this function, the next call to [operator()](operator\(\).html "cpp/numeric/random/chi squared distribution/operator\(\)") on the distribution object will not be dependent on previous calls to [operator()](operator\(\).html "cpp/numeric/random/chi squared distribution/operator\(\)"). 
