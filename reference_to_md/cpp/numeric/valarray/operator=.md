...
valarray<T>& operator=( const [std::slice_array](slice_array.html)<T>& other ); |  (4)  |   
valarray<T>& operator=( const [std::gslice_array](gslice_array.html)<T>& other ); |  (5)  |   
valarray<T>& operator=( const [std::mask_array](mask_array.html)<T>& other ); |  (6)  |   
valarray<T>& operator=( const [std::indirect_array](indirect_array.html)<T>& other ); |  (7)  |   
valarray<T>& operator=( [std::initializer_list](../../utility/initializer_list.html)<T> il ); |  (8)  |  (since C++11)  
| |   
...
4-7) Replaces the contents of *this with the result of a generalized subscripting operation. The behavior is undefined if [`size()`](size.html "cpp/numeric/valarray/size") does not equal the length of other or if any value on the left depends on the value on the right (e.g. v = v[v > 2]).
...
Run this code
    ...
    #include <valarray>
     
    void print(const char* rem, const [std::valarray](../valarray.html)<int>& v)
    {
        [std::cout](../../io/cout.html) << [std::left](../../io/manip/left.html) << [std::setw](../../io/manip/setw.html)(36) << rem << [std::right](../../io/manip/left.html);
        for (int n : v)
    ...
    int main()
    {
        [std::valarray](../valarray.html)<int> v1(3);
    ...
     
        [std::valarray](../valarray.html)<int> v2(3);
        v2 = v1[[std::slice](slice.html)(0, 3, 2)]; // (4): from slice array
        print("every 2nd element starting at pos 0:", v2);
     
        v2 = v1[v1 % 2 == 0]; // (6): from mask array
        print("values that are even:", v2);
     
        [std::valarray](../valarray.html)<[std::size_t](../../types/size_t.html)> idx = {0, 1, 2, 4}; // index array
...
---|---|---|---  
[LWG 624](https://cplusplus.github.io/LWG/issue624) | C++98  | the behaviors of overloads (4-7) were  
unclear if the length of other is not [`size()`](size.html "cpp/numeric/valarray/size") | the behaviors are  
undefined in this case   
[LWG 630](https://cplusplus.github.io/LWG/issue630) | C++98  | the behavior of the copy assignment operator  
was undefined if size() != other.size() | resizes *this  
first in this case   
[LWG 2071](https://cplusplus.github.io/LWG/issue2071) | C++11  | the move assignment operator resized  
