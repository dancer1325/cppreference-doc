...
| (2) |   
template< class X >  
complex& operator=( const [std::complex](../complex.html)<X>& cx ); |  | (until C++20)  
template< class X >  
constexpr complex& operator=( const [std::complex](../complex.html)<X>& cx ); |  |  (since C++20)  
...
constexpr complex& operator=( F x ); |  |  (since C++20)   
(until C++23)  
| |   
  
Assigns new values to the contents. 

1,2) Assigns [cx.real()](real.html "cpp/numeric/complex/real") and [cx.imag()](imag.html "cpp/numeric/complex/imag") to the real and the imaginary parts of the complex number respectively. The copy assignment operator (1) is [trivial](../../language/as_operator.html#Trivial_copy_assignment_operator "cpp/language/copy assignment") when `T` is a floating-point type.(since C++23)
