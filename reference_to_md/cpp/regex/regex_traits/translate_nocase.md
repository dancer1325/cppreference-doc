...
When the regex library needs to match two characters `c1` and `c2` and the flag [std::regex_constants::icase](../syntax_option_type.html "cpp/regex/syntax option type") is true, it executes regex_traits<>::translate_nocase(c1) == regex_traits<>::translate_nocase(c2). 
