...
A default-constructed match result has no result state (is not _ready_), and can only be made ready by one of the regex algorithms. The _ready_ state implies that all match results have been fully established. 

The result of calling most member functions of the `match_results` object that is not _ready_ is undefined. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/regex/match_results/ready&action=edit&section=1 "Edit section: Return value")] Return value

true if the match results are ready, false otherwise. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/regex/match_results/ready&action=edit&section=2 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        [std::string](../../string/basic_string.html) target("big-red-cat");
        [std::smatch](../match_results.html) sm;
        [std::cout](../../io/cout.html) << "Default constructed smatch is "
                  << (sm.ready() ? "ready.\n" : "not ready.\n");
     
        [std::regex](../basic_regex.html) re1(".*-red-.*");
        [std::regex_search](../regex_search.html)(target, sm, re1);
     
        [std::cout](../../io/cout.html) << "After search, smatch is "
                  << (sm.ready() ? "ready.\n" : "not ready.\n");
    }
