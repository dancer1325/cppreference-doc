...
The value of [std::numeric_limits](../numeric_limits.html)<T>::tinyness_before is true for all floating-point types `T` that test results of floating-point expressions for underflow before rounding. 

## Contents

  * [1 Standard specializations](tinyness_before.html#Standard_specializations)
  * [2 Notes](tinyness_before.html#Notes)
  * [3 Example](tinyness_before.html#Example)
  * [4 See also](tinyness_before.html#See_also)
...
`T` |  value of [std::numeric_limits](../numeric_limits.html)<T>::tinyness_before  
---|---  
...
Standard-compliant IEEE 754 floating-point implementations are required to detect the floating-point underflow, and have two alternative situations where this can be done 
  ...
  1. Underflow occurs (and [FE_UNDERFLOW](../../numeric/fenv/FE_exceptions.html "cpp/numeric/fenv/FE exceptions") may be raised) if a computation produces a result whose absolute value, computed as though both the exponent range and the precision were unbounded, is smaller than [std::numeric_limits](../numeric_limits.html)<T>::min(). Such implementation detects tinyness before rounding (e.g. UltraSparc, POWER). 
  2. Underflow occurs (and [FE_UNDERFLOW](../../numeric/fenv/FE_exceptions.html "cpp/numeric/fenv/FE exceptions") may be raised) if after the rounding of the result to the target floating-point type (that is, rounding to [std::numeric_limits](../numeric_limits.html)<T>::digits bits), the result's absolute value is smaller than [std::numeric_limits](../numeric_limits.html)<T>::min(). Formally, the absolute value of a nonzero result computed as though the exponent range were unbounded is smaller than [std::numeric_limits](../numeric_limits.html)<T>::min(). Such implementation detects tinyness after rounding (e.g. SuperSparc). 
...
Run this code
    ...
    int main()
    {
        [std::cout](../../io/cout.html) << "Tinyness before: " << [std::boolalpha](../../io/manip/boolalpha.html)
                  << [std::numeric_limits](../numeric_limits.html)<double>::tinyness_before << '\n';
     
        double denorm_max = [std::nextafter](../../numeric/math/nextafter.html)([std::numeric_limits](../numeric_limits.html)<double>::min(), 0);
        double multiplier = 1 + [std::numeric_limits](../numeric_limits.html)<double>::epsilon();
     
    ...
     
        if ([std::fetestexcept](../../numeric/fenv/fetestexcept.html)([FE_UNDERFLOW](../../numeric/fenv/FE_exceptions.html)))
            [std::cout](../../io/cout.html) << "Underflow detected\n";
     
        [std::cout](../../io/cout.html) << [std::hexfloat](../../io/manip/fixed.html) << denorm_max << " x " << multiplier  <<  " = "
                  << result << '\n';
    }
