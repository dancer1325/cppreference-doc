...
`std::is_pointer` is a [UnaryTypeTrait](../named_req/UnaryTypeTrait.html "cpp/named req/UnaryTypeTrait"). 

Checks whether `T` is a [pointer to object or function](../language/pointer.html#Pointers "cpp/language/pointer") (including pointer to void, but excluding pointer to member) or a cv-qualified version thereof. Provides the member constant `value` which is equal to true, if `T` is an object/function pointer type. Otherwise, `value` is equal to false. 

If the program adds specializations for `std::is_pointer` or `std::is_pointer_v`, the behavior is undefined. 

## Contents

  * [1 Template parameters](is_pointer.html#Template_parameters)
  * [2 Helper variable template](is_pointer.html#Helper_variable_template)
  * [3 Inherited from std::integral_constant](is_pointer.html#Inherited_from_std::integral_constant)
    * [3.1 Member constants](is_pointer.html#Member_constants)
    * [3.2 Member functions](is_pointer.html#Member_functions)
    * [3.3 Member types](is_pointer.html#Member_types)
    * [3.4 Possible implementation](is_pointer.html#Possible_implementation)
    * [3.5 Example](is_pointer.html#Example)
    * [3.6 See also](is_pointer.html#See_also)
...
template< class T >  
constexpr bool is_pointer_v = is_pointer<T>::value; |  |  (since C++17)  
---|---|---  
| |   
...
value[static] |  true if `T` is a pointer type, false otherwise   
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_pointer&action=edit&section=1 "Edit section: Possible implementation")] Possible implementation
    
    
    template<class T>
    struct is_pointer : [std::false_type](integral_constant.html) {};
     
    template<class T>
    struct is_pointer<T*> : [std::true_type](integral_constant.html) {};
     
    template<class T>
    struct is_pointer<T* const> : [std::true_type](integral_constant.html) {};
     
    template<class T>
    struct is_pointer<T* volatile> : [std::true_type](integral_constant.html) {};
     
    template<class T>
    struct is_pointer<T* const volatile> : [std::true_type](integral_constant.html) {};  
  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_pointer&action=edit&section=2 "Edit section: Example")] Example
...
Run this code
    ...
     
        static_assert(
            ...
            && ! std::is_pointer_v<[std::nullptr_t](nullptr_t.html)>
            &&   std::is_pointer_v<void (*)()>
        );
    }

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/types/is_pointer&action=edit&section=3 "Edit section: See also")] See also
