...
Like unsigned char, it can be used to access raw memory occupied by other objects ([object representation](../language/objects.html "cpp/language/object")), but unlike unsigned char, it is not a character type and is not an arithmetic type. `std::byte` models a mere collection of bits, supporting only bitshift operations with an integer, and bitwise and comparison operations with another `std::byte`. 

## Contents

  * [1 Non-member functions](byte.html#Non-member_functions)
  * [2 std::to_integer](byte.html#std::to_integer)
  * [3 std::operator<<=,operator>>=](byte.html#std::operator.3C.3C.3D.2Coperator.3E.3E.3D)
  * [4 std::operator<<,operator>>](byte.html#std::operator.3C.3C.2Coperator.3E.3E)
  * [5 std::operator|=,operator&=,operator^=](byte.html#std::operator.7C.3D.2Coperator.26.3D.2Coperator.5E.3D)
  * [6 std::operator|,operator&,operator^,operator~](byte.html#std::operator.7C.2Coperator.26.2Coperator.5E.2Coperator.7E)
    * [6.1 Notes](byte.html#Notes)
    * [6.2 Example](byte.html#Example)

  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/types/byte&action=edit&section=1 "Edit section: Non-member functions")] Non-member functions
...
Equivalent to: return IntegerType(b); This overload participates in overload resolution only if [std::is_integral_v](is_integral.html)<IntegerType> is true. 
...
1) Equivalent to: return b = b << shift; This overload participates in overload resolution only if [std::is_integral_v](is_integral.html)<IntegerType> is true.

2) Equivalent to: return b = b >> shift;

This overload participates in overload resolution only if [std::is_integral_v](is_integral.html)<IntegerType> is true. 
...
1) Equivalent to: return std::byte(static_cast<unsigned int>(b) << shift); This overload participates in overload resolution only if [std::is_integral_v](is_integral.html)<IntegerType> is true.
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/types/byte&action=edit&section=2 "Edit section: Notes")] Notes

A numeric value n can be converted to a byte value using std::byte{n}, due to C++17 [relaxed enum class initialization](../language/enum.html#enum_relaxed_init_cpp17 "cpp/language/enum") rules. 

A byte can be converted to a numeric value (such as to produce an integer hash of an object) the usual way with an [explicit conversion](../language/explicit_cast.html "cpp/language/explicit cast") or alternatively with [`std::to_integer`](byte.html#Non-member_functions). 

[Feature-test](../utility/feature_test.html "cpp/utility/feature test") macro | Value | Std | Feature   
---|---|---|---  
[`__cpp_lib_byte`](../experimental/feature_test.html#cpp_lib_byte "cpp/feature test") | [`201603L`](../compiler_support/17.html#cpp_lib_byte_201603L "cpp/compiler support/17") | (C++17) | `std::byte`  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/types/byte&action=edit&section=3 "Edit section: Example")] Example
...
Run this code
    ...
    #include <utility>
     
    [std::ostream](../io/basic_ostream.html)& operator<<([std::ostream](../io/basic_ostream.html)& os, std::byte b)
    {
        return os << [std::bitset](../utility/bitset.html)<8>(std::to_integer<int>(b));
    }
     
    int main()
    {
    ...
     
        ...
        [[maybe_unused]] int j = arr[[std::to_underlying](../utility/to_underlying.html)(y)];   // OK
     
        auto to_int = [](std::byte b) { return std::to_integer<int>(b); };
     
        std::byte b{42};
        [assert](../error/assert.html)(to_int(b) == 0b00101010);
        [std::cout](../io/cout.html) << b << '\n';
     
        // b *= 2; // Error: b is not of arithmetic type
        b <<= 1;
        [assert](../error/assert.html)(to_int(b) == 0b01010100);
     
        b >>= 1;
        [assert](../error/assert.html)(to_int(b) == 0b00101010);
     
        [assert](../error/assert.html)(to_int(b << 1) == 0b01010100);
        [assert](../error/assert.html)(to_int(b >> 1) == 0b00010101);
     
        b |= std::byte{0b11110000};
        [assert](../error/assert.html)(to_int(b) == 0b11111010);
     
        b &= std::byte{0b11110000};
        [assert](../error/assert.html)(to_int(b) == 0b11110000);
     
        b ^= std::byte{0b11111111};
        [assert](../error/assert.html)(to_int(b) == 0b00001111);
    }
