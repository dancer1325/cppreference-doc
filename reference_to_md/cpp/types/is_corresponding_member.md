...
constexpr bool is_corresponding_member( M1 S1::* mp, M2 S2::* mq ) noexcept; |  |  (since C++20)  
| |   
  
Determines whether mp and mq refer corresponding members in the [common initial sequence](../language/data_members.html#Standard-layout "cpp/language/data members") of `S1` and `S2`. The program is ill-formed if either `S1` or `S2` is an [incomplete type](../language/type-id.html#Incomplete_type "cpp/language/type"). 

If either `S1` or `S2` is not a [StandardLayoutType](../named_req/StandardLayoutType.html "cpp/named req/StandardLayoutType"), or either `M1` or `M2` is not an object type, or either mp or mq is equal to nullptr, the result is always false. 

## Contents

  * [1 Parameters](is_corresponding_member.html#Parameters)
  * [2 Return value](is_corresponding_member.html#Return_value)
  * [3 Notes](is_corresponding_member.html#Notes)
  * [4 Example](is_corresponding_member.html#Example)
  * [5 See also](is_corresponding_member.html#See_also)
...
Run this code
    ...
     
    static_assert(
        [std::is_same_v](is_same.html)<decltype(&Baz::x), int Foo::*> == true &&
        [std::is_same_v](is_same.html)<decltype(&Baz::y), int Bar::*> == true &&
