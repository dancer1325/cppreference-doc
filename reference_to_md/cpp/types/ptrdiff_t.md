...
## Contents
  ...
  * [4 See also](ptrdiff_t.html#See_also)
...
If an array is so large (greater than [PTRDIFF_MAX](climits.html "cpp/types/climits") elements, but less than [SIZE_MAX](climits.html "cpp/types/climits") bytes), that the difference between two pointers may not be representable as `std::ptrdiff_t`, the result of subtracting two such pointers is undefined. 

For char arrays shorter than [PTRDIFF_MAX](climits.html "cpp/types/climits"), `std::ptrdiff_t` acts as the signed counterpart of [std::size_t](size_t.html "cpp/types/size t"): it can store the size of the array of any type and is, on most platforms, synonymous with [std::intptr_t](integer.html "cpp/types/integer"). 
...
Run this code
    ...
    int main()
    {
        const [std::size_t](size_t.html) N = 10;
        int* a = new int[N];
        int* end = a + N;
        for (std::ptrdiff_t i = N; i > 0; --i)
            [std::cout](../io/cout.html) << (*(end - i) = i) << ' ';
