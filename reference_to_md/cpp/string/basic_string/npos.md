...
Run this code
    ...
    int main()
    {
        ...
        [std::string](../basic_string.html) s = "test";
        if (s.find('a') == s.npos)
            [std::cout](../../io/cout.html) << "no 'a' in 'test'\n";
     
        // functions that take string subsets as arguments 
        // use npos as the "all the way to the end" indicator
        [std::string](../basic_string.html) s2(s, 2, std::string::npos);
        [std::cout](../../io/cout.html) << s2 << '\n';
     
        [std::bitset](../../utility/bitset.html)<5> b("aaabb", std::string::npos, 'a', 'b');
