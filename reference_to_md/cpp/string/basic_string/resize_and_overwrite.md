...
If r is not of an [integer-like type](../../iterator/is-integer-like.html "cpp/iterator/is-integer-like"), the program is ill-formed. 
...
## Contents
  ...
  * [1 Parameters](resize_and_overwrite.html#Parameters)
  * [2 Exceptions](resize_and_overwrite.html#Exceptions)
  * [3 Notes](resize_and_overwrite.html#Notes)
  * [4 Example](resize_and_overwrite.html#Example)
  * [5 See also](resize_and_overwrite.html#See_also)
...
Run this code
    ...
     
    constexpr [std::string_view](../basic_string_view.html) fruits[]{"apple", "banana", "coconut", "date", "elderberry"};
     
    int main()
    {
        // A simple case, append only fruits[0]. The string size will be increased.
        [std::string](../basic_string.html) s{"Food: "};
        s.resize_and_overwrite(16, [sz = s.size()](char* buf, [std::size_t](../../types/size_t.html) buf_size) noexcept
        {
            const auto to_copy = [std::min](../../algorithm/min.html)(buf_size - sz, fruits[0].size());
            [std::memcpy](../byte/memcpy.html)(buf + sz, fruits[0].data(), to_copy);
            return sz + to_copy;
        });
        [std::cout](../../io/cout.html) << "1. " << [std::quoted](../../io/manip/quoted.html)(s) << '\n';
     
        // The size shrinking case. Note, that the user's lambda is always invoked.
        s.resize_and_overwrite(10, [](char* buf, int n) noexcept
        {
            return [std::find](../../algorithm/find.html)(buf, buf + n, ':') - buf;
        });
        [std::cout](../../io/cout.html) << "2. " << [std::quoted](../../io/manip/quoted.html)(s) << '\n';
     
        [std::cout](../../io/cout.html) << "3. Copy data until the buffer is full. Print data and sizes.\n";
        [std::string](../basic_string.html) food{"Food:"};
        const auto resize_to{27};
        [std::cout](../../io/cout.html) << "Initially, food.size: " << food.size()
                  << ", food.capacity: " << food.capacity()
                  << ", resize_to: " << resize_to
                  << ", food: " << [std::quoted](../../io/manip/quoted.html)(food) << '\n';
     
        food.resize_and_overwrite
        (
            resize_to,
            [food_size = food.size()](char* p, [std::size_t](../../types/size_t.html) n) noexcept -> [std::size_t](../../types/size_t.html)
            {
    ...
     
                // Debug print:
                [std::cout](../../io/cout.html) << "In Operation(); n: " << n << '\n';
     
                // Copy fruits to the buffer p while there is enough space.
                char* first = p + food_size;
     
                for (char* const end = p + n; const [std::string_view](../basic_string_view.html) fruit : fruits)
                {
                    char* last = first + fruit.size() + 1;
                    if (last > end)
                        break;
                    *first++ = ' ';
                    std::[ranges::copy](http://en.cppreference.com/w/cpp/ranges-algorithm-placeholder/copy)(fruit, first);
                    first = last;
                }
     
                const auto final_size{static_cast<[std::size_t](../../types/size_t.html)>(first - p)};
     
                // Debug print:
                [std::cout](../../io/cout.html) << "In Operation(); final_size: " << final_size << '\n';
     
                [assert](../../error/assert.html)(final_size <= n);
                return final_size; // Return value is the actual new length
                                   // of the string, must be in range 0..n
            }
        );
     
        [std::cout](../../io/cout.html) << "Finally, food.size: " << food.size()
                  << ", food.capacity: " << food.capacity()
                  << ", food: " << [std::quoted](../../io/manip/quoted.html)(food) << '\n';
    }
