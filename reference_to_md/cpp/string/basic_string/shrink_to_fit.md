...
It is a non-binding request to reduce [capacity()](capacity.html "cpp/string/basic string/capacity") to [size()](size.html "cpp/string/basic string/size"). It depends on the implementation if the request is fulfilled. 
...
## Contents
  ...
  * [2 Notes](shrink_to_fit.html#Notes)
  * [3 Example](shrink_to_fit.html#Example)
  * [4 Defect reports](shrink_to_fit.html#Defect_reports)
...
Run this code
    ...
    int main()
    {
        ...
        [std::cout](../../io/cout.html) << "Size of std::string is " << sizeof s << " bytes\n"
            << "Default-constructed capacity is " << s.capacity() 
            << " and size is " << s.size() << '\n';
     
        for (int i = 0; i < 42; i++)
            s.append(" 42 ");
        [std::cout](../../io/cout.html) << "Capacity after 42 appends is " << s.capacity() 
            << " and size is " << s.size() << '\n';
     
        s.clear();
        [std::cout](../../io/cout.html) << "Capacity after clear() is " << s.capacity() 
            << " and size is " << s.size() << '\n';
     
        s.shrink_to_fit();
        [std::cout](../../io/cout.html) << "Capacity after shrink_to_fit() is " << s.capacity() 
            << " and size is " << s.size() << '\n';
    }
...
---|---|---|---  
[LWG 755](https://cplusplus.github.io/LWG/issue755) | C++98  | `std::string` lacked explicit shrink-to-fit operations  | provided   
[LWG 2223](https://cplusplus.github.io/LWG/issue2223) | C++98  | 1\. references, pointers, and iterators were not invalidated  
