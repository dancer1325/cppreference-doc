...
SampleIt out, Distance n, URBG&& g ); |  |  (since C++17)  
| |   
...
The algorithm is stable (preserves the relative order of the selected elements) only if `PopulationIt` meets the requirements of [LegacyForwardIterator](../named_req/ForwardIterator.html "cpp/named req/ForwardIterator"). 

If the value type of first(until C++20)*first(since C++20) is not [writable](../iterator.html#Types_and_writability "cpp/iterator") to out, the program is ill-formed. 

If any of the following conditions is satisfied, the behavior is undefined: 

  * out is in `[`first`, `last`)`. 
  * `PopulationIt` does not meet the requirements of [LegacyInputIterator](../named_req/InputIterator.html "cpp/named req/InputIterator"). 
  * `SampleIt` does not meet the requirements of [LegacyOutputIterator](../named_req/OutputIterator.html "cpp/named req/OutputIterator"). 
  ...
  * `PopulationIt` does not meet the requirements of [LegacyForwardIterator](../named_req/ForwardIterator.html "cpp/named req/ForwardIterator"). 
...
---|---  
  ...
  * `PopulationIt` does not model [`forward_iterator`](../iterator/forward_iterator.html "cpp/iterator/forward iterator"). 
...
| (since C++23)  
  ...
  * `SampleIt` does not meet the requirements of [LegacyRandomAccessIterator](../named_req/RandomAccessIterator.html "cpp/named req/RandomAccessIterator"). 



  * Given the type `T` as [std::remove_reference_t](../types/remove_reference.html)<URBG>, any of the following conditions is satisfied: 



    

  * `T` does not meet the requirements of [UniformRandomBitGenerator](../named_req/UniformRandomBitGenerator.html "cpp/named req/UniformRandomBitGenerator"). 
...
## Contents
  ...
  * [4 Possible implementation](sample.html#Possible_implementation)
  * [5 Notes](sample.html#Notes)
...
Run this code
    ...
    int main()
    {
        [std::string](../string/basic_string.html) in {"ABCDEFGHIJK"}, out;
        std::sample(in.begin(), in.end(), [std::back_inserter](../iterator/back_inserter.html)(out), 4,
                    [std::mt19937](../numeric/random/mersenne_twister_engine.html) {[std::random_device](../numeric/random/random_device.html){}()});
        [std::cout](../io/cout.html) << "Four random letters out of " << in << " : " << out << '\n';
    }
