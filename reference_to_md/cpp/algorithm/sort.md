...
1,2) \\(\scriptsize O(N \cdot \log(N))\\)O(N·log(N)) comparisons using operator<(until C++20)[std::less](../utility/functional/less.html){}(since C++20).
...
[Introsort](https://en.wikipedia.org/wiki/Introsort "enwiki:Introsort") can handle all cases with \\(\scriptsize O(N \cdot \log(N))\\)O(N·log(N)) comparisons (without incurring additional overhead in the average case), and thus is usually used for implementing `sort()`. 
...
Run this code
    ...
     
        auto print = [&s]([std::string_view](../string/basic_string_view.html) const rem)
        {
            for (auto a : s)
    ...
     
        std::sort(s.begin(), s.end(), [std::greater](../utility/functional/greater.html)<int>());
