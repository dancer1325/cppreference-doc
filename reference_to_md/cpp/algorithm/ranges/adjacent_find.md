...
std::[projected](http://en.cppreference.com/w/cpp/ranges-placeholder/iterator/projected)<I, Proj>> Pred = [ranges::equal_to](http://en.cppreference.com/w/cpp/ranges-functional-placeholder/equal_to) >  
constexpr I  
adjacent_find( I first, S last, Pred pred = {}, Proj proj = {} ); |  (1)  |  (since C++20)  
...
std::[projected](http://en.cppreference.com/w/cpp/ranges-placeholder/iterator/projected)<[ranges::iterator_t](../../ranges/iterator_t.html)<R>, Proj>> Pred = [ranges::equal_to](http://en.cppreference.com/w/cpp/ranges-functional-placeholder/equal_to) >  
constexpr [ranges::borrowed_iterator_t](../../ranges/borrowed_iterator_t.html)<R>  
adjacent_find( R&& r, Pred pred = {}, Proj proj = {} ); |  (2)  |  (since C++20)  
| |   
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/ranges/adjacent_find&action=edit&section=4 "Edit section: Possible implementation")] Possible implementation
    ...
    struct adjacent_find_fn
    {
        ...
        constexpr I operator()(I first, S last, Pred pred = {}, Proj proj = {}) const
        {
            if (first == last)
                return first;
            auto next = [ranges::next](../../iterator/ranges/next.html)(first);
            for (; next != last; ++next, ++first)
                if ([std::invoke](../../utility/functional/invoke.html)(pred, [std::invoke](../../utility/functional/invoke.html)(proj, *first), [std::invoke](../../utility/functional/invoke.html)(proj, *next)))
                    return first;
            return next;
        }
     
...
Run this code
    ...
    {
        return std::[ranges::cend](../../ranges/cend.html)(r) != std::ranges::adjacent_find(r,
    ...
     
        if (auto it = ranges::adjacent_find(v.begin(), v.end()); it == v.end())
            [std::cout](../../io/cout.html) << "No matching adjacent elements\n";
        else
            [std::cout](../../io/cout.html) << "The first adjacent pair of equal elements is at ["
                      << [ranges::distance](../../iterator/ranges/distance.html)(v.begin(), it) << "] == " << *it << '\n';
     
        if (auto it = ranges::adjacent_find(v, [ranges::greater](http://en.cppreference.com/w/cpp/ranges-functional-placeholder/greater)()); it == v.end())
            [std::cout](../../io/cout.html) << "The entire vector is sorted in ascending order\n";
        else
            [std::cout](../../io/cout.html) << "The last element in the non-decreasing subsequence is at ["
                      << [ranges::distance](../../iterator/ranges/distance.html)(v.begin(), it) << "] == " << *it << '\n';
    }
