...
void stable_sort( RandomIt first, RandomIt last ); |  (1) | (constexpr since C++26)  
template< class ExecutionPolicy, class RandomIt >  
void stable_sort( ExecutionPolicy&& policy,  
RandomIt first, RandomIt last ); |  (2)  |  (since C++17)  
template< class RandomIt, class Compare >  
void stable_sort( RandomIt first, RandomIt last, Compare comp ); |  (3) | (constexpr since C++26)  
...
## Contents
  ...
  * [2 Complexity](stable_sort.html#Complexity)
  * [3 Exceptions](stable_sort.html#Exceptions)
  * [4 Possible implementation](stable_sort.html#Possible_implementation)
  * [5 Notes](stable_sort.html#Notes)
...
1,2) \\(\scriptsize O(N \cdot \log(N))\\)O(N·log(N)) comparisons using operator<(until C++20)[std::less](../utility/functional/less.html){}(since C++20) if enough extra memory is available, otherwise \\(\scriptsize O(N \cdot \log^{2}(N))\\)O(N·log2  
(N)) comparisons.
...
Run this code
    ...
    {
        int age;
        [std::string](../string/basic_string.html) name; // Does not participate in comparisons
    };
     
    ...
    {
        auto v = [std::array](../container/array.html){3, 1, 4, 1, 5, 9};
        std::stable_sort(v.begin(), v.end());
        return v;
    }
    static_assert(std::[ranges::is_sorted](ranges/is_sorted.html)(get_sorted()));
    #endif
     
    int main()
    {
        [std::vector](../container/vector.html)<Employee> v{{108, "Zaphod"}, {32, "Arthur"}, {108, "Ford"}};
     
        std::stable_sort(v.begin(), v.end());
     
        for (const Employee& e : v)
            [std::cout](../io/cout.html) << e.age << ", " << e.name << '\n';
    }
