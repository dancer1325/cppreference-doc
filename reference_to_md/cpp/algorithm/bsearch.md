...
---|---|---  
void* bsearch( const void* key, const void* ptr, [std::size_t](../types/size_t.html) count,  
...
## Contents
  ...
  * [5 See also](bsearch.html#See_also)
...
Run this code
    ...
    int main()
    {
        [std::array](../container/array.html) arr{1, 2, 3, 4, 5, 6, 7, 8};
     
        for (const int key : {4, 8, 9})
        {
            const int* p = static_cast<int*>(
                std::bsearch(&key,
                    ...
                    compare<int>));
     
            [std::cout](../io/cout.html) << "value " << key;
            if (p)
                [std::cout](../io/cout.html) << " found at position " << (p - arr.data()) << '\n';
            else
