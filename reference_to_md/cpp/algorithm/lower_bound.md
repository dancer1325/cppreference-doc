...
1) The order is determined by operator<:  Returns the first iterator iter in `[`first`, `last`)` where bool(*iter < value) is false, or last if no such iter exists. If the elements elem of `[`first`, `last`)` are not [partitioned](../algorithm.html#Requirements "cpp/algorithm") with respect to the expression bool(elem < value), the behavior is undefined.  | (until C++20)  
---|---  
Equivalent to std::lower_bound(first, last, value, [std::less](../utility/functional/less.html){}).  | (since C++20)  
...
If the elements elem of `[`first`, `last`)` are not [partitioned](../algorithm.html#Requirements "cpp/algorithm") with respect to the expression bool(comp(elem, value)), the behavior is undefined.
...
## Contents
  ...
  * [4 Possible implementation](lower_bound.html#Possible_implementation)
  * [5 Notes](lower_bound.html#Notes)
  * [6 Example](lower_bound.html#Example)
  * [7 Defect reports](lower_bound.html#Defect_reports)
  * [8 See also](lower_bound.html#See_also)
...
[lower_bound (1)](lower_bound.html#Version_1)  
---  
    ...
    {
        return std::lower_bound(first, last, value, [std::less](../utility/functional/less.html){});
    }  
  
[lower_bound (2)](lower_bound.html#Version_2)  
...
Unlike [std::binary_search](binary_search.html "cpp/algorithm/binary search"), `std::lower_bound` does not require operator< or comp to be asymmetric (i.e., a < b and b < a always have different results). In fact, it does not even require value < *iter or comp(value, *iter) to be well-formed for any iterator iter in `[`first`, `last`)`. 
...
Run this code
    ...
     
            ...
            lower != data.end()
                ? [std::cout](../io/cout.html) << *lower << " at index " << [std::distance](../iterator/distance.html)(data.begin(), lower)
    ...
     
        ...
        #endif
        [assert](../error/assert.html)((*it == CD{2, 2}));
    }
...
[LWG 384](https://cplusplus.github.io/LWG/issue384) | C++98  | at most \\(\scriptsize \log(N)+1\\)log(N)+1 comparisons were allowed  | corrected to \\(\scriptsize \log_{2}(N)+O(1)\\)log2(N)+1  
