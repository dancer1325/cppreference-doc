...
1) The equivalence is checked using operator<:  If !bool(*iter < value) && !bool(value < *iter) is true for some iterator iter in `[`first`, `last`)`, returns true. Otherwise returns false. If any of the following conditions is satisfied, the behavior is undefined: 
...
| (until C++20)  
---|---  
Equivalent to std::binary_search(first, last, value, [std::less](../utility/functional/less.html){}).  | (since C++20)  
...
## Contents
  ...
  * [7 Defect reports](binary_search.html#Defect_reports)
  * [8 See also](binary_search.html#See_also)
...
`std::binary_search` only checks whether an equivalent element exists. To obtain an iterator to that element (if exists), [std::lower_bound](lower_bound.html "cpp/algorithm/lower bound") should be used instead. 
...
[binary_search (1)](binary_search.html#Version_1)  
---  
    ...
    {
        return std::binary_search(first, last, value, [std::less](../utility/functional/less.html){});
    }  
  
[binary_search (2)](binary_search.html#Version_2)  
    ...
    {
        first = [std::lower_bound](lower_bound.html)(first, last, value, comp);
...
Run this code
    ...
     
        ...
        auto cmpz = [](CD x, CD y){ return abs(x) < abs(y); };
            ...
            [assert](../error/assert.html)(std::binary_search(nums.cbegin(), nums.cend(), {4, 2}, cmpz));
        #else
            [assert](../error/assert.html)(std::binary_search(nums.cbegin(), nums.cend(), CD{4, 2}, cmpz));
        #endif
    }
