...
ForwardIt remove_if( ForwardIt first, ForwardIt last, UnaryPred p ); |  (3) | (constexpr since C++20)  
...
If the [value type](../iterator.html#Types_and_writability "cpp/iterator") of `ForwardIt` is not [CopyAssignable](../named_req/CopyAssignable.html "cpp/named req/CopyAssignable"), the behavior is undefined.  | (until C++11)  
---|---  
If the type of *first is not [MoveAssignable](../named_req/MoveAssignable.html "cpp/named req/MoveAssignable"), the behavior is undefined.  | (since C++11)  
  
## Contents

  * [1 Explanation](remove.html#Explanation)
  * [2 Parameters](remove.html#Parameters)
  * [3 Return value](remove.html#Return_value)
  * [4 Complexity](remove.html#Complexity)
  * [5 Exceptions](remove.html#Exceptions)
  * [6 Possible implementation](remove.html#Possible_implementation)
  * [7 Notes](remove.html#Notes)
  * [8 Example](remove.html#Example)
  * [9 Defect reports](remove.html#Defect_reports)
  * [10 See also](remove.html#See_also)
...
Removing is done by shifting the elements in the range in such a way that the elements that are not to be removed appear in the beginning of the range. 
  ...
  * Shifting is done by [copy assignment](../language/as_operator.html "cpp/language/copy assignment")(until C++11)[move assignment](../language/move_operator.html "cpp/language/move assignment")(since C++11). 
  ...
  * All iterators in `[`result`, `last`)` are still [dereferenceable](../iterator.html#Dereferenceability_and_validity "cpp/iterator"). 
...
---  
    ...
    {
        first = [std::find](find.html)(first, last, value);
        if (first != last)
...
[remove_if (3)](remove.html#Version_3)  
      
    
    template<class ForwardIt, class UnaryPred>
    ForwardIt remove_if(ForwardIt first, ForwardIt last, UnaryPred p)
    {
        first = [std::find_if](find.html)(first, last, p);
        if (first != last)
            for (ForwardIt i = first; ++i != last;)
                if (!p(*i))
                    *first++ = std::move(*i);
...
The same effect can also be achieved by the following non-member functions: 
  ...
  * [std::erase](../container/vector/erase2.html "cpp/container/vector/erase2"), which has [overloads](../container.html#Non-member_function_table "cpp/container") for all standard sequence containers. 
  * [std::erase_if](../container/vector/erase2.html "cpp/container/vector/erase2"), which has [overloads](../container.html#Non-member_function_table "cpp/container") for all standard containers. 
...
These algorithms cannot be used with associative containers such as [std::set](../container/set.html "cpp/container/set") and [std::map](../container/map.html "cpp/container/map") because their iterator types do not dereference to [MoveAssignable](../named_req/MoveAssignable.html "cpp/named req/MoveAssignable") types (the keys in these containers are not modifiable). 

The standard library also defines an overload of [`std::remove`](../io/c/remove.html "cpp/io/c/remove") in [`<cstdio>`](../header/cstdio.html "cpp/header/cstdio"), which takes a const char* and is used to delete files. 
...
Run this code
    ...
    int main()
    {
        [std::string](../string/basic_string.html) str1{"Quick  Red  Dog"};
        [std::cout](../io/cout.html) << "1) " << [std::quoted](../io/manip/quoted.html)(str1) << '\n';
        const auto noSpaceEnd = std::remove(str1.begin(), str1.end(), ' ');
        [std::cout](../io/cout.html) << "2) " << [std::quoted](../io/manip/quoted.html)(str1) << '\n';
     
        // The spaces are removed from the string only logically.
        // Note, we use view, the original string is still not shrunk:
        [std::cout](../io/cout.html) << "3) " << [std::quoted](../io/manip/quoted.html)([std::string_view](../string/basic_string_view.html)(str1.begin(), noSpaceEnd))
                  << ", size: " << str1.size() << '\n';
     
        str1.erase(noSpaceEnd, str1.end());
        // The spaces are removed from the string physically.
        [std::cout](../io/cout.html) << "4) " << [std::quoted](../io/manip/quoted.html)(str1) << ", size: " << str1.size() << '\n';
     
        [std::string](../string/basic_string.html) str2 = "Jumped\n Over\tA\vLazy \t  Fox\r\n";
        str2.erase(std::remove_if(str2.begin(), 
                                  str2.end(),
                                  [](unsigned char x) { return [std::isspace](../string/byte/isspace.html)(x); }),
                   str2.end());
        [std::cout](../io/cout.html) << "5) " << [std::quoted](../io/manip/quoted.html)(str2) << '\n';
     
        ...
        #else
            nums.erase(std::remove(nums.begin(), nums.end(), [std::complex](../numeric/complex.html)<double>{1, 3}),
                       nums.end());
        #endif
