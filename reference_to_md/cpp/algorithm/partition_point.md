...
ForwardIt partition_point( ForwardIt first, ForwardIt last, UnaryPred p ); |  |  (since C++11)   
...
If the elements elem of `[`first`, `last`)` are not [partitioned](../algorithm.html#Requirements "cpp/algorithm") with respect to the expression bool(p(elem)), the behavior is undefined. 
...
## Contents
  ...
  * [5 Possible implementation](partition_point.html#Possible_implementation)
  * [6 Example](partition_point.html#Example)
  * [7 See also](partition_point.html#See_also)
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/algorithm/partition_point&action=edit&section=5 "Edit section: Possible implementation")] Possible implementation
    ...
    {
        for (auto length = [std::distance](../iterator/distance.html)(first, last); 0 < length; )
        {
            auto half = length / 2;
            auto middle = [std::next](../iterator/next.html)(first, half);
            if (p(*middle))
            {
                first = [std::next](../iterator/next.html)(middle);
                length -= (half + 1);
            }
            else
                length = half;
...
Run this code
    ...
     
        ...
        const auto i = [std::distance](../iterator/distance.html)(v.cbegin(), pp);
