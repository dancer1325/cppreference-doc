...
const T& old_value, const T& new_value ); |  (1) | (constexpr since C++20)  
...
UnaryPred p, const T& new_value ); |  | (constexpr since C++20)   
(until C++26)  
...
( InputIt first, InputIt last, OutputIt d_first,  
UnaryPred p, const T& new_value ); |  |  (since C++26)  
| (4) |   
...
( ExecutionPolicy&& policy,  
ForwardIt1 first, ForwardIt1 last, ForwardIt2 d_first,  
UnaryPred p, const T& new_value ); |  |  (since C++17)   
(until C++26)  
template< class ExecutionPolicy, class ForwardIt1, class ForwardIt2,  
class UnaryPred, class T = typename [std::iterator_traits](../iterator/iterator_traits.html)  
...
If any of the results of the expressions *first and new_value is not [writable](../iterator.html#Types_and_writability "cpp/iterator") to d_first, the program is ill-formed. 
...
## Contents
  ...
  * [4 Exceptions](replace_copy.html#Exceptions)
  * [5 Possible implementation](replace_copy.html#Possible_implementation)
  * [6 Notes](replace_copy.html#Notes)
  * [7 Example](replace_copy.html#Example)
  * [8 Defect reports](replace_copy.html#Defect_reports)
  * [9 See also](replace_copy.html#See_also)
...
[replace_copy_if (3)](replace_copy.html#Version_3)  
...
Run this code
    ...
    int main()
    {
        [std::vector](../container/vector.html)<short> src{3, 1, 4, 1, 5, 9, 2, 6, 5};
        println(src);
        [std::vector](../container/vector.html)<int> dst(src.size());
        std::replace_copy_if(src.cbegin(), src.cend(),
                             dst.begin(),
                             [](short n){ return n > 5; }, 0);
        println(dst);
     
        [std::vector](../container/vector.html)<[std::complex](../numeric/complex.html)<double>> src2{{1, 3}, {2, 4}, {3, 5}},
                                          dst2(src2.size());
        println(src2);
...
---|---|---|---  
[LWG 283](https://cplusplus.github.io/LWG/issue283) | C++98  | `T` was required to be [CopyAssignable](../named_req/CopyAssignable.html "cpp/named req/CopyAssignable") (and [EqualityComparable](../named_req/EqualityComparable.html "cpp/named req/EqualityComparable") for  
`replace_copy`), but the value type of `InputIt` is not always `T` | removed the requirement   
[LWG 337](https://cplusplus.github.io/LWG/issue337) | C++98  | `replace_copy_if` only required `InputIt` to  
meet the requirements of [LegacyIterator](../named_req/Iterator.html "cpp/named req/Iterator")[[1]](replace_copy.html#cite_note-1) | corrected to  
[LegacyInputIterator](../named_req/InputIterator.html "cpp/named req/InputIterator")  
  
  1. [↑](replace_copy.html#cite_ref-1) The actual defect in the C++ standard is that the template parameter `InputIterator` was misspecified as `Iterator`. This affects the type requirements because the C++ standard states that for the function templates in the algorithms library, the template type parameters whose name ends with `Iterator` imply the type requirements of the corresponding iterator categories.
