...
template< class InputIt, class UnaryPred >  
InputIt find_if_not( InputIt first, InputIt last, UnaryPred q ); |  (5) | (since C++11)   
(constexpr since C++20)  
template< class ExecutionPolicy, class ForwardIt, class UnaryPred >  
ForwardIt find_if_not( ExecutionPolicy&& policy,  
ForwardIt first, ForwardIt last, UnaryPred q ); |  (6)  |  (since C++17)  
| |   
...
## Contents
  ...
  * [4 Exceptions](find.html#Exceptions)
  * [5 Possible implementation](find.html#Possible_implementation)
  * [6 Notes](find.html#Notes)
  * [7 Example](find.html#Example)
  * [8 Defect reports](find.html#Defect_reports)
  * [9 See also](find.html#See_also)
...
If C++11 is not available, an equivalent to `std::find_if_not` is to use `std::find_if` with the negated predicate. 
    ...
    {
        return std::find_if(first, last, [std::not1](../utility/functional/not1.html)(q));
    }  
    ...
     
        for (const int needle : {3, 5})
            if (std::find(haystack.begin(), haystack.end(), needle) == haystack.end())
                [std::cout](../io/cout.html) << "haystack does not contain " << needle << '\n';
            else
                [std::cout](../io/cout.html) << "haystack contains " << needle << '\n';
    }
     
    void example_predicate()
    {
        for (const auto& haystack : {[std::array](../container/array.html){3, 1, 4}, {1, 3, 5}})
        {
            const auto it = std::find_if(haystack.begin(), haystack.end(), is_even);
            if (it != haystack.end())
                [std::cout](../io/cout.html) << "haystack contains an even number " << *it << '\n';
            else
                [std::cout](../io/cout.html) << "haystack does not contain even numbers\n";
        }
    }
     
    void example_list_init()
    {
        [std::vector](../container/vector.html)<[std::complex](../numeric/complex.html)<double>> haystack{{4.0, 2.0}};
    #ifdef __cpp_lib_algorithm_default_value_type
        // T gets deduced making list-initialization possible
        const auto it = std::find(haystack.begin(), haystack.end(), {4.0, 2.0});
    #else
        const auto it = std::find(haystack.begin(), haystack.end(), [std::complex](../numeric/complex.html){4.0, 2.0});
    #endif
        [assert](../error/assert.html)(it == haystack.begin());  
    }
     
    int main()
    {
