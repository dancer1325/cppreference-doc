...
void partial_sort( RandomIt first, RandomIt middle, RandomIt last ); |  (1) | (constexpr since C++20)  
...
## Contents
  ...
  * [2 Complexity](partial_sort.html#Complexity)
  * [3 Exceptions](partial_sort.html#Exceptions)
  * [4 Possible implementation](partial_sort.html#Possible_implementation)
  * [5 Notes](partial_sort.html#Notes)
    * [5.1 Algorithm](partial_sort.html#Algorithm)
    * [5.2 Intended use](partial_sort.html#Intended_use)
  * [6 Example](partial_sort.html#Example)
  * [7 Defect reports](partial_sort.html#Defect_reports)
  * [8 See also](partial_sort.html#See_also)
...
1,2) Approximately \\(\scriptsize N \cdot \log(M)\\)N·log(M) comparisons using operator<(until C++20)[std::less](../utility/functional/less.html){}(since C++20).
...
[partial_sort (1)](partial_sort.html#Version_1)  
---  
    ...
    void partial_sort(RandomIt first, RandomIt middle, RandomIt last)
    {
        typedef typename [std::iterator_traits](../iterator/iterator_traits.html)<RandomIt>::value_type VT;
        std::partial_sort(first, middle, last, [std::less](../utility/functional/less.html)<VT>());
    }  
  
[partial_sort (3)](partial_sort.html#Version_3)  
    ...
    namespace impl
    {
        ...
        void sift_down(RandomIt first, RandomIt last, const Compare& comp)
        {
            ...
            const auto length = static_cast<[std::size_t](../types/size_t.html)>(last - first);
            [std::size_t](../types/size_t.html) current = 0;
            [std::size_t](../types/size_t.html) next = 2;
            while (next < length)
            {
                ...
                if (!comp(*(first + current), *(first + next)))
                    return;
                [std::iter_swap](iter_swap.html)(first + current, first + next);
                current = next;
                next = 2 * current + 2;
            }
            --next;
    ...
     
        ...
        {
            [std::make_heap](make_heap.html)(first, middle, comp);
            for (auto i = middle; i != last; ++i)
            {
                if (comp(*i, *first))
                {
                    [std::iter_swap](iter_swap.html)(first, i);
...
To select elements, a heap is used (see [heap](https://en.wikipedia.org/wiki/Heap_\(data_structure\)#Applications "enwiki:Heap \(data structure\)")). For example, for operator< as comparison function, _max-heap_ is used to select middle − first smallest elements. 
...
Run this code
    ...
    void print(const auto& s, int middle)
    {
        ...
        {
            for (auto i = s.size() + middle; --i; [std::cout](../io/cout.html) << "  ")
            {}
     
            for ([std::cout](../io/cout.html) << '^'; middle++ < 0; [std::cout](../io/cout.html) << "--")
            {}
        }
        [std::cout](../io/cout.html) << '\n';
    };
    ...
    int main()
    {
        [std::array](../container/array.html)<int, 10> s{5, 7, 4, 2, 8, 6, 1, 9, 0, 3};
        print(s, 0);
        std::partial_sort(s.begin(), s.begin() + 3, s.end());
        print(s, 3);
        std::partial_sort(s.rbegin(), s.rbegin() + 4, s.rend());
        print(s, -4);
        std::partial_sort(s.rbegin(), s.rbegin() + 5, s.rend(), [std::greater](../utility/functional/greater.html){});
        print(s, -5);
    }
...
---|---|---|---  
[P0896R4](https://wg21.link/P0896R4) | C++98  | `[`first`, `middle`)` and `[`middle`, `last`)`  
