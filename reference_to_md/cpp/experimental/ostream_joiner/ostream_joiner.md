...
1) Constructs the iterator with the private ostream_type* member initialized with [std::addressof](../../memory/addressof.html)(stream), the private delimiter member initialized with delimiter, and the private "first element" flag set to true.

2) Constructs the iterator with the private ostream_type* member initialized with [std::addressof](../../memory/addressof.html)(stream), the private delimiter member initialized with `std::move(delimiter)`, and the private "first element" flag set to true.
