...
template< class U >  
/* see below */ constexpr propagate_const( propagate_const<U>&& pu ); |  (3)  |  (library fundamentals TS v2)  
template< class U >  
/* see below */ constexpr propagate_const( U&& u ); |  (4)  |  (library fundamentals TS v2)  
...
This constructor does not participate in overload resolution unless [std::is_constructible](../../types/is_constructible.html)<T, U>::value is true, and is `explicit` if and only if [std::is_convertible](../../types/is_convertible.html)<U, T>::value is false.

4) Initializes this->t_ as if by direct-non-list-initialization with the expression [std::forward](../../utility/forward.html)<U>(u).  
This constructor does not participate in overload resolution unless [std::is_constructible](../../types/is_constructible.html)<T, U>::value is true and [std::decay_t](../../types/decay.html)<U> is not a specialization of `propagate_const`. This constructor is `explicit` if and only if [std::is_convertible](../../types/is_convertible.html)<U, T>::value is false.

5) Copy constructor is explicitly deleted. `propagate_const` is non-copyable.

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/experimental/propagate_const/propagate_const&action=edit&section=1 "Edit section: Parameters")] Parameters
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/experimental/propagate_const/propagate_const&action=edit&section=2 "Edit section: Notes")] Notes
