...
Deallocates the storage pointed to by p, which must have been allocated from a [`memory_resource`](../memory_resource.html "cpp/experimental/memory resource") `x` that compares equal to *resource() using x.allocate(n * sizeof(T), alignof(T)). 
