...
space_info space( const path& p, error_code& ec ) noexcept; |  |  (filesystem TS)  
| |   
...
Populates and returns an object of type [`space_info`](space_info.html "cpp/experimental/fs/space info"), set from the members of the POSIX `struct statvfs` as follows: 
