...
All emit() calls transferring characters to the same wrapped stream buffer object appear to execute in a total order, where each emit() call [synchronizes-with](../../atomic/memory_order.html "cpp/atomic/memory order") subsequent emit() calls in that total order, even if these calls are made through difference instances of std::basic_syncbuf/std::basic_osyncstream. In practice, this means that emit() takes a lock uniquely associated with the wrapped stream object: for example, it could be held in a static hash map where the address of the wrapped stream is used as the key. 

If a call had been made to [`sync`](sync.html "cpp/io/basic syncbuf/sync") since the last call to emit(), then also flushes the wrapped stream by calling [`pubsync()`](../basic_streambuf/pubsync.html "cpp/io/basic streambuf/pubsync") on it. 

## Contents

  * [1 Parameters](emit.html#Parameters)
  * [2 Return value](emit.html#Return_value)
  * [3 Notes](emit.html#Notes)
  * [4 Example](emit.html#Example)
  * [5 See also](emit.html#See_also)
...
true if all of the following is true: 
  ...
  * the call to [`pubsync()`](../basic_streambuf/pubsync.html "cpp/io/basic streambuf/pubsync"), if requested, also completed successfully. 
