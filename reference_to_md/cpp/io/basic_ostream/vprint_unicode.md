...
---|---|---  
void vprint_unicode( [std::ostream](../basic_ostream.html)& os,  
[std::string_view](../../string/basic_string_view.html) fmt, [std::format_args](../../utility/format/basic_format_args.html) args ); |  |  (since C++23)  
| |   
  
Format args according to the format string fmt, and writes the result to the output stream os. Behaves as [FormattedOutputFunction](../../named_req/FormattedOutputFunction.html "cpp/named req/FormattedOutputFunction") of os, except that some details of error reporting [differ](vprint_unicode.html#Exceptions). 

Performs the following operations in order: 

  1. First, the function constructs and checks the [sentry](sentry.html "cpp/io/basic ostream/sentry") object. 
  2. Initializes an automatic variable as if by [std::string](../../string/basic_string.html) out = [std::vformat](../../utility/format/vformat.html)(os.getloc(), fmt, args);. 
  3. Writes out to os: 
  ...
  * If os refers to a terminal that is only capable of displaying Unicode via a [native Unicode API](../vprint_unicode.html#Notes "cpp/io/vprint unicode"), flushes os and writes out to the terminal using the native Unicode API. 
  * Otherwise, inserts the character sequence `[`out.begin()`, `out.end()`)` into os. 



If writing to the terminal or inserting into os fails, calls os.setstate([std::ios_base::badbit](../ios_base/iostate.html)). 

After writing characters to os, establishes an [observable checkpoint](../../language/as_if.html "cpp/language/as if").  | (since C++26)  
---|---  
  
If out contains invalid Unicode [code units](https://en.wikipedia.org/wiki/Character_encoding#Terminology "enwiki:Character encoding") when the native Unicode API is used, the behavior is undefined. 
