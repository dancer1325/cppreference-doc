...
[std::basic_filebuf](../basic_filebuf.html)& operator=( [std::basic_filebuf](../basic_filebuf.html)&& rhs ); |  (1)  |  (since C++11)  
---|---|---  
[std::basic_filebuf](../basic_filebuf.html)& operator=( const [std::basic_filebuf](../basic_filebuf.html)& rhs ) = delete; |  (2)  |   
| |   
  
Assigns another `basic_filebuf` object. 

1) First calls [close()](close.html "cpp/io/basic filebuf/close") to close the associated file, then moves the contents of rhs into *this: the put and get buffers, the associated file, the locale, the openmode, the is_open flag, and any other state. After the move, rhs is not associated with a file and rhs.is_open() == false.

2) The copy assignment operator is deleted; `basic_filebuf` is not [CopyAssignable](../../named_req/CopyAssignable.html "cpp/named req/CopyAssignable").
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/io/basic_filebuf/operator%3D&action=edit&section=1 "Edit section: Parameters")] Parameters

rhs  |  \-  |  another `basic_filebuf` that will be moved from   
---|---|---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/io/basic_filebuf/operator%3D&action=edit&section=2 "Edit section: Return value")] Return value

*this

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/io/basic_filebuf/operator%3D&action=edit&section=3 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        [std::ofstream](../basic_ofstream.html){"test.in"} << "test\n"; // writes via a temporary object
        [std::ifstream](../basic_ifstream.html) fin("test.in"); // read-only stream
        [std::ofstream](../basic_ofstream.html) fout("test.out"); // write-only stream
     
        [std::string](../../string/basic_string.html) s;
        [std::getline](../../string/basic_string/getline.html)(fin, s);
        [std::cout](../cout.html) << "s = [" << s << "]\n"; // s contains "test"
     
        [assert](../../error/assert.html)(fout.is_open());
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/io/basic_filebuf/operator%3D&action=edit&section=4 "Edit section: See also")] See also
