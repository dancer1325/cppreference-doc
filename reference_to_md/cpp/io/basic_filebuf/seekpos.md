...
1) If the file is open for writing, writes the put area and any unshift sequences required by the currently imbued locale, using [overflow()](../basic_streambuf/overflow.html "cpp/io/basic streambuf/overflow").

2) Repositions the file pointer, as if by calling [std::fsetpos()](../c/fsetpos.html "cpp/io/c/fsetpos").

3) If the file is open for reading, updates the get area if necessary.

If sp was not obtained by calling [seekoff()](../basic_streambuf/pubseekoff.html "cpp/io/basic streambuf/pubseekoff") or `seekpos()` on the same file, the behavior is undefined. 
