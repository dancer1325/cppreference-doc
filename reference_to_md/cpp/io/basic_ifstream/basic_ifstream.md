...
= [std::ios_base::in](../ios_base/openmode.html) ); |  (2)  |   
explicit basic_ifstream( const std::filesystem::path::value_type* filename,  
[std::ios_base::openmode](../ios_base/openmode.html) mode  
= [std::ios_base::in](../ios_base/openmode.html) ); |  (3)  |  (since C++17)  
explicit basic_ifstream( const [std::string](../../string/basic_string.html)& filename,  
[std::ios_base::openmode](../ios_base/openmode.html) mode  
= [std::ios_base::in](../ios_base/openmode.html) ); |  (4)  |  (since C++11)  
template< class FsPath >  
explicit basic_ifstream( const FsPath& filename,  
[std::ios_base::openmode](../ios_base/openmode.html) mode  
= [std::ios_base::in](../ios_base/openmode.html) ); |  (5)  |  (since C++17)  
...
2,3) First, performs the same steps as the default constructor, then associates the stream with a file by calling rdbuf()->open(filename, mode | [std::ios_base::in](../ios_base/openmode.html)) (see [std::basic_filebuf::open](../basic_filebuf/open.html "cpp/io/basic filebuf/open") for the details on the effects of that call). If the [`open()`](open.html "cpp/io/basic ifstream/open") call returns a null pointer, sets setstate(failbit). Overload (3) is only provided if std::filesystem::path::value_type is not char.(since C++17)

4,5) Same as basic_ifstream(filename.c_str(), mode). (5) participates in overload resolution only if `FsPath` is [std::filesystem::path](../../filesystem/path.html "cpp/filesystem/path").(since C++17)

6) Move constructor. First, move-constructs the base class from other (which does not affect the [`rdbuf()`](rdbuf.html "cpp/io/basic ifstream/rdbuf") pointer), then move-constructs the [std::basic_filebuf](../basic_filebuf.html "cpp/io/basic filebuf") member, then calls this->set_rdbuf() to install the new `basic_filebuf` as the [`rdbuf()`](rdbuf.html "cpp/io/basic ifstream/rdbuf") pointer in the base class.

7) The copy-constructor is deleted: this class is not copyable.

## Contents

  * [1 Parameters](basic_ifstream.html#Parameters)
  * [2 Example](basic_ifstream.html#Example)
  * [3 Defect reports](basic_ifstream.html#Defect_reports)
  * [4 See also](basic_ifstream.html#See_also)
...
Run this code
    ...
    int main()
    {
        [std::ifstream](../basic_ifstream.html) f0;
        [std::ifstream](../basic_ifstream.html) f1("test.bin", std::ios::binary);
        [std::string](../../string/basic_string.html) name = "example.txt";
        [std::ifstream](../basic_ifstream.html) f2(name);
        [std::ifstream](../basic_ifstream.html) f3(std::move(f1));
    }
...
(public member function of `std::basic_istream<CharT,Traits>`) [[edit]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/io/basic_istream/dsc_constructor&action=edit)
