...
---|---|---  
int ungetc( int ch, [std::FILE](FILE.html) *stream ); |  |   
| |   
  
If `ch` does not equal [EOF](../c.html "cpp/io/c"), pushes the character `ch` (reinterpreted as unsigned char) into the input buffer associated with the stream `stream` in such a manner than subsequent read operation from `stream` will retrieve that character. The external device associated with the stream is not modified. 

Stream repositioning operations [std::fseek](fseek.html "cpp/io/c/fseek"), [std::fsetpos](fsetpos.html "cpp/io/c/fsetpos"), and [std::rewind](rewind.html "cpp/io/c/rewind") discard the effects of `ungetc`. 
...
If `ch` equals [EOF](../c.html "cpp/io/c"), the operation fails and the stream is not affected. 

A successful call to `ungetc` clears the end of file status flag [std::feof](feof.html "cpp/io/c/feof"). 
...
## Contents
  ...
  * [5 See also](ungetc.html#See_also)
...
Run this code
    ...
    #include <cstdio>
     
    void demo_scanf(const char* fmt, [std::FILE](FILE.html)* s)
    {
        while (*fmt != '\0') {
            if (*fmt == '%') {
                switch (*++fmt) {
                    case 'u': {
                        int c{};
                        while ([std::isspace](../../string/byte/isspace.html)(c=[std::getc](fgetc.html)(s))) {}
                        unsigned int num{};
                        while ([std::isdigit](../../string/byte/isdigit.html)(c)) {
                            num = num*10 + c-'0';
                            c = [std::getc](fgetc.html)(s);
                        }
                        [std::printf](fprintf.html)("%%u scanned %u\n", num);
                        std::ungetc(c, s);
                        break;
                    }
                    case 'c': {
                        int c = [std::getc](fgetc.html)(s);
                        [std::printf](fprintf.html)("%%c scanned '%c'\n", c);
                        break;
                    }
                }
            } else {
    ...
    int main()
    {
        if ([std::FILE](FILE.html)* f = [std::fopen](fopen.html)("input.txt", "w+")) {
            [std::fputs](fputs.html)("123x", f);
            [std::rewind](rewind.html)(f);
            demo_scanf("%u%c", f);
