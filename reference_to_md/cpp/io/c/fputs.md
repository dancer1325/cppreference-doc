...
---|---|---  
int fputs( const char* str, [std::FILE](FILE.html)* stream ); |  |   
| |   
  
Writes every character from the null-terminated string `str` to the output stream `stream`, as if by repeatedly executing [std::fputc](fputc.html "cpp/io/c/fputc"). 
...
## Contents
  ...
  * [5 See also](fputs.html#See_also)
...
Run this code
    ...
     
        if (rc == [EOF](../c.html))
            [std::perror](perror.html)("fputs()"); // POSIX requires that errno is set
    }
