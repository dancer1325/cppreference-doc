...
---|---|---  
void setbuf( [std::FILE](FILE.html)* stream, char* buffer ); |  |   
| |   
  
Sets the internal buffer to use for I/O operations performed on the C stream stream. 

If buffer is not null, equivalent to [std::setvbuf](setvbuf.html)(stream, buffer, [_IOFBF](../c.html), [BUFSIZ](../c.html)). 

If buffer is null, equivalent to [std::setvbuf](setvbuf.html)(stream, nullptr, [_IONBF](../c.html), 0), which turns off buffering. 
...
[std::setvbuf](setvbuf.html "cpp/io/c/setvbuf") should also be used to detect errors, since `std::setbuf` does not indicate success or failure. 

This function may only be used after stream has been associated with an open file, but before any other operation (other than a failed call to `std::setbuf`/[std::setvbuf](setvbuf.html "cpp/io/c/setvbuf")). 

A common error is setting the buffer of [stdin](std_streams.html "cpp/io/c/std streams") or [stdout](std_streams.html "cpp/io/c/std streams") to an array whose lifetime ends before the program terminates: 
...
Run this code
    ...
     
        ...
        [std::putchar](putchar.html)('a'); // appears immediately on unbuffered stream
        [std::this_thread::sleep_for](../../thread/sleep_for.html)(1s);
        [std::putchar](putchar.html)('b');
    }
