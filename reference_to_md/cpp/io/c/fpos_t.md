...
`std::fpos_t` is a non-array complete object type, can be used to store (by [std::fgetpos](fgetpos.html "cpp/io/c/fgetpos")) and restore (by [std::fsetpos](fsetpos.html "cpp/io/c/fsetpos")) the position and multibyte parser state (if any) for a C stream. 
