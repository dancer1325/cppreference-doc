...
---|---|---  
[std::FILE](FILE.html)* freopen( const char* filename, const char* mode, [std::FILE](FILE.html)* stream ); |  |   
| |   
  
First, attempts to close the file associated with stream, ignoring any errors. Then, if filename is not null, attempts to open the file specified by filename using mode as if by [std::fopen](fopen.html "cpp/io/c/fopen"), and associates that file with the file stream pointed to by stream. If filename is a null pointer, then the function attempts to reopen the file that is already associated with stream (it is implementation defined which mode changes are allowed in this case). 
...
## Contents
  ...
  * [6 See also](freopen.html#See_also)
...
Run this code
    ...
    int main()
    {
        [std::printf](fprintf.html)("stdout is printed to console\n");
        if (std::freopen("redir.txt", "w", stdout))
        {
            [std::printf](fprintf.html)("stdout is redirected to a file\n"); // this is written to redir.txt
            [std::fclose](fclose.html)(stdout);
        }
    }
