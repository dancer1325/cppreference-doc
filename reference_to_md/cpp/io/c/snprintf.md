...
| Formatted output  
---  
| **printf fprintfsprintfsnprintf**(C++11)  
---  
...
int fprintf( [std::FILE](FILE.html)* stream, const char* format, ... ); |  (2)  |   
int sprintf( char* buffer, const char* format, ... ); |  (3)  |   
int snprintf( char* buffer, [std::size_t](../../types/size_t.html) buf_size, const char* format, ... ); |  (4)  |  (since C++11)  
| |   
...
## Contents
  ...
  * [1 Parameters](printf.html#Parameters)
  * [2 Return value](printf.html#Return_value)
  * [3 Notes](printf.html#Notes)
  * [4 Example](printf.html#Example)
  * [5 See also](printf.html#See_also)
...
`s` |  Writes a **character string**. 
  ...
  * If the **l** specifier is used, the argument must be a pointer to the initial element of an array of wchar_t, which is converted to char array as if by a call to [std::wcrtomb](../../string/multibyte/wcrtomb.html "cpp/string/multibyte/wcrtomb") with zero-initialized conversion state. 
...
`F` (C++11) |  Converts **floating-point number** to the decimal notation in the style _[-]ddd.ddd_. 
  ...
  * For infinity and not-a-number conversion style see [notes](printf.html#Notes). 
...
Calling `std::snprintf` with zero buf_size and null pointer for buffer is useful (when the overhead of double-call is acceptable) to determine the necessary buffer size to contain the output: 
    ...
    auto fmt = "sqrt(2) = %f";
    int sz = std::snprintf(nullptr, 0, fmt, [std::sqrt](../../numeric/math/sqrt.html)(2));
...
Run this code
    ...
     
        ...
        [std::uint32_t](../../types/integer.html) val = [std::numeric_limits](../../types/numeric_limits.html)<[std::uint32_t](../../types/integer.html)>::max();
        std::printf("\tLargest 32-bit value is %" [PRIu32](../../types/integer.html) " or %#" [PRIx32](../../types/integer.html) "\n",
                                                     val,            val);
    }
