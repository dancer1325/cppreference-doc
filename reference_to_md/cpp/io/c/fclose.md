...
---|---|---  
int fclose( [std::FILE](FILE.html)* stream ); |  |   
| |   
...
Whether or not the operation succeeds, the stream is no longer associated with a file, and the buffer allocated by [std::setbuf](setbuf.html "cpp/io/c/setbuf") or [std::setvbuf](setvbuf.html "cpp/io/c/setvbuf"), if any, is also disassociated and deallocated if automatic allocation was used. 

If any data are written to an output device, returning from `std::fclose` establishes an [observable checkpoint](../../language/as_if.html "cpp/language/as if").  | (since C++26)  
---|---  
...
## Contents
  ...
  * [4 See also](fclose.html#See_also)
