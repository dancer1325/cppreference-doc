...
Reads [stdin](std_streams.html "cpp/io/c/std streams") into given character string until a newline character is found or end-of-file occurs. 
...
Run this code
    ...
    int main()
    {
        [std::puts](puts.html)("Never use std::gets(). Use std::fgets() instead!");
     
        [std::array](../../container/array.html)<char, 16> buf;
     
        [std::printf](fprintf.html)("Enter a string:\n>");
     
        if ([std::fgets](fgets.html)(buf.data(), buf.size(), stdin))
        {
            const auto len = [std::strlen](../../string/byte/strlen.html)(buf.data());
            [std::printf](fprintf.html)(
                "The input string:\n[%s] is %s and has the length %li characters.\n",
                buf.data(), len + 1 < buf.size() ? "not truncated" : "truncated", len
            );
        }
        else if ([std::feof](feof.html)(stdin))
        {
            [std::puts](puts.html)("Error: the end of stdin stream has been reached.");
        }
        else if ([std::ferror](ferror.html)(stdin))
        {
            [std::puts](puts.html)("I/O error when reading from stdin.");
        }
        else
        {
            [std::puts](puts.html)("Unknown stdin error.");
        }
    }
