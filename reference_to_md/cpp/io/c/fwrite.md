...
---|---|---  
[std::size_t](../../types/size_t.html) fwrite( const void* buffer, [std::size_t](../../types/size_t.html) size, [std::size_t](../../types/size_t.html) count, [std::FILE](FILE.html)* stream ); |  |   
| |   
  
Writes up to count binary objects from the given array buffer to the output stream stream. The objects are written as if by reinterpreting each object as an array of unsigned char and calling [std::fputc](fputc.html "cpp/io/c/fputc") size times for each object to write those unsigned chars into stream, in order. The file position indicator for the stream is advanced by the number of characters written. 
...
## Contents
  ...
  * [4 See also](fwrite.html#See_also)
...
Run this code
    ...
    int main ()
    {
        ...
        if ([std::FILE](FILE.html)* f1 = [std::fopen](fopen.html)("file.bin", "wb"))
        {
            [std::array](../../container/array.html)<int, 3> v = {42, -1, 7}; // underlying storage of std::array is an array
            std::fwrite(v.data(), sizeof v[0], v.size(), f1);
            [std::fclose](fclose.html)(f1);
        }
     
        // read the same data and print it to the standard output
        if ([std::FILE](FILE.html)* f2 = [std::fopen](fopen.html)("file.bin", "rb"))
        {
            [std::vector](../../container/vector.html)<int> rbuf(10); // underlying storage of std::vector is also an array
            [std::size_t](../../types/size_t.html) sz = [std::fread](fread.html)(rbuf.data(), sizeof rbuf[0], rbuf.size(), f2);
            [std::fclose](fclose.html)(f2);
            for ([std::size_t](../../types/size_t.html) n = 0; n < sz; ++n)
                [std::printf](fprintf.html)("%d\n", rbuf[n]);
        }
    }
