...
---|---|---  
[std::FILE](FILE.html)* tmpfile(); |  |   
| |   
  
Creates and opens a temporary file with a unique auto-generated filename. 

The file is opened as a binary file for update (as by [std::fopen](fopen.html "cpp/io/c/fopen") with access mode "wb+"). At least [TMP_MAX](../c.html "cpp/io/c") files may be opened during the lifetime of a program (this limit may be shared with [std::tmpnam](tmpnam.html "cpp/io/c/tmpnam") and may be further limited by [FOPEN_MAX](../c.html "cpp/io/c")). 

If the program closes the file, e.g. by executing [std::fclose](fclose.html "cpp/io/c/fclose"), the file is automatically deleted. 

If the program terminates normally (by calling [std::exit](../../utility/program/exit.html "cpp/utility/program/exit"), returning from main, etc), all files that were opened by calling `std::tmpfile` are also automatically deleted. 
...
## Contents
  ...
  * [5 See also](tmpfile.html#See_also)
...
Run this code
    ...
    int main()
    {
        [std::cout](../cout.html) << "TMP_MAX = " << [TMP_MAX](../c.html) << '\n'
                  << "FOPEN_MAX = " << [FOPEN_MAX](../c.html) << '\n';
        [std::FILE](FILE.html)* tmpf = std::tmpfile();
        [std::fputs](fputs.html)("Hello, world", tmpf);
        [std::rewind](rewind.html)(tmpf);
        char buf[6];
        [std::fgets](fgets.html)(buf, sizeof buf, tmpf);
        [std::cout](../cout.html) << buf << '\n';
     
        // Linux-specific method to display the tmpfile name
        [std::cout](../cout.html) << fs::read_symlink(
                         fs::path("/proc/self/fd") / [std::to_string](../../string/basic_string/to_string.html)(fileno(tmpf))
                     ) << '\n';
    }
