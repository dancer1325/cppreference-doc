...
Specializes the [std::swap](../../utility/swap.html "cpp/algorithm/swap") algorithm for [std::basic_fstream](../basic_fstream.html). Exchanges the state of lhs with that of rhs. Effectively calls lhs.swap(rhs). 
...
Run this code
    ...
    #include <string>
     
    bool create_stream([std::fstream](../basic_fstream.html)& fs)
    {
        try
        {
    ...
     
            if ([std::fstream](../basic_fstream.html) ts{some_name, some_flags}; ts.is_open())
            {
    ...
    int main()
    {
        if ([std::fstream](../basic_fstream.html) fs; create_stream(fs))
        {
            // use fs stream
        }
    }
