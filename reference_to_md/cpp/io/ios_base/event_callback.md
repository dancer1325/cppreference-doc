...
The type of function callbacks that can be registered using [register_callback()](register_callback.html "cpp/io/ios base/register callback") to be called on specific events. 

type is a value of type [`ios_base::event`](event.html "cpp/io/ios base/event") which indicates the type of the event that will invoke this callback. 

ios refers to the stream object for which the callback is invoked: *this is passed as the argument when callbacks are invoked by [std::ios_base](../ios_base.html "cpp/io/ios base") and [std::basic_ios](../basic_ios.html "cpp/io/basic ios") member functions. 
