[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/io/strstream/pcount&action=edit)
...
Returns the number of characters that were output in the put area of the associated [std::strstreambuf](../strstreambuf.html). Effectively calls rdbuf()->pcount(). 
...
Run this code
    ...
    int main()
    {
        ...
        [std::cout](../cout.html) << "The size of the output is " << dyn.pcount()
                  << " and it holds \"" << dyn.str() << "\"\n";
        dyn.freeze(false);
     
        char buf[10];
        [std::strstream](../strstream.html) user(buf, 10); // user-provided output buffer
        user << 1.23; // note: no std::ends
        [std::cout](../cout.html).write(buf, user.pcount());
        [std::cout](../cout.html) << '\n';
    }
