[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/io/strstream/str&action=edit)
...
Before a call to `str()` that uses the result as a C-string, the stream buffer must be null-terminated. Regular output such as with stream << 1.2 does not store a null terminator, it must be appended explicitly, typically with the manipulator [std::ends](../manip/ends.html "cpp/io/manip/ends"). 

After a call to `str()`, dynamic streams become frozen. A call to [`freeze(false)`](freeze.html "cpp/io/strstream/freeze") is required before exiting the scope in which this strstream object was created. otherwise the destructor will leak memory. Also, additional output to a frozen stream may be truncated once it reaches the end of the allocated buffer, which may leave the buffer not null-terminated. 
...
Run this code
    ...
    int main()
    {
        ...
        [std::cout](../cout.html) << "The output stream holds \"";
        [std::cout](../cout.html).write(dyn.str(), dyn.pcount()) << "\"\n"; 
        // the stream is now frozen due to str()
        dyn << " More text" << [std::ends](../manip/ends.html);
        [std::cout](../cout.html) << "The output stream holds \"";
        [std::cout](../cout.html).write(dyn.str(), dyn.pcount()) << "\"\n";
        dyn.freeze(false);
    }
