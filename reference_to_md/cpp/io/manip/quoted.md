...
/*unspecified*/ quoted( const [std::basic_string](../../string/basic_string.html)<CharT, Traits, Allocator>& s,  
CharT delim = CharT('"'), CharT escape = CharT('\\\') ); |  (2)  |  (since C++14)  
template< class CharT, class Traits>  
/*unspecified*/ quoted( [std::basic_string_view](../../string/basic_string_view.html)<CharT, Traits> s,  
CharT delim = CharT('"'), CharT escape = CharT('\\\') ); |  (3)  |  (since C++17)  
template< class CharT, class Traits, class Allocator >  
/*unspecified*/ quoted( [std::basic_string](../../string/basic_string.html)<CharT, Traits, Allocator>& s,  
CharT delim=CharT('"'), CharT escape=CharT('\\\') ); |  (4)  |  (since C++14)  
| |   
  
Allows insertion and extraction of quoted strings, such as the ones found in [CSV](https://en.wikipedia.org/wiki/Comma-separated_values "enwiki:Comma-separated values") or [XML](https://en.wikipedia.org/wiki/XML "enwiki:XML"). 

1-3) When used in an expression out << quoted(s, delim, escape), where `out` is an output stream with `char_type` equal to `CharT` and, for overloads (2,3), `traits_type` equal to `Traits`, behaves as a [FormattedOutputFunction](../../named_req/FormattedOutputFunction.html "cpp/named req/FormattedOutputFunction"), which inserts into out a sequence of characters `seq` constructed as follows:
...
c) In the end, delim is appended to `seq` once more.
    ...
    Finally, outputs each character from the resulting sequence as if by calling out.rdbuf()->sputn(seq, n), where n=[std::max](../../algorithm/max.html)(out.width(), seq.size()) and out.width(0) to cancel the effects of [std::setw](setw.html "cpp/io/manip/setw"), if any. 

4) When used in an expression in >> quoted(s, delim, escape), where `in` is an input stream with `char_type` equal to `CharT` and `traits_type` equal to `Traits`, extracts characters from in, using [std::basic_istream::operator>>](../basic_istream/operator_gtgt.html "cpp/io/basic istream/operator gtgt"), according to the following rules:
...
3) Extracts successive characters from `in` and appends them to s, except that whenever an escape character is extracted, it is ignored and the next character is appended to s. Extraction stops when !in == true or when an unescaped delim character is found.
...
## Contents
  ...
  * [1 Parameters](quoted.html#Parameters)
  * [2 Return value](quoted.html#Return_value)
  * [3 Exceptions](quoted.html#Exceptions)
  * [4 Notes](quoted.html#Notes)
  * [5 Example](quoted.html#Example)
  * [6 See also](quoted.html#See_also)
...
Run this code
    ...
    {
        const [std::string](../../string/basic_string.html) in = "std::quoted() quotes this string and embedded \"quotes\" too";
        [std::stringstream](../basic_stringstream.html) ss;
        ss << std::quoted(in);
        [std::string](../../string/basic_string.html) out;
        ss >> std::quoted(out);
     
        [std::cout](../cout.html) << "Default delimiter case:\n"
                     "read in     [" << in << "]\n"
                     "stored as   [" << ss.str() << "]\n"
                     "written out [" << out << "]\n\n";
    }
    ...
     
        [std::cout](../cout.html) << "Custom delimiter case:\n"
                     "read in     [" << in << "]\n"
                     "stored as   [" << ss.str() << "]\n"
                     "written out [" << out << "]\n\n";
    }
