...
* _storage duration_
  * := [object](objects.md)'s property /
    * == minimum potential lifetime of the storage / contains the object
  * determined -- by the -- construct / create the object
  * ALLOWED ones
    * static
      * associated -- with -- 
        * objects / introduced -- by -- [declarations](declarations.md) & 
        * [temporary objects](lifetime.md#Temporary_object_lifetime) 
    * thread OR thread-local
      * requirements
        * C++11
    * automatic 
    * dynamic 

* The dynamic storage duration is associated with objects created by a [new expression](new.html "cpp/language/new") or with [implicitly created objects](objects.html#Object_creation "cpp/language/object"). 
...
mutable has no effect on storage duration
* For its usage, see [const/volatile](cv.html "cpp/language/cv"). 
