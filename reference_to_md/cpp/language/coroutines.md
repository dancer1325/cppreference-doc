* coroutine
  * == function / 
    * requirements
      * return type
      * 's definition contain SOME of
        * `co_await`
          * suspend execution UNTIL resumed
        * `co_yield`
          * suspend execution -- by -- returning a value
        * `co_return`
          * complete execution -- by -- returning a value
    * can
      * suspend the execution
      * be resumed later
    * stackless
      * == suspend execution -- by -- returning to the caller 
        * data / to resume execution
          * stored | separately from the stack
  * allows
    * sequential code / executes asynchronously 
      * _Example:_ handle non-blocking I/O WITHOUT explicit callbacks
    * algorithms | lazy-computed infinite sequences + other uses
