...
const Facet& use_facet( const [std::locale](locale.html)& loc ); |  |   
| |   
  
Obtains a reference to a facet implemented by loc. 

The program is ill-formed if Facet is not a [facet](locale/facet.html#Facet_class "cpp/locale/locale/facet") whose definition contains the public static member `**id**` or it is a volatile-qualified facet. 

## Contents

  * [1 Parameters](use_facet.html#Parameters)
  * [2 Return value](use_facet.html#Return_value)
  * [3 Exceptions](use_facet.html#Exceptions)
  * [4 Notes](use_facet.html#Notes)
  * [5 Example](use_facet.html#Example)
  * [6 Defect reports](use_facet.html#Defect_reports)
  * [7 See also](use_facet.html#See_also)
...
A [std::locale](locale.html "cpp/locale/locale") object should not be a temporary if a reference to the `Facet` object obtained from `use_facet` is used after the end of statement: 
    ...
    // BAD:
    auto& f = std::use_facet<[std::moneypunct](moneypunct.html)<char, true>>([std::locale](locale.html){"no_NO.UTF-8"});
    foo(f.curr_symbol()); // Error: f internally uses a dangling reference
                          // to a std::locale object that no longer exists.
    // GOOD:
    auto loc = [std::locale](locale.html){"is_IS.UTF-8"}; // OK: a non-temporary object
...
Run this code
    ...
    int main()
    {
        for (const char* name: {"en_US.UTF-8", "de_DE.UTF-8", "en_GB.UTF-8"})
            [std::cout](../io/cout.html) << "Your currency string is "
                      << std::use_facet<[std::moneypunct](moneypunct.html)<char, true>>([std::locale](locale.html){name}).
                         curr_symbol() << '\n';
    }
...
---|---|---|---  
[LWG 31](https://cplusplus.github.io/LWG/issue31) | C++98  | the returned reference remained usable  
