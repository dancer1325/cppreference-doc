...
[monotonic_buffer_resource::do_allocate](do_allocate.html "cpp/memory/monotonic buffer resource/do allocate")  
...
---|---|---  
explicit monotonic_buffer_resource( [std::pmr::memory_resource](../memory_resource.html)* upstream ); |  (2)  |  (since C++17)  
explicit monotonic_buffer_resource( [std::size_t](../../types/size_t.html) initial_size ); |  (3)  |  (since C++17)  
monotonic_buffer_resource( [std::size_t](../../types/size_t.html) initial_size,  
[std::pmr::memory_resource](../memory_resource.html)* upstream ); |  (4)  |  (since C++17)  
monotonic_buffer_resource( void* buffer, [std::size_t](../../types/size_t.html) buffer_size ); |  (5)  |  (since C++17)  
monotonic_buffer_resource( void* buffer, [std::size_t](../../types/size_t.html) buffer_size,  
[std::pmr::memory_resource](../memory_resource.html)* upstream ); |  (6)  |  (since C++17)  
monotonic_buffer_resource( const monotonic_buffer_resource& ) = delete; |  (7)  |  (since C++17)  
| |   
  
Constructs a [`monotonic_buffer_resource`](../monotonic_buffer_resource.html "cpp/memory/monotonic buffer resource"). The constructors not taking an upstream memory resource pointer use the return value of [std::pmr::get_default_resource](../get_default_resource.html "cpp/memory/get default resource") as the upstream memory resource. 

1,2) Sets the _current buffer_ to null and the _next buffer size_ to an implementation-defined size.

3,4) Sets the _current buffer_ to null and the _next buffer size_ to a size no smaller than initial_size.

5,6) Sets the _current buffer_ to buffer and the _next buffer size_ to buffer_size (but not less than 1). Then increase the _next buffer size_ by an implementation-defined growth factor (which does not have to be integral).

7) Copy constructor is deleted.

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/monotonic_buffer_resource/monotonic_buffer_resource&action=edit&section=1 "Edit section: Parameters")] Parameters 
