...
Deallocates the storage referenced by the pointer p, which must be a pointer obtained by an earlier call to [allocate()](allocate.html "cpp/memory/allocator/allocate") or [`allocate_at_least()`](allocate_at_least.html "cpp/memory/allocator/allocate at least")(since C++23). 

The argument n must be equal to the first argument of the call to [allocate()](allocate.html "cpp/memory/allocator/allocate") that originally produced p, or in the range `[`m`, `count`]` if p is obtained from a call to allocate_at_least(m) which returned {p, count}(since C++23); otherwise, the behavior is undefined. 

Calls ::[operator delete](../new/operator_delete.html)(void*) or ::[operator delete](../new/operator_delete.html)(void*, [std::align_val_t](../new/align_val_t.html))(since C++17), but it is unspecified when and how it is called. 
...
Run this code
    ...
    class S
    {
        ...
        int m{};
        void pre() const { [std::cout](../../io/cout.html) << "#" << m << [std::string](../../string/basic_string.html)(m, ' '); }
    public:
        S(int x) : m{n++} { pre(); [std::cout](../../io/cout.html) << "S::S(" << x << ");\n"; }
        ~S() { pre(); [std::cout](../../io/cout.html) << "S::~S();\n"; }
        void id() const { pre(); [std::cout](../../io/cout.html) << "S::id();\n"; }
    };
     
    int main()
    {
        constexpr [std::size_t](../../types/size_t.html) n{4};
        [std::allocator](../allocator.html)<S> allocator;
        try
        {
            ...
            //  allocator.construct(&s[i], i + 42); // removed in C++20
                [std::construct_at](../construct_at.html)(&s[i], i + 42);   // since C++20
            }
            [std::for_each_n](../../algorithm/for_each_n.html)(s, n, [](const auto& e) { e.id(); });
            [std::destroy_n](../destroy_n.html)(s, n);
            allocator.deallocate(s, n);
        }
        catch ([std::bad_array_new_length](../new/bad_array_new_length.html) const& ex) { [std::cout](../../io/cout.html) << ex.what() << '\n'; }
        catch ([std::bad_alloc](../new/bad_alloc.html) const& ex) { [std::cout](../../io/cout.html) << ex.what() << '\n'; }
    }
