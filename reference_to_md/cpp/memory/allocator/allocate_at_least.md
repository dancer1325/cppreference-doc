...
Allocates count * sizeof(T) bytes of uninitialized storage, where `count` is an unspecified integer value not less than n, by calling ::[operator new](../new/operator_new.html) (possibly with an additional [std::align_val_t](../new/align_val_t.html "cpp/memory/new/align val t") argument), but it is unspecified when and how this function is called. 
...
The "unspecified when and how" wording makes it possible to [combine or optimize away heap allocations](../../language/new.html#Allocation "cpp/language/new") made by the standard library containers, even though such optimizations are disallowed for direct calls to ::[operator new](../new/operator_new.html). For example, this is implemented by libc++ ([[1]](https://github.com/llvm-mirror/libcxx/blob/master@%7B2017-02-09%7D/include/memory#L1766-L1772) and [[2]](https://github.com/llvm-mirror/libcxx/blob/master@%7B2017-02-09%7D/include/new#L211-L217)). 

After calling [`allocate_at_least`](allocate_at_least.html#top) and before construction of elements, pointer arithmetic of T* is well-defined within the allocated array, but the behavior is undefined if elements are accessed. 
...
Run this code
    ...
    int main()
    {
        const [std::size_t](../../types/size_t.html) count{69};
        [std::allocator](../allocator.html)<int> alloc;
        [std::allocation_result](../allocation_result.html) res{alloc.allocate_at_least(count)};
        [std::print](../../io/print.html)("count: {}\n"
                   "res.ptr: {}\n"
