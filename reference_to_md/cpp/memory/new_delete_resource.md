...
---|---|---  
[std::pmr::memory_resource](memory_resource.html)* new_delete_resource() noexcept; |  |  (since C++17)  
| |   
...
Returns a pointer `p` to a static storage duration object of a type derived from [std::pmr::memory_resource](memory_resource.html "cpp/memory/memory resource"), with the following properties: 
  ...
  * its `allocate()` function uses ::[operator new](new/operator_new.html) to allocate memory; 
  * its `deallocate()` function uses ::[operator delete](new/operator_delete.html) to deallocate memory; 
