...
---|---|---  
template< class T >  
void swap( [std::shared_ptr](../shared_ptr.html)<T>& lhs, [std::shared_ptr](../shared_ptr.html)<T>& rhs ) noexcept; |  |  (since C++11)  
| |   
  
Specializes the [std::swap](../../utility/swap.html "cpp/algorithm/swap") algorithm for [std::shared_ptr](../shared_ptr.html). Swaps the contents of lhs and rhs. Calls lhs.swap(rhs). 
