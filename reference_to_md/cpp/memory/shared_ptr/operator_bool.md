...
## Contents
  ...
  * [5 See also](operator_bool.html#See_also)
...
Run this code
    ...
    #include <memory>
     
    void report([std::shared_ptr](../shared_ptr.html)<int> ptr) 
    {
        if (ptr)
            [std::cout](../../io/cout.html) << "*ptr=" << *ptr << "\n";
        else
            [std::cout](../../io/cout.html) << "ptr is not a valid pointer.\n";
    }
     
    int main()
    {
        [std::shared_ptr](../shared_ptr.html)<int> ptr;
        report(ptr);
     
        ptr = [std::make_shared](make_shared.html)<int>(7);
        report(ptr);
    }
