...
---|---|---  
template< class Ptr >  
constexpr auto to_address( const Ptr& p ) noexcept; |  (1)  |  (since C++20)  
template< class T >  
constexpr T* to_address( T* p ) noexcept; |  (2)  |  (since C++20)  
| |   
  
Obtain the address represented by p without forming a reference to the object pointed to by p. 

1) [Fancy pointer](../named_req/Allocator.html#Fancy_pointers "cpp/named req/Allocator") overload: If the expression [std::pointer_traits](pointer_traits.html)<Ptr>::to_address(p) is well-formed, returns the result of that expression. Otherwise, returns std::to_address(p.operator->()).

2) Raw pointer overload: If `T` is a function type, the program is ill-formed. Otherwise, returns p unmodified.

## Contents

  * [1 Parameters](to_address.html#Parameters)
  * [2 Return value](to_address.html#Return_value)
  * [3 Possible implementation](to_address.html#Possible_implementation)
  * [4 Notes](to_address.html#Notes)
  * [5 Example](to_address.html#Example)
  * [6 See also](to_address.html#See_also)
...
### Possible implementation
    ...
    {
        static_assert(![std::is_function_v](../types/is_function.html)<T>);
        return p;
    }
     
    template<class T>
    ...
    {
        if constexpr (requires{ [std::pointer_traits](pointer_traits.html)<T>::to_address(p); })
            return [std::pointer_traits](pointer_traits.html)<T>::to_address(p);
        else
...
The fancy pointer overload of `std::to_address` inspects the [std::pointer_traits](pointer_traits.html)<Ptr> specialization. If instantiating that specialization is itself ill-formed (typically because `element_type` cannot be defined), that results in a hard error outside the immediate context and renders the program ill-formed. 

`std::to_address` may additionally be used on iterators that satisfy [std::contiguous_iterator](../iterator/contiguous_iterator.html "cpp/iterator/contiguous iterator"). 
...
Run this code
    ...
    auto allocator_new(A& a)
    {
        ...
        try
        {
            [std::allocator_traits](allocator_traits.html)<A>::construct(a, std::to_address(p));
        }
        catch (...)
        {
            a.deallocate(p, 1);
    ...
     
    template<class A>
    void allocator_delete(A& a, typename [std::allocator_traits](allocator_traits.html)<A>::pointer p)
    {
        [std::allocator_traits](allocator_traits.html)<A>::destroy(a, std::to_address(p));
        a.deallocate(p, 1);
    }
     
    int main()
    {
        [std::allocator](allocator.html)<int> a;
