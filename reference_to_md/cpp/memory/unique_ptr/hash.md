...
struct hash<[std::unique_ptr](../unique_ptr.html)<T, Deleter>>; |  |  (since C++11)  
---|---|---  
| |   
  
The template specialization of [std::hash](../../utility/hash.html "cpp/utility/hash") for [std::unique_ptr](../unique_ptr.html)<T, Deleter> allows users to obtain hashes of objects of type [std::unique_ptr](../unique_ptr.html)<T, Deleter>. 

The specialization [std::hash](../../utility/hash.html)<[std::unique_ptr](../unique_ptr.html)<T,D>> is enabled (see [std::hash](../../utility/hash.html "cpp/utility/hash")) if [std::hash](../../utility/hash.html)<typename [std::unique_ptr](../unique_ptr.html)<T,D>::pointer> is enabled, and is disabled otherwise. 

When enabled, for a given [std::unique_ptr](../unique_ptr.html)<T, D> p, this specialization ensures that [std::hash](../../utility/hash.html)<[std::unique_ptr](../unique_ptr.html)<T, D>>()(p) == [std::hash](../../utility/hash.html)<typename [std::unique_ptr](../unique_ptr.html)<T, D>::pointer>()(p.get()). 
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/unique_ptr/hash&action=edit&section=1 "Edit section: Example")] Example
...
Run this code
    ...
    struct Foo
    {
        Foo(int num) : nr(num) { [std::cout](../../io/cout.html) << "Foo(" << nr << ")\n"; }
     
    ...
    int main()
    {
        [std::cout](../../io/cout.html) << [std::boolalpha](../../io/manip/boolalpha.html) << [std::hex](../../io/manip/hex.html);
     
        Foo* foo = new Foo(5);
        [std::unique_ptr](../unique_ptr.html)<Foo> up(foo); 
        [std::cout](../../io/cout.html) << "hash(up):    " << [std::hash](../../utility/hash.html)<[std::unique_ptr](../unique_ptr.html)<Foo>>()(up) << '\n'
                  << "hash(foo):   " << [std::hash](../../utility/hash.html)<Foo*>()(foo) << '\n'
                  << "*up==*foo:   " << (*up == *foo) << "\n\n";
     
        [std::unique_ptr](../unique_ptr.html)<Foo> other = [std::make_unique](make_unique.html)<Foo>(5);
        [std::cout](../../io/cout.html) << "hash(up):    " << [std::hash](../../utility/hash.html)<[std::unique_ptr](../unique_ptr.html)<Foo>>()(up) << '\n'
                  << "hash(other): " << [std::hash](../../utility/hash.html)<[std::unique_ptr](../unique_ptr.html)<Foo>>()(other) << '\n'
                  << "*up==*other: " <<(*up == *other) << "\n\n";
    }
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/unique_ptr/hash&action=edit&section=2 "Edit section: See also")] See also
