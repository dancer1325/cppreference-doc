...
` `::new ([`_voidify_`](voidify.html "cpp/memory/voidify")(*d_first))  
` `typename [std::iterator_traits](../iterator/iterator_traits.html)<NoThrowForwardIt>::value_type(/* value */);  
return d_first;
...
This overload participates in overload resolution only if all following conditions are satisfied:  [std::is_execution_policy_v](../algorithm/is_execution_policy.html)<[std::decay_t](../types/decay.html)<ExecutionPolicy>> is true.  | (until C++20)  
---|---  
[std::is_execution_policy_v](../algorithm/is_execution_policy.html)<[std::remove_cvref_t](../types/remove_cvref.html)<ExecutionPolicy>> is true.  | (since C++20)  
  
  


If d_first` + ``[`​0​`, `[std::distance](../iterator/distance.html)(first, last)`)` overlaps with `[`first`, `last`)`, the behavior is undefined.  | (since C++20)  
---|---  
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=1 "Edit section: Parameters")] Parameters

first, last  |  \-  |  the pair of iterators defining the [range](../iterator.html#Ranges "cpp/iterator") of elements to move   
---|---|---  
d_first  |  \-  |  the beginning of the destination range   
policy  |  \-  |  the [execution policy](../algorithm/execution_policy_tag_t.html "cpp/algorithm/execution policy tag t") to use   
Type requirements   
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=2 "Edit section: Return value")] Return value

As described above. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=3 "Edit section: Complexity")] Complexity

Linear in the distance between first and last. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=4 "Edit section: Exceptions")] Exceptions

The overload with a template parameter named `ExecutionPolicy` reports errors as follows: 

  * If execution of a function invoked as part of the algorithm throws an exception and `ExecutionPolicy` is one of the [standard policies](../algorithm/execution_policy_tag_t.html "cpp/algorithm/execution policy tag t"), [std::terminate](../error/terminate.html "cpp/error/terminate") is called. For any other `ExecutionPolicy`, the behavior is implementation-defined. 
  * If the algorithm fails to allocate memory, [std::bad_alloc](new/bad_alloc.html "cpp/memory/new/bad alloc") is thrown. 



### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=5 "Edit section: Notes")] Notes

When the input iterator deferences to an rvalue, the behavior of `std::uninitialized_move` is same as [std::uninitialized_copy](uninitialized_copy.html "cpp/memory/uninitialized copy"). 

[Feature-test](../utility/feature_test.html "cpp/utility/feature test") macro | Value | Std | Feature   
---|---|---|---  
[`__cpp_lib_raw_memory_algorithms`](../experimental/feature_test.html#cpp_lib_raw_memory_algorithms "cpp/feature test") | [`202411L`](../compiler_support/26.html#cpp_lib_raw_memory_algorithms_202411L "cpp/compiler support/26") | (C++26) | constexpr for [specialized memory algorithms](../memory.html#Uninitialized_memory_algorithms "cpp/memory"), ([1](uninitialized_move.html#Version_1))  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=6 "Edit section: Possible implementation")] Possible implementation
    ...
    {
        using ValueType = typename [std::iterator_traits](../iterator/iterator_traits.html)<NoThrowForwardIt>::value_type;
        auto current = d_first;
        try
        {
            for (; first != last; ++first, (void) ++current)
            {
                auto addr = static_cast<void*>([std::addressof](addressof.html)(*current));
                if constexpr ([std::is_lvalue_reference_v](../types/is_lvalue_reference.html)<decltype(*first)>)
        ...
        {
            [std::destroy](destroy.html)(d_first, current);
            throw;
        }
    }  
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=7 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        [std::string](../string/basic_string.html) in[]{"Home", "Work!"};
        print("initially, in: ", [std::begin](../iterator/begin.html)(in), [std::end](../iterator/end.html)(in));
     
        if (
            constexpr auto sz = [std::size](../iterator/size.html)(in);
            void* out = [std::aligned_alloc](c/aligned_alloc.html)(alignof([std::string](../string/basic_string.html)), sizeof([std::string](../string/basic_string.html)) * sz))
        {
            try
            {
                ...
                auto last{first + sz};
                std::uninitialized_move([std::begin](../iterator/begin.html)(in), [std::end](../iterator/end.html)(in), first);
     
                print("after move, in: ", [std::begin](../iterator/begin.html)(in), [std::end](../iterator/end.html)(in));
                print("after move, out: ", first, last);
     
                [std::destroy](destroy.html)(first, last);
            }
            catch (...)
            {
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=8 "Edit section: Defect reports")] Defect reports
...
[LWG 3918](https://cplusplus.github.io/LWG/issue3918) | C++17  | additional temporary materialization was required  
when the input iterator deferences to a prvalue  | copies the element in this case   
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/memory/uninitialized_move&action=edit&section=9 "Edit section: See also")] See also
