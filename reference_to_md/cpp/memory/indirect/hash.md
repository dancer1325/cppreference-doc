...
struct hash<std::indirect<T, Allocator>>; |  |  (since C++26)  
| |   
  
The partial specialization of [std::hash](../../utility/hash.html "cpp/utility/hash") for std::indirect allows users to obtain hashes of the objects owned by `indirect` objects. 

The specialization [std::hash](../../utility/hash.html)<std::indirect<T, Allocator>> is [enabled](../../utility/hash.html "cpp/utility/hash") if [std::hash](../../utility/hash.html)<T> is enabled, and is disabled otherwise. 

When enabled, for an object obj of type std::indirect<T, Allocator>, [std::hash](../../utility/hash.html)<std::indirect<T, Allocator>>()(obj) evaluates to: 

  * [std::hash](../../utility/hash.html)<T>()(*obj), if obj is not [valueless](valueless_after_move.html "cpp/memory/indirect/valueless after move"). 
