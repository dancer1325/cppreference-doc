...
| (1) |   
template< class T >  
struct owner_less; /* undefined */ |  |  (since C++11)   
(until C++17)  
template< class T = void >  
struct owner_less; /* undefined */ |  |  (since C++17)  
template< class T >  
struct owner_less<[std::shared_ptr](shared_ptr.html)<T>>; |  (2)  |  (since C++11)  
template< class T >  
struct owner_less<[std::weak_ptr](weak_ptr.html)<T>>; |  (3)  |  (since C++11)  
template<>  
struct owner_less<void>; |  (4)  |  (since C++17)  
| |   
  
This function object provides owner-based (as opposed to value-based) mixed-type ordering of both [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr") and [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr"). The order is such that two smart pointers compare equivalent only if they are both empty or if they share ownership, even if the values of the raw pointers obtained by `get()` are different (e.g. because they point at different subobjects within the same object). 

1) Owner-based mixed-type ordering is not provided for types other than [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr") and [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr").

2) The owner-based mixed-type ordering of [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr").

It is the preferred comparison predicate when building associative containers with [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr") as keys, that is, [std::map](../container/map.html)<[std::shared_ptr](shared_ptr.html)<T>, U, std::owner_less<[std::shared_ptr](shared_ptr.html)<T>>>.

3) The owner-based mixed-type ordering of [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr").

It is the preferred comparison predicate when building associative containers with [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr") as keys, that is, [std::map](../container/map.html)<[std::weak_ptr](weak_ptr.html)<T>, U, std::owner_less<[std::weak_ptr](weak_ptr.html)<T>>>.

4) The void specialization deduces the parameter types from the arguments.

The default operator< is not defined for weak pointers, and may wrongly consider two shared pointers for the same object non-equivalent (see [`std::shared_ptr::owner_before`](shared_ptr/owner_before.html "cpp/memory/shared ptr/owner before")). 

| 

## Contents

  * [1 Specializations](owner_less.html#Specializations)
  * [2 Nested types](owner_less.html#Nested_types)
  * [3 Member functions](owner_less.html#Member_functions)
  * [4 std::owner_less::operator()](owner_less.html#std::owner_less::operator.28.29)
    * [4.1 Parameters](owner_less.html#Parameters)
    * [4.2 Return value](owner_less.html#Return_value)
    * [4.3 Defect reports](owner_less.html#Defect_reports)
    * [4.4 See also](owner_less.html#See_also)
...
`first_argument_type` (deprecated in C++17) |  (2) [std::shared_ptr](shared_ptr.html)<T>  
(3) [std::weak_ptr](weak_ptr.html)<T>  
`second_argument_type` (deprecated in C++17) |  (2) [std::shared_ptr](shared_ptr.html)<T>  
(3) [std::weak_ptr](weak_ptr.html)<T>  
(until C++20)  
...
const [std::shared_ptr](shared_ptr.html)<T>& rhs ) const noexcept; |  |  (since C++11)  
member only of specialization (3) |  |   
bool operator()( const [std::weak_ptr](weak_ptr.html)<T>& lhs,   
const [std::weak_ptr](weak_ptr.html)<T>& rhs ) const noexcept; |  |  (since C++11)  
