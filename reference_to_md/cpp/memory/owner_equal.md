...
This function object provides owner-based (as opposed to value-based) mixed-type equal comparison of both [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr") and [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr"). The comparison is such that two smart pointers compare equivalent only if they are both empty or if they share ownership, even if the values of the raw pointers obtained by `get()` are different (e.g. because they point at different subobjects within the same object). 

1) Owner-based mixed-type equal comparison is not provided for types other than [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr") and [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr").

2) The owner-based mixed-type equal comparison of [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr") and [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr").

It is the preferred comparison predicate when building unordered associative containers with [std::shared_ptr](shared_ptr.html "cpp/memory/shared ptr") and [std::weak_ptr](weak_ptr.html "cpp/memory/weak ptr") as keys together with std::owner_hash, that is, [std::unordered_map](../container/unordered_map.html)<[std::shared_ptr](shared_ptr.html)<T>, U, std::owner_hash, std::owner_equal> or [std::unordered_map](../container/unordered_map.html)<[std::weak_ptr](weak_ptr.html)<T>, U, std::owner_hash, std::owner_equal>.

3) `std::owner_equal` deduces the parameter types from the arguments.

## Contents

  * [1 Nested types](owner_equal.html#Nested_types)
  * [2 Member functions](owner_equal.html#Member_functions)
  * [3 std::owner_equal::operator()](owner_equal.html#std::owner_equal::operator.28.29)
    * [3.1 Parameters](owner_equal.html#Parameters)
    * [3.2 Return value](owner_equal.html#Return_value)
    * [3.3 Notes](owner_equal.html#Notes)
    * [3.4 See also](owner_equal.html#See_also)
...
const [std::shared_ptr](shared_ptr.html)<U>& rhs ) const noexcept; |  |  (since C++26)  
---|---|---  
template< class T, class U >  
bool operator()( const [std::shared_ptr](shared_ptr.html)<T>& lhs,   
const [std::weak_ptr](weak_ptr.html)<U>& rhs ) const noexcept; |  |  (since C++26)  
template< class T, class U >  
