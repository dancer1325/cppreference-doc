...
**filesystem::relative filesystem::proximate**  
...
---|---|---  
path relative( const [std::filesystem::path](path.html)& p,  
[std::error_code](../error/error_code.html)& ec ); |  (1)  |  (since C++17)  
path relative( const [std::filesystem::path](path.html)& p,  
const [std::filesystem::path](path.html)& base = [std::filesystem::current_path](current_path.html)() ); |  (2)  |  (since C++17)  
path relative( const [std::filesystem::path](path.html)& p,  
const [std::filesystem::path](path.html)& base,  
[std::error_code](../error/error_code.html)& ec ); |  (3)  |  (since C++17)  
path proximate( const [std::filesystem::path](path.html)& p,  
[std::error_code](../error/error_code.html)& ec ); |  (4)  |  (since C++17)  
path proximate( const [std::filesystem::path](path.html)& p,  
const [std::filesystem::path](path.html)& base = [std::filesystem::current_path](current_path.html)() ); |  (5)  |  (since C++17)  
path proximate( const [std::filesystem::path](path.html)& p,  
const [std::filesystem::path](path.html)& base,  
...
2,3) Returns p made relative to base. Resolves symlinks and normalizes both p and base before other processing. Effectively returns [std::filesystem::weakly_canonical](canonical.html)(p).lexically_relative([std::filesystem::weakly_canonical](canonical.html)(base)) or [std::filesystem::weakly_canonical](canonical.html)(p, ec).lexically_relative([std::filesystem::weakly_canonical](canonical.html)(base, ec)), except the error code form returns path() at the first error occurrence, if any.

4) Returns proximate(p, current_path(), ec).

5,6) Effectively returns [std::filesystem::weakly_canonical](canonical.html)(p).lexically_proximate([std::filesystem::weakly_canonical](canonical.html)(base)) or [std::filesystem::weakly_canonical](canonical.html)(p, ec).lexically_proximate([std::filesystem::weakly_canonical](canonical.html)(base, ec)), except the error code form returns path() at the first error occurrence, if any.

## Contents

  * [1 Parameters](relative.html#Parameters)
  * [2 Return value](relative.html#Return_value)
  * [3 Exceptions](relative.html#Exceptions)
  * [4 Example](relative.html#Example)
  * [5 See also](relative.html#See_also)
...
2,5) Throws [std::filesystem::filesystem_error](filesystem_error.html "cpp/filesystem/filesystem error") on underlying OS API errors, constructed with p as the first path argument, base as the second path argument, and the OS error code as the error code argument.
...
Run this code
    ...
    #include <iostream>
     
    void show([std::filesystem::path](path.html) x, [std::filesystem::path](path.html) y)
    {
