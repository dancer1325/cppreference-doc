...
---|---|---  
bool exists( [std::filesystem::file_status](file_status.html) s ) noexcept; |  (1)  |  (since C++17)  
bool exists( const [std::filesystem::path](path.html)& p ); |  (2)  |  (since C++17)  
bool exists( const [std::filesystem::path](path.html)& p, [std::error_code](../error/error_code.html)& ec ) noexcept; |  (3)  |  (since C++17)  
| |   
...
2,3) Let s be a [std::filesystem::file_status](file_status.html) determined as if by status(p) or status(p, ec) (symlinks are followed), respectively. Returns exists(s). The non-throwing overload calls ec.clear() if status_known(s).
...
## Contents
  ...
  * [4 Notes](exists.html#Notes)
  * [5 Example](exists.html#Example)
  * [6 See also](exists.html#See_also)
...
2) Throws [std::filesystem::filesystem_error](filesystem_error.html "cpp/filesystem/filesystem error") on underlying OS API errors, constructed with p as the first path argument and the OS error code as the error code argument.

3) Sets a [std::error_code](../error/error_code.html)& parameter to the OS API error code if an OS API call fails, and executes ec.[`clear`](../error/error_code/clear.html "cpp/error/error code/clear")() if no errors occur.
