...
Returns the substring from the beginning of [`filename()`](filename.html "cpp/filesystem/path/filename") up to and not including the last period (`**.**`) character, with the following exceptions: 
  ...
  * If the filename is one of the special filesystem components dot or dot-dot, or if it has no periods, the function returns the entire [`filename()`](filename.html "cpp/filesystem/path/filename"). 
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/filesystem/path/stem&action=edit&section=1 "Edit section: Parameters")] Parameters

(none) 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/filesystem/path/stem&action=edit&section=2 "Edit section: Return value")] Return value
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/filesystem/path/stem&action=edit&section=3 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        for (const fs::path p : {"/foo/bar.txt", "/foo/.bar", "foo.bar.baz.tar"})
            [std::cout](../../io/cout.html) << "path: " << p << ", stem: " << p.stem() << '\n';
     
        [std::cout](../../io/cout.html) << '\n';
     
        for (fs::path p = "foo.bar.baz.tar"; !p.extension().empty(); p = p.stem())
            [std::cout](../../io/cout.html) << "path: " << p << ", extension: " << p.extension() << '\n';
    }
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/filesystem/path/stem&action=edit&section=4 "Edit section: See also")] See also
