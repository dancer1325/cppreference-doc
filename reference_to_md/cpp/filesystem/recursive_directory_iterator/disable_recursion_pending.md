...
The call modifies the pending recursion flag on the iterator in such a way that the next time [`increment`](increment.html "cpp/filesystem/recursive directory iterator/increment") is called, the iterator will advance within the current directory even if it is currently referring to a subdirectory that hasn't been visited. 

The status of the pending recursion flag can be queried with [`recursion_pending()`](recursion_pending.html "cpp/filesystem/recursive directory iterator/recursion pending"), which is false after this call. It is reset back to true after [`increment`](increment.html "cpp/filesystem/recursive directory iterator/increment"), and its initial value is also true. 
...
Run this code
    ...
    int main()
    {
        ...
        for (auto i = fs::recursive_directory_iterator("sandbox");
             ...
             ++i)
        {
            [std::cout](../../io/cout.html) << [std::string](../../string/basic_string.html)(i.depth() * 2, ' ') << *i;
