...
---|---|---  
bool is_fifo( [std::filesystem::file_status](file_status.html) s ) noexcept; |  (1)  |  (since C++17)  
bool is_fifo( const [std::filesystem::path](path.html)& p ); |  (2)  |  (since C++17)  
bool is_fifo( const [std::filesystem::path](path.html)& p, [std::error_code](../error/error_code.html)& ec ) noexcept; |  (3)  |  (since C++17)  
| |   
  
Checks if the given file status or path corresponds to a FIFO or pipe file as if determined by POSIX [S_ISFIFO](https://pubs.opengroup.org/onlinepubs/9699919799/basedefs/sys_stat.h.html). 
