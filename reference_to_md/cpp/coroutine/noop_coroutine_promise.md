...
A no-op coroutine behaves as if it 
  ...
  * never reaches its final suspended point if there is any [std::coroutine_handle](coroutine_handle.html "cpp/coroutine/coroutine handle") referring to it. 

| This section is incomplete  
Reason: more precise wording   
---|---  
  
No-op coroutines can be started by [std::noop_coroutine](noop_coroutine.html "cpp/coroutine/noop coroutine"), and controlled by the coroutine handle it returns. The returned coroutine handle is of type [std::noop_coroutine_handle](coroutine_handle.html "cpp/coroutine/coroutine handle"), which is a synonym for [std::coroutine_handle](coroutine_handle.html)<std::noop_coroutine_promise>. 
