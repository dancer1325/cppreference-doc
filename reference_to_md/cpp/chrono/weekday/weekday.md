...
constexpr weekday( const [std::chrono::sys_days](../system_clock.html)& sd ) noexcept; |  (3)  |  (since C++20)  
constexpr explicit weekday( const [std::chrono::local_days](../local_t.html)& ld ) noexcept; |  (4)  |  (since C++20)  
| |   
...
Run this code
    ...
    int main()
    {
        constexpr [std::chrono::weekday](../weekday.html) friday{5}; // uses overload (2)
        static_assert(friday == [std::chrono::Friday](../weekday.html));
     
        for (int y{2020}; y <= 2024; ++y)
        {
            const [std::chrono::year](../year.html) cur_year{y};
            for (int cur_month{1}; cur_month != 13; ++cur_month)
            {
                const [std::chrono::year_month_day](../year_month_day.html) ymd{cur_year/cur_month/13};
                const [std::chrono::weekday](../weekday.html) cur_weekday{[std::chrono::sys_days](../system_clock.html)(ymd)}; // (3)
                if (cur_weekday == friday)
                    [std::cout](../../io/cout.html) << ymd << " is " << friday << '\n';
            }
        }
    }
