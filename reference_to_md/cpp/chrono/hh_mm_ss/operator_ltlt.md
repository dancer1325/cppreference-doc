...
const [std::chrono::hh_mm_ss](../hh_mm_ss.html)<Duration>& t ); |  |  (since C++20)  
| |   
  
Outputs t into the stream os. 

Equivalent to return os << [std::format](../../utility/format/format.html)(os.getloc(), STATICALLY_WIDEN<CharT>("{:L%T}"), hms); where STATICALLY_WIDEN<CharT>("{:L%T}") is "{:L%T}" if `CharT` is char, and L"{:L%T}" if `CharT` is wchar_t. 
