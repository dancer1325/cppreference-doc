...
time_point( const time_point<Clock, Duration2>& t ); |  |  (since C++11)   
(constexpr since C++14)  
| |   
  
Constructs a new `time_point` from one of several optional data sources. 

1) Default constructor, creates a `time_point` representing the `Clock`'s epoch (i.e., [time_since_epoch()](time_since_epoch.html "cpp/chrono/time point/time since epoch") is zero).
...
Run this code
    ...
    #include <iostream>
     
    using Clock = [std::chrono::steady_clock](../steady_clock.html);
    using TimePoint = [std::chrono::time_point](../time_point.html)<Clock>;
     
    void print_ms(const TimePoint& point) 
    {
        ...
        [std::cout](../../io/cout.html) << [std::chrono::duration_cast](../duration/duration_cast.html)<Ms>(since_epoch) << '\n';
    }
     
    int main() 
    {
    ...
     
        const Clock::duration duration_4_seconds = [std::chrono::seconds](../duration.html)(4);
