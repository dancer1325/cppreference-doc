...
**year_month_weekday::operator sys_days year_month_weekday::operator local_days**  
...
Converts *this to a [std::chrono::time_point](../time_point.html "cpp/chrono/time point") representing the same date as this `year_month_weekday`. 

1) If year().ok() && month().ok() && weekday().ok(): 

    

  * If index() == 0, returns a `sys_days` that represents the date 7 days prior to the first [`weekday()`](accessors.html "cpp/chrono/year month weekday/accessors") of the year and month. 
