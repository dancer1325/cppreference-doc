...
**year_month_day_last::operator sys_days year_month_day_last::operator local_days**  
...
Converts *this to a [std::chrono::time_point](../time_point.html "cpp/chrono/time point") representing the same date as this `year_month_day_last`. This is equivalent to composing a `year_month_day` from `year()`, `month()` and `day()` and converting that `year_month_day` to the destination type. 

1) Equivalent to [std::chrono::sys_days](../system_clock.html)(year()/month()/day()).
...
Run this code
    ...
    int main()
    {
        constexpr [std::chrono::year](../year.html) y{2023y};
        constexpr [std::array](../../container/array.html) quarters{"1st"sv, "2nd"sv, "3rd"sv, "4th"sv};
        constexpr auto mq{12 / 4}; // months per quarter        
     
        [std::cout](../../io/cout.html) << "In year " << static_cast<int>(y) << '\n';
        for (auto q = 1; q < 5; ++q)
        {
            const auto ls = y / [std::chrono::month](../month.html)(q * mq) / Sunday[last];
            const auto ld = y / [std::chrono::month](../month.html)(q * mq) / last;
            // subtract last Sunday from last day for day of week
            const auto index = (sys_days(ld) - sys_days(ls)).count();
            [std::cout](../../io/cout.html) << "The " << quarters[q - 1] << " quarter ends on a "  
                      << [std::chrono::weekday](../weekday.html)(index) << '\n';
        }
    }
