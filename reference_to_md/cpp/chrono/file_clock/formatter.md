[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/chrono/clock_time_formatter&action=edit)
...
struct formatter<[std::chrono::file_time](../file_clock.html)<Duration>, CharT>; |  |  (since C++20)  
| |   
  
Specialization of [std::formatter](../../utility/format/formatter.html "cpp/utility/format/formatter") that defines formatting rules for a std::chrono::file_time. 

  * If `**%Z**` is used, it is replaced with "UTC" widened to `CharT`. 
  * If `**%z**` or a modified variant thereof is used, an offset of `0min` will be formatted. 
  * The date and time formatted are equivalent to that formatted for [std::chrono::clock_cast](../clock_cast.html)<[std::chrono::system_clock](../system_clock.html)>(tp) or [std::chrono::clock_cast](../clock_cast.html)<[std::chrono::utc_clock](../utc_clock.html)>(tp), where tp represents the formatting argument. 
...
Unless otherwise specified, if the chrono-spec is empty, the chrono object is formatted as if by [streaming](operator_ltlt.html "cpp/chrono/file clock/operator ltlt") it to an object `os` of type [std::basic_ostringstream](../../io/basic_ostringstream.html)<CharT> with the formatting locale (one of [std::locale::classic](../../locale/locale/classic.html)(), the passed [std::locale](../../locale/locale.html "cpp/locale/locale") object, and [std::locale::global](../../locale/locale/global.html)()) [imbued](../../io/ios_base/imbue.html "cpp/io/ios base/imbue") and copying os.str() to the output buffer with additional padding and adjustments as per format specifiers. 
...
Run this code
    ...
    int main()
    {
        [std::chrono::time_point](../time_point.html)<[std::chrono::utc_clock](../utc_clock.html)> epoch;
     
        [std::cout](../../io/cout.html) << [std::format](../../utility/format/format.html)("The time of the Unix epoch was {0:%F}T{0:%R%z}.", epoch)
                  << '\n';
    }
