...
**year_month_weekday_last::operator sys_days year_month_weekday_last::operator local_days**  
...
Converts *this to a [std::chrono::time_point](../time_point.html "cpp/chrono/time point") representing the same date as this `year_month_weekday_last`. 

1) If [`ok()`](ok.html "cpp/chrono/year month weekday last/ok") is true, returns a `sys_days` that represents the last [`weekday()`](accessors.html "cpp/chrono/year month weekday last/accessors") of the [`year()`](accessors.html "cpp/chrono/year month weekday last/accessors") and [`month()`](accessors.html "cpp/chrono/year month weekday last/accessors"). Otherwise, the returned value is unspecified.

2) Same as (1), but returns `local_days` instead of `sys_days`. Equivalent to local_days(sys_days(*this).time_since_epoch()).

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year_month_weekday_last/operator_days&action=edit&section=1 "Edit section: Example")] Example
