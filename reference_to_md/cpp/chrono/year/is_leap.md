...
Determines if *this represents a leap year in the [proleptic Gregorian calendar](https://en.wikipedia.org/wiki/Proleptic_Gregorian_calendar "enwiki:Proleptic Gregorian calendar"). 
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year/is_leap&action=edit&section=1 "Edit section: Return value")] Return value

true if *this represents a leap year, otherwise false. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year/is_leap&action=edit&section=2 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        ...
        for (const [std::chrono::year](../year.html) y : {2020y, 2021y, 2000y, 3000y})
        {
            if (const int iy{static_cast<int>(y)}; y.is_leap())
                [std::cout](../../io/cout.html) << iy << " is a leap year because it is divisible by "
                          << (iy % 400 == 0 ? "400\n" : "4 and not divisible by 100\n");
            else
                [std::cout](../../io/cout.html) << iy << " is not a leap year\n";
        }
    }
