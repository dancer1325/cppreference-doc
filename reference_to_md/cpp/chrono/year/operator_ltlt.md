...
operator<<( [std::basic_ostream](../../io/basic_ostream.html)<CharT, Traits>& os, const [std::chrono::year](../year.html)& y ); |  |  (since C++20)  
| |   
  
Forms a [std::basic_string](../../string/basic_string.html)<CharT> s consisting of the year value stored in y formatted as a decimal number, left-padded with ​0​ to four digits if the result would otherwise be less than four digits. Then, if !y.ok(), append " is not a valid year" to the formatted string. Inserts that string into os. 

Equivalent to 

return os << (y.ok() ?  
[std::format](../../utility/format/format.html)(STATICALLY_WIDEN<CharT>("{:%Y}"), y) :  
[std::format](../../utility/format/format.html)(STATICALLY_WIDEN<CharT>("{:%Y} is not a valid year"), y));

where STATICALLY_WIDEN<CharT>("...") is "..." if `CharT` is `char`, and L"..." if `CharT` is `wchar_t`. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year/operator_ltlt&action=edit&section=1 "Edit section: Return value")] Return value

os

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year/operator_ltlt&action=edit&section=2 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        constexpr [std::chrono::year](../year.html) y1{2020}, y2{-020}, y3{98304};
        [std::cout](../../io/cout.html) << y1 << '\n'
                  << y2 << '\n'
                  << y3 << '\n';
    }
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year/operator_ltlt&action=edit&section=3 "Edit section: See also")] See also
...
---|---  
[ std::formatter<std::chrono::year>](formatter.html "cpp/chrono/year/formatter")(C++20) |  formatting support for `year`   
