...
**zoned_time::get_local_time zoned_time::operator local_time**  
...
[std::chrono::local_time](../local_t.html)<duration> get_local_time() const; |  |  (since C++20)  
---|---|---  
explicit operator [std::chrono::local_time](../local_t.html)<duration>() const; |  |  (since C++20)  
| |   
  
Obtains a [std::chrono::local_time](../local_t.html)<duration> representing the local time in the time zone corresponding to the time point *this represents. 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/zoned_time/get_local_time&action=edit&section=1 "Edit section: Return value")] Return value

A [std::chrono::local_time](../local_t.html)<duration> computed as if by zone->to_local(get_sys_time()), where `zone` is the non-static data member holding the stored time zone pointer. 
