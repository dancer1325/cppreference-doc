...
Member functions  
---  
[zoned_time::zoned_time](zoned_time.html "cpp/chrono/zoned time/zoned time")  
...
struct formatter<[std::chrono::zoned_time](../zoned_time.html)<Duration, TimeZonePtr>, CharT>  
: [std::formatter](../../utility/format/formatter.html)</*local-time-format-t*/  
<[std::common_type_t](../../types/common_type.html)<Duration,  
[std::chrono::seconds](../duration.html)>>, CharT> |  (1)  |  (since C++20)  
template< class Duration >  
/*local-time-format-t*/<Duration>  
local_time_format( const [std::chrono::local_time](../local_t.html)<Duration>& tp,  
const [std::string](../../string/basic_string.html)* abbrev = nullptr,  
const [std::chrono::seconds](../duration.html)* offset_sec = nullptr ); |  (2)  |  (since C++20)  
Helper templates |  |   
template< class Duration >  
struct /*local-time-format-t*/  
{  
[std::chrono::local_time](../local_t.html)<Duration> /*time*/;  
const [std::string](../../string/basic_string.html)* /*abbrev*/;  
const [std::chrono::seconds](../duration.html)* /*offset_sec*/;  
}; |  (3) | (since C++20)   
(exposition only*)  
template< class Duration, class CharT >  
struct formatter</*local-time-format-t*/<Duration>, CharT>; |  (4)  |  (since C++20)  
| |   
  
1) Specialization of [std::formatter](../../utility/format/formatter.html "cpp/utility/format/formatter") that defines formatting rules for a std::chrono::zoned_time. 
...
using common_duration_type = [std::common_type_t](../../types/common_type.html)<Duration, [std::chrono::seconds](../duration.html)>;  
using formatter_type = [std::formatter](../../utility/format/formatter.html)<`_local-time-format-t_` ﻿<common_duration_type>;  
  
[std::chrono::sys_info](../sys_info.html) info = tp.get_info();  
...
3) An exposition-only type containing all information needed for formatting a std::chrono::zoned_time: 
  ...
  * `_abbrev_` contains the time zone abbreviation. 
  * `_offset_sec_` contains the offset from UTC. 
  * `_time_` contains all other information needed.



4) Specialization of [std::formatter](../../utility/format/formatter.html "cpp/utility/format/formatter") that defines underlying formatting rules for a std::chrono::zoned_time.
...
Let f be the formatting argument, the specialization has the following extra formatting rules: 
  ...
  * If `**%Z**` is used, it is replaced with *f.abbrev if f.abbrev is not a null pointer value; otherwise an exception of type [std::format_error](../../utility/format/format_error.html "cpp/utility/format/format error") is thrown. 
  * If `**%z**` or a modified variant of `**%z**` is used, it is replaced with *f.offset_sec if f.offset_sec is not a null pointer value; otherwise an exception of type [std::format_error](../../utility/format/format_error.html "cpp/utility/format/format error") is thrown.
...
Unless otherwise specified, if the chrono-spec is empty, the chrono object is formatted as if by [streaming](operator_ltlt.html "cpp/chrono/zoned time/operator ltlt") it to an object `os` of type [std::basic_ostringstream](../../io/basic_ostringstream.html)<CharT> with the formatting locale (one of [std::locale::classic](../../locale/locale/classic.html)(), the passed [std::locale](../../locale/locale.html "cpp/locale/locale") object, and [std::locale::global](../../locale/locale/global.html)()) [imbued](../../io/ios_base/imbue.html "cpp/io/ios base/imbue") and copying os.str() to the output buffer with additional padding and adjustments as per format specifiers. 
...
---|---|---|---  
[LWG 4124](https://cplusplus.github.io/LWG/issue4124) | C++20  | 1\. chrono-spec could not be omitted  
2\. `zoned_time` could not be formatted  
with resolution coarser than seconds  | 1\. it can be omitted  
2\. can be formatted with  
those resolutions   
