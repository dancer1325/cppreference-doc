...
const [std::chrono::zoned_time](../zoned_time.html)<Duration, TimeZonePtr>& tp ); |  |  (since C++20)  
| |   
  
Outputs tp to the stream os, as if by [std::format](../../utility/format/format.html)(os.getloc(), fmt, tp), where `fmt` is "{:L%F %T %Z}" if `CharT` is char, or L"{:L%F %T %Z}" if `CharT` is wchar_t. 

## Contents

  * [1 Parameters](operator_ltlt.html#Parameters)
  * [2 Return value](operator_ltlt.html#Return_value)
  * [3 Example](operator_ltlt.html#Example)
  * [4 Defect reports](operator_ltlt.html#Defect_reports)
