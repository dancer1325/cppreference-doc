[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/chrono/clock_time_from_stream&action=edit)
...
[leap_second_info](leap_second_info.html "cpp/chrono/utc clock/leap second info")(C++20)  
...
Member functions  
---  
[utc_clock::now](now.html "cpp/chrono/utc clock/now")  
[utc_clock::to_sys](to_sys.html "cpp/chrono/utc clock/to sys")  
[utc_clock::from_sys](from_sys.html "cpp/chrono/utc clock/from sys")  
Time point I/O  
[operator<<](operator_ltlt.html "cpp/chrono/utc clock/operator ltlt")  
[formatter<std::chrono::utc_time>](formatter.html "cpp/chrono/utc clock/formatter")  
**from_stream**  
...
[std::chrono::utc_time](../utc_clock.html)<Duration>& tp,   
...
`%S`  
`%_N_ S`  
`%OS` |  Parses the second as a decimal number. The width _N_ specifies the maximum number of characters to read. The default width is 2 if `Duration` is convertible to [std::chrono::seconds](../duration.html "cpp/chrono/duration"), otherwise it is determined by the decimal precision of `Duration` and the field is interpreted as a long double in a fixed format, with the decimal point character determined by the locale (if encountered). Leading zeroes are permitted but not required. The modified command `%OS` interprets the locale's alternative representation.   
