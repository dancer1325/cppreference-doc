...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year_month_day/ok&action=edit&section=1 "Edit section: Return value")] Return value
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year_month_day/ok&action=edit&section=2 "Edit section: Possible implementation")] Possible implementation
    
    
    constexpr bool std::chrono::year_month_day::ok() const noexcept
    {
        return year().ok() && month().ok() && day().ok() &&
            day() <= (year()/month()/[std::chrono::last](../last_spec.html)).day();
    }  
  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year_month_day/ok&action=edit&section=3 "Edit section: Example")] Example
...
Run this code
    ...
    int main()
    {
        constexpr auto ymd1 {[std::chrono::day](../day.html)(1)/[std::chrono::July](../month.html)/2020};
        static_assert(ymd1.ok());
     
        constexpr auto ymd2 {[std::chrono::year](../year.html)(2020)/7/42};
        static_assert(not ymd2.ok());
     
        constexpr auto ymd3 {[std::chrono::February](../month.html)/29/2020}; // ok, leap year
        static_assert(ymd3.ok());
     
        constexpr auto ymd4 = ymd3 + [std::chrono::years](../duration.html){1}; // bad, not a leap year
        static_assert(ymd4 == [std::chrono::February](../month.html)/29/2021 and not ymd4.ok());
     
        // to fix the bad date we may want to snap to the last day of the month:
        if constexpr (!ymd4.ok())
        {
            constexpr auto ymd = ymd4.year()/ymd4.month()/[std::chrono::last](../last_spec.html);
            static_assert(ymd == [std::chrono::February](../month.html)/28/2021 and ymd.ok());
        }
     
        // or we may want to overflow to the next month:
        if constexpr (!ymd4.ok())
        {
            constexpr auto st = [std::chrono::sys_time](../system_clock.html)<[std::chrono::days](../duration.html)>{ymd4};
            constexpr auto ymd = [std::chrono::year_month_day](../year_month_day.html){st};
            static_assert(ymd == [std::chrono::March](../month.html)/1/2021 and ymd.ok());
        }
    }
