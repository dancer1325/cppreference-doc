...
**year_month_day::operator sys_days year_month_day::operator local_days**  
...
---|---|---  
constexpr explicit operator [std::chrono::local_days](../local_t.html)() const noexcept; |  (2)  |  (since C++20)  
| |   
  
Converts *this to a [std::chrono::time_point](../time_point.html "cpp/chrono/time point") representing the same date as this [`year_month_day`](../year_month_day.html "cpp/chrono/year month day"). 

1) If ok() is true, the return value holds a count of days from the [std::chrono::system_clock](../system_clock.html "cpp/chrono/system clock") epoch (1970-01-01) to *this. The result is negative if *this represent a date prior to it.
...
A std::chrono::sys_days in the range [[std::chrono::days](../duration.html){-12687428}, [std::chrono::days](../duration.html){11248737}], when converted to [`year_month_day`](../year_month_day.html "cpp/chrono/year month day") and back, yields the same value.

2) Same as (1) but returns local_days instead. Equivalent to return local_days(sys_days(*this).time_since_epoch());.

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year_month_day/operator_days&action=edit&section=1 "Edit section: Notes")] Notes

Converting to std::chrono::sys_days and back can be used to normalize a [`year_month_day`](../year_month_day.html "cpp/chrono/year month day") that contains an invalid day but a valid year and month: 
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/chrono/year_month_day/operator_days&action=edit&section=2 "Edit section: Example")] Example
...
Run this code
    ...
    {
        using namespace std::chrono;
        const auto today = sys_days{[std::chrono::floor](../duration/floor.html)<days>(system_clock::now())};
        for (const year_month_day ymd : {{November/15/2020}, {November/15/2120}, today})
        {
            [std::cout](../../io/cout.html) << ymd;
            const auto delta = (sys_days{ymd} - today).count();
            (delta < 0) ? [std::cout](../../io/cout.html) << " was " << -delta << " day(s) ago\n" :
            (delta > 0) ? [std::cout](../../io/cout.html) << " is " << delta << " day(s) from now\n"
                        : [std::cout](../../io/cout.html) << " is today!\n";
        }
    }
