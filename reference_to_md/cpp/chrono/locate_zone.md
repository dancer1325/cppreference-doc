...
---|---|---  
const [std::chrono::time_zone](time_zone.html)* locate_zone( [std::string_view](../string/basic_string_view.html) tz_name ); |  |  (since C++20)  
| |   
  
Convenience function for locating a time zone in the [time zone database](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones "enwiki:List of tz database time zones"). Equivalent to [std::chrono::get_tzdb](tzdb_functions.html)().locate_zone(tz_name). 

## Contents

  * [1 Parameters](locate_zone.html#Parameters)
  * [2 Exceptions](locate_zone.html#Exceptions)
  * [3 Notes](locate_zone.html#Notes)
  * [4 Example](locate_zone.html#Example)
  * [5 See also](locate_zone.html#See_also)
    ...
    int main()
    {
        try
        {
            for (const auto zone : {"Atlantic/Bermuda", "Alcatraz"})
                [std::cout](../io/cout.html) << std::chrono::locate_zone(zone)->name() << '\n';
        }
