...
Obtains a pointer to a std::chrono::time_zone in this database that represents the time zone designated by tz_name. If p is the returned pointer, then either p->name() == tz_name or that there is a [std::chrono::time_zone_link](../time_zone_link.html "cpp/chrono/time zone link") l in this database such that p->name() == l.target() && l.name() == tz_name. 

## Contents

  * [1 Parameters](locate_zone.html#Parameters)
  * [2 Return value](locate_zone.html#Return_value)
  * [3 Exceptions](locate_zone.html#Exceptions)
