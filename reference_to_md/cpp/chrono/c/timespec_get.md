...
---|---|---  
int timespec_get( [std::timespec](timespec.html)* ts, int base ); |  (1)  |  (since C++17)  
#define TIME_UTC /* implementation-defined */ |  (2)  |  (since C++17)  
| |   
  
1) Modifies the [std::timespec](timespec.html "cpp/chrono/c/timespec") object pointed to by ts to hold the current calendar time in the time base base.
...
## Contents
  ...
  * [5 See also](timespec_get.html#See_also)
...
Run this code
    ...
    int main()
    {
        [std::timespec](timespec.html) ts;
        std::timespec_get(&ts, TIME_UTC);
        char buf[100];
        [std::strftime](strftime.html)(buf, sizeof buf, "%D %T", [std::gmtime](gmtime.html)(&ts.tv_sec));
        [std::cout](../../io/cout.html) << "Current time: " << buf << '.' << ts.tv_nsec << " UTC\n";
    }
