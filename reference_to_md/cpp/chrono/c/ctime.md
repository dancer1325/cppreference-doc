...
---|---|---  
char* ctime( const [std::time_t](time_t.html)* time ); |  |   
| |   
  
Converts given time since epoch to a calendar local time and then to a textual representation, as if by calling [std::asctime](asctime.html)([std::localtime](localtime.html)(time)). The resulting string has the following format: 
...
## Contents
  ...
  * [5 See also](ctime.html#See_also)
...
Run this code
    ...
    int main()
    {
        [std::time_t](time_t.html) result = [std::time](time.html)(nullptr);
        [std::cout](../../io/cout.html) << std::ctime(&result);
     
        char buffer[32];
        [std::strncpy](../../string/byte/strncpy.html)(buffer, std::ctime(&result), 26);
        [assert](../../error/assert.html)('\n' == buffer[[std::strlen](../../string/byte/strlen.html)(buffer) - 1]);
