...
---|---|---  
[std::clock_t](clock_t.html) clock(); |  |   
| |   
  
Returns the approximate processor time used by the process since the beginning of an implementation-defined era related to the program's execution. To convert result value to seconds divide it by [CLOCKS_PER_SEC](CLOCKS_PER_SEC.html "cpp/chrono/c/CLOCKS PER SEC"). 
...
## Contents
  ...
  * [2 Exceptions](clock.html#Exceptions)
  * [3 Notes](clock.html#Notes)
  * [4 Example](clock.html#Example)
...
Processor time used by the program so far. 
  ...
  * If the processor time used is not available, returns ([std::clock_t](clock_t.html))(-1). 
  * If the value of the processor time used cannot be represented by [std::clock_t](clock_t.html "cpp/chrono/c/clock t"), returns an unspecified value. 
...
Run this code
    ...
    int main()
    {
        const [std::clock_t](clock_t.html) c_start = std::clock();
        auto t_start = [std::chrono::high_resolution_clock::now](../high_resolution_clock/now.html)();
        [std::thread](../../thread/thread.html) t1(f);
        [std::thread](../../thread/thread.html) t2(f); // f() is called on two threads
        t1.join();
        t2.join();
        const [std::clock_t](clock_t.html) c_end = std::clock();
        const auto t_end = [std::chrono::high_resolution_clock::now](../high_resolution_clock/now.html)();
     
        [std::cout](../../io/cout.html) << [std::fixed](../../io/manip/fixed.html) << [std::setprecision](../../io/manip/setprecision.html)(2) << "CPU time used: "
                  << 1000.0 * (c_end - c_start) / [CLOCKS_PER_SEC](CLOCKS_PER_SEC.html) << "ms\n"
                  << "Wall clock time passed: "
                  << [std::chrono::duration](../duration.html)<double, [std::milli](../../numeric/ratio/ratio.html)>(t_end - t_start) << '\n';
    }
