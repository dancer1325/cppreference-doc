...
Returns an [iterator](iterator.html "cpp/ranges/chunk by view/iterator") to the first element of the [`chunk_by_view`](../chunk_by_view.html "cpp/ranges/chunk by view"). 

Equivalent to: 
    
    
    [ranges::iterator_t](../iterator_t.html)<V> iter;
     
    if (begin_.has_value())
        iter = begin_.value();
    else
    {
        iter = /*find_next*/([ranges::begin](../begin.html)(base()));
        begin_ = iter; // caching
    }
     
    return /*iterator*/(*this, [ranges::begin](../begin.html)(base()), iter);

The behavior is undefined if the underlying predicate `_[pred_](../chunk_by_view.html#pred "cpp/ranges/chunk by view")_` does not contain a value. 
