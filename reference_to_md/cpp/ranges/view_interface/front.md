...
requires [ranges::forward_range](../forward_range.html)<const D>; |  (2)  |  (since C++20)  
| |   
...
1) Let `derived` be static_cast<D&>(*this). Equivalent to return *[ranges::begin](../begin.html)(derived);. The behavior is undefined if [`empty()`](empty.html "cpp/ranges/view interface/empty") is true (i.e. the beginning iterator compares equal to the sentinel), even if the iterator obtained in the same way is dereferenceable.
...
A notable exception is [std::ranges::basic_istream_view](../basic_istream_view.html "cpp/ranges/basic istream view"). For it never satisfies [`forward_range`](../forward_range.html "cpp/ranges/forward range"), the view cannot use the inherited `front()`. 
