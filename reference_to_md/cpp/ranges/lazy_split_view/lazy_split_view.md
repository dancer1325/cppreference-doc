...
constexpr explicit lazy_split_view( R&& r, [ranges::range_value_t](../range_size_t.html)<R> e ); |  (3)  |  (since C++20)  
| |   
  
Constructs a `lazy_split_view`. 

1) Default constructor. [Value-initializes](../../language/value_initialization.html "cpp/language/value initialization") the underlying view `_[base_](../lazy_split_view.html#base "cpp/ranges/lazy split view")_` and the delimiter `_[pattern_](../lazy_split_view.html#pattern "cpp/ranges/lazy split view")_`.

2) Initializes the underlying view `_base__` with std::move(base) and the delimiter `_pattern__` with std::move(pattern).
...
([P2711R1](https://wg21.link/P2711R1))  | C++20  | the multi-parameter constructor ([2](lazy_split_view.html#Version_2)) was not explicit  | made explicit 
