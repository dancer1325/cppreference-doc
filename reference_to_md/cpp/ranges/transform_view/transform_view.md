...
---|---|---  
constexpr explicit transform_view( V base, F fun ); |  (2)  |  (since C++20)  
| |   
  
Constructs a `transform_view`. 

1) Default constructor. [Value-initializes](../../language/value_initialization.html "cpp/language/value initialization") the underlying view `_[base_](../transform_view.html#base "cpp/ranges/transform view")_` and the transformation function `_[fun_](../transform_view.html#fun "cpp/ranges/transform view")_`.
...
Run this code
    ...
    int main()
    {
        [std::cout](../../io/cout.html) << [std::setprecision](../../io/manip/setprecision.html)(15) << [std::fixed](../../io/manip/fixed.html);
        auto atan1term{std::ranges::[views::transform](../transform_view.html)
        (
            [](int n) { return ((n % 2) ? -1 : 1) * 1.0 / (2 * n + 1); }
        )};
        for (const int iterations : {1, 2, 3, 4, 5, 10, 100, 1000, 1'000'000})
        {
            auto seq{std::ranges::[views::iota](../iota_view.html)(0, iterations) bitor atan1term};
            const double accum{*std::[ranges::fold_left_first](../../algorithm/ranges/fold_left_first.html)(seq, [std::plus](../../utility/functional/plus.html){})};
            [std::cout](../../io/cout.html) << "π ≈ " << 4 * accum << " (iterations: " << iterations << ")\n";
        }
        [std::cout](../../io/cout.html) << "π ≈ " << [std::numbers::pi](../../numeric/constants.html) << " (std::numbers::pi)\n";
    }
...
[P2325R3](https://wg21.link/P2325R3) | C++20  | if `F` is not [`default_initializable`](../../concepts/default_initializable.html "cpp/concepts/default initializable"), the default constructor  
constructs a `transform_view` which does not contain an `F` | the `transform_view` is also  
