...
---|---|---  
template< class T >  
concept approximately_sized_range = [ranges::range](range.html)<T> &&  
requires(T& t) {  
ranges::reserve_hint(t);  
}; |  |  (since C++26)  
| |   
  
The `approximately_sized_range` concept specifies the requirements of a [`range`](range.html "cpp/ranges/range") type that can estimate its size in constant time with the `reserve_hint` function. 

## Contents

  * [1 Semantic requirements](approximately_sized_range.html#Semantic_requirements)
  * [2 Notes](approximately_sized_range.html#Notes)
  * [3 Example](approximately_sized_range.html#Example)
  * [4 See also](approximately_sized_range.html#See_also)
...
Given an lvalue t of type [std::remove_reference_t](../types/remove_reference.html)<T>, `T` models `approximately_sized_range` only if 
  ...
  * has a value that is not negative and is representable in [ranges::range_difference_t](range_size_t.html)<T>, and 



  * if [ranges::iterator_t](iterator_t.html)<T> models [`forward_iterator`](../iterator/forward_iterator.html "cpp/iterator/forward iterator"), ranges::reserve_hint(t) is well-defined regardless of the evaluation of [ranges::begin](begin.html)(t) (in other words, a single-pass approximately sized range may support a call to `reserve_hint` only before the first call to `begin`, but a forward range must support size at all times). 
