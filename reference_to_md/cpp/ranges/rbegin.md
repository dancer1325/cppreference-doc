...
template< class T >  
requires /* see below */  
constexpr [std::input_or_output_iterator](../iterator/input_or_output_iterator.html) auto rbegin( T&& t ); |  |  (since C++20)  
| |   
...
If `T` is an array type and [std::remove_all_extents_t](../types/remove_all_extents.html)<[std::remove_reference_t](../types/remove_reference.html)<T>> is incomplete, then the call to `ranges::rbegin` is ill-formed, no diagnostic required. 

If the argument is an lvalue or [ranges::enable_borrowed_range](borrowed_range.html)<[std::remove_cv_t](../types/remove_cv.html)<T>> is true, then a call to `ranges::rbegin` is [expression-equivalent](../language/expressions.html#Expression-equivalence "cpp/language/expressions") to: 

  1. [`_decay-copy_`](../standard_library/decay-copy.html "cpp/standard library/decay-copy")(t.rbegin())(until C++23)auto(t.rbegin())(since C++23), if that expression is valid and its type models [std::input_or_output_iterator](../iterator/input_or_output_iterator.html "cpp/iterator/input or output iterator"). 
  2. Otherwise, [`_decay-copy_`](../standard_library/decay-copy.html "cpp/standard library/decay-copy")(rbegin(t))(until C++23)auto(rbegin(t))(since C++23), if `T` is a class or enumeration type, that expression is valid and its type models [std::input_or_output_iterator](../iterator/input_or_output_iterator.html "cpp/iterator/input or output iterator"), where the meaning of `rbegin` is established as if by performing [argument-dependent lookup](../language/adl.html "cpp/language/adl") only. 
  3. Otherwise, [std::make_reverse_iterator](../iterator/make_reverse_iterator.html)([ranges::end](end.html)(t)) if both [ranges::begin](begin.html)(t) and [ranges::end](end.html)(t) are valid expressions, have the same type, and that type models [std::bidirectional_iterator](../iterator/bidirectional_iterator.html "cpp/iterator/bidirectional iterator"). 



In all other cases, a call to `ranges::rbegin` is ill-formed, which can result in [substitution failure](../language/sfinae.html "cpp/language/sfinae") when ranges::rbegin(t) appears in the immediate context of a template instantiation. 

## Contents

  * [1 Customization point objects](rbegin.html#Customization_point_objects)
  * [2 Notes](rbegin.html#Notes)
  * [3 Example](rbegin.html#Example)
  * [4 Defect reports](rbegin.html#Defect_reports)
...
Run this code
    ...
    int main()
    {
        ...
        [std::cout](../io/cout.html) << *vi << '\n';
        *vi = 42; // OK
     
        int a[] = {-5, 10, 15};
        auto ai = std::ranges::rbegin(a);
        [std::cout](../io/cout.html) << *ai << '\n';
        *ai = 42; // OK
     
        // auto x_x = std::ranges::rbegin(std::vector{6, 6, 6});
        // ill-formed: the argument is an rvalue (see Notes ↑)
     
        auto si = std::ranges::rbegin([std::span](../container/span.html){a}); // OK
        static_assert(std::[ranges::enable_borrowed_range](borrowed_range.html)<
            [std::remove_cv_t](../types/remove_cv.html)<decltype([std::span](../container/span.html){a})>>);
        *si = 42; // OK
    }
