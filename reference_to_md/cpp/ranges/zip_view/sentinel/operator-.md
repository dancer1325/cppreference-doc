...
friend constexpr  
[std::common_type_t](../../../types/common_type.html)<[ranges::range_difference_t](../../range_size_t.html)</*maybe-const*/<OtherConst, Views>>...>  
...
Let `_DIST_(x, y, i)` be a distance calculated by expression equivalent to std::get<i>(x.current_) - std::get<i>(y.end_) for some integer `i`. 

1) the value with the smallest absolute value among `_DIST_(x, y, i)` of all `i` in range `0 ≤ i < sizeof...(Views)`
...
Run this code
    ...
    int main()
    {
        ...
        auto z = {1.1, 2.2};
        auto w = [std::list](../../../container/list.html){1, 2, 3};
     
        auto p = std::[views::zip](../../zip_view.html)(x, y, z);
        [assert](../../../error/assert.html)(p.begin() - p.end() == +2);
        [assert](../../../error/assert.html)(p.end() - p.begin() == -2);
     
        [[maybe_unused]]
        auto q = std::[views::zip](../../zip_view.html)(x, y, w);
     
