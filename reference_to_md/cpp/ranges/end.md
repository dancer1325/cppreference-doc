...
template< class T >  
requires /* see below */  
constexpr [std::sentinel_for](../iterator/sentinel_for.html)<[ranges::iterator_t](iterator_t.html)<T>> auto end( T&& t ); |  |  (since C++20)  
| |   
...
If the argument is an lvalue or [ranges::enable_borrowed_range](borrowed_range.html)<[std::remove_cv_t](../types/remove_cv.html)<T>> is true, then a call to `ranges::end` is [expression-equivalent](../language/expressions.html#Expression-equivalence "cpp/language/expressions") to: 

  1. t + [std::extent_v](../types/extent.html)<T> if t has an array type of known bound. 
     * If [std::remove_all_extents_t](../types/remove_all_extents.html)<[std::remove_reference_t](../types/remove_reference.html)<T>> is incomplete, then the call to `ranges::end` is ill-formed, [no diagnostic required](../ndr.html "cpp/language/ndr"). 
  2. Otherwise, [`_decay-copy_`](../standard_library/decay-copy.html "cpp/standard library/decay-copy")(t.end())(until C++23)auto(t.end())(since C++23), if that expression is valid, and its type models [std::sentinel_for](../iterator/sentinel_for.html)<[ranges::iterator_t](iterator_t.html)<T>>. 
  3. Otherwise, [`_decay-copy_`](../standard_library/decay-copy.html "cpp/standard library/decay-copy")(end(t))(until C++23)auto(end(t))(since C++23), if `T` is a class or enumeration type, that expression is valid and its converted type models [std::sentinel_for](../iterator/sentinel_for.html)<[ranges::iterator_t](iterator_t.html)<T>>, where the meaning of `end` is established as if by performing [argument-dependent lookup](../language/adl.html "cpp/language/adl") only. 



In all other cases, a call to `ranges::end` is ill-formed, which can result in [substitution failure](../language/sfinae.html "cpp/language/sfinae") when the call to `ranges::end` appears in the immediate context of a template instantiation. 

## Contents

  * [1 Customization point objects](end.html#Customization_point_objects)
  * [2 Notes](end.html#Notes)
...
Run this code
    ...
    int main()
    {
        [std::vector](../container/vector.html)<int> vec{3, 1, 4};
        if (std::[ranges::find](../algorithm/ranges/find.html)(vec, 5) != std::ranges::end(vec))
            [std::cout](../io/cout.html) << "found a 5 in vector vec!\n";
     
        int arr[]{5, 10, 15};
        if (std::[ranges::find](../algorithm/ranges/find.html)(arr, 5) != std::ranges::end(arr))
            [std::cout](../io/cout.html) << "found a 5 in array arr!\n";
    }
