...
Defines the initializer which can be used to initialize [std::atomic_flag](atomic_flag.html "cpp/atomic/atomic flag") to clear (false) state in the form [std::atomic_flag](atomic_flag.html) v = ATOMIC_FLAG_INIT;. It is unspecified if it can be used with other initialization contexts. 

If the flag has is a [complete object](../language/objects.html#Subobjects "cpp/language/object") with [static storage duration](../language/storage_duration.html#Static_storage_duration "cpp/language/storage duration"), this [initialization is static](../language/initialization.html#Static_initialization "cpp/language/initialization"). 

This is the only way to initialize [std::atomic_flag](atomic_flag.html "cpp/atomic/atomic flag") to a definite value: the value held after any other initialization is unspecified.  | (until C++20)  
---|---  
...
Run this code
    ...
    #include <atomic>
     
    [std::atomic_flag](atomic_flag.html) static_flag = ATOMIC_FLAG_INIT; // static initialization,
    // guaranteed to be available during dynamic initialization of static objects.
     
    int main()
    {
        [std::atomic_flag](atomic_flag.html) automatic_flag = ATOMIC_FLAG_INIT; // guaranteed to work
...
---|---|---|---  
[LWG 2159](https://cplusplus.github.io/LWG/issue2159) | C++11  | it was unclear whether `ATOMIC_FLAG_INIT`  
can be used with other initialization contexts  | other usages are  
not guaranteed   
