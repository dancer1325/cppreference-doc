...
**atomic_ref::compare_exchange_strong atomic_ref::compare_exchange_weak**  
...
[std::memory_order](../memory_order.html) failure ) const noexcept; |  (1) | (constexpr since C++26)  
---|---|---  
bool compare_exchange_weak  
( value_type& expected, value_type desired,  
[std::memory_order](../memory_order.html) order =  
[std::memory_order_seq_cst](../memory_order.html) ) const noexcept; |  (2) | (constexpr since C++26)  
bool compare_exchange_strong  
( value_type& expected, value_type desired,  
[std::memory_order](../memory_order.html) success,  
[std::memory_order](../memory_order.html) failure ) const noexcept; |  (3) | (constexpr since C++26)  
bool compare_exchange_strong  
( value_type& expected, value_type desired,  
[std::memory_order](../memory_order.html) order =  
[std::memory_order_seq_cst](../memory_order.html) ) const noexcept; |  (4) | (constexpr since C++26)  
| |   
  
Atomically compares the [value representation](../../language/objects.html "cpp/language/object") of the referenced object with that of expected, and if those are bitwise-equal, replaces the former with desired (performs a read-modify-write operation). Otherwise, loads the actual value stored in the referenced object into expected (performs a load operation). 

1,3) The memory models for the read-modify-write and load operations are success and failure respectively.

2,4) order is used for both read-modify-write and load operations, except that [std::memory_order_acquire](../memory_order.html) and [std::memory_order_relaxed](../memory_order.html) are used for the load operation if order is [std::memory_order_acq_rel](../memory_order.html) or [std::memory_order_release](../memory_order.html) respectively.

These overloads participate in overload resolution only if [std::is_const_v](../../types/is_const.html)<T> is false. 

If failure is not [std::memory_order_relaxed](../memory_order.html), [std::memory_order_consume](../memory_order.html), [std::memory_order_acquire](../memory_order.html) or [std::memory_order_seq_cst](../memory_order.html), the behavior is undefined. 
...
## Contents
  ...
  * [4 Defect reports](compare_exchange.html#Defect_reports)
  * [5 Example](compare_exchange.html#Example)
