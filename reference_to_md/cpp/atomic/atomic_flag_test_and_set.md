...
---|---|---  
bool atomic_flag_test_and_set( volatile [std::atomic_flag](atomic_flag.html)* obj ) noexcept; |  (1)  |  (since C++11)  
bool atomic_flag_test_and_set( [std::atomic_flag](atomic_flag.html)* obj ) noexcept; |  (2)  |  (since C++11)  
bool atomic_flag_test_and_set_explicit( volatile [std::atomic_flag](atomic_flag.html)* obj,   
[std::memory_order](memory_order.html) order ) noexcept; |  (3)  |  (since C++11)  
bool atomic_flag_test_and_set_explicit( [std::atomic_flag](atomic_flag.html)* obj,   
[std::memory_order](memory_order.html) order ) noexcept; |  (4)  |  (since C++11)  
| |   
  
Atomically changes the state of a [std::atomic_flag](atomic_flag.html "cpp/atomic/atomic flag") pointed to by obj to set (true) and returns the value it held before. 
...
## Contents
  ...
  * [3 Notes](atomic_flag_test_and_set.html#Notes)
  * [4 Example](atomic_flag_test_and_set.html#Example)
  * [5 See also](atomic_flag_test_and_set.html#See_also)
...
Run this code
    ...
    #include <vector>
     
    [std::atomic_flag](atomic_flag.html) lock = [ATOMIC_FLAG_INIT](ATOMIC_FLAG_INIT.html);
     
    void f(int n)
    {
        for (int cnt = 0; cnt < 100; ++cnt)
        {
            while (std::atomic_flag_test_and_set_explicit(&lock, [std::memory_order_acquire](memory_order.html)))
                ; // spin until the lock is acquired
            [std::cout](../io/cout.html) << "Output from thread " << n << '\n';
            [std::atomic_flag_clear_explicit](atomic_flag_clear.html)(&lock, [std::memory_order_release](memory_order.html));
        }
    }
     
    int main()
