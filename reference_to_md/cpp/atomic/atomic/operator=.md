...
1,2) Atomically assigns desired to the atomic variable. Equivalent to store(desired).  It is deprecated if [std::atomic](../atomic.html)<T>::is_always_lock_free is false and overload (2) participates in overload resolution.  | (since C++20)  
---|---  
  
3,4) Atomic variables are not [CopyAssignable](../../named_req/CopyAssignable.html "cpp/named req/CopyAssignable").
