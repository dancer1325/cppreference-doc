...
[atomic::compare_exchange_strongatomic::compare_exchange_weak](compare_exchange.html "cpp/atomic/atomic/compare exchange")  
...
---|---|---  
T fetch_add( T arg, [std::memory_order](../memory_order.html) order =  
[std::memory_order_seq_cst](../memory_order.html) ) noexcept; |  (1)  |  (since C++11)  
T fetch_add( T arg, [std::memory_order](../memory_order.html) order =  
[std::memory_order_seq_cst](../memory_order.html) ) volatile noexcept; |  (2)  |  (since C++11)  
member only of `atomic<T*>` partial specialization |  |   
T* fetch_add( [std::ptrdiff_t](../../types/ptrdiff_t.html) arg,   
...
1,2) For signed integral types, arithmetic is defined to use two’s complement representation. There are no undefined results.  For floating-point types, the [floating-point environment](../../numeric/fenv.html "cpp/numeric/fenv") in effect may be different from the calling thread's floating-point environment. The operation need not conform to the corresponding [std::numeric_limits](../../types/numeric_limits.html "cpp/types/numeric limits") traits but is encouraged to do so. If the result is not a representable value for its type, the result is unspecified but the operation otherwise has no undefined behavior.  | (since C++20)  
---|---  
...
It is deprecated if [std::atomic](../atomic.html)<T>::is_always_lock_free is false and overload (2) or (4) participates in overload resolution.  | (since C++20)  
---|---  
  
## Contents

  * [1 Parameters](fetch_add.html#Parameters)
  * [2 Return value](fetch_add.html#Return_value)
  * [3 Example](fetch_add.html#Example)
  * [4 Defect reports](fetch_add.html#Defect_reports)
  * [5 See also](fetch_add.html#See_also)

  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/atomic/atomic/fetch_add&action=edit&section=1 "Edit section: Parameters")] Parameters

arg  |  \-  |  the other argument of arithmetic addition   
---|---|---  
order  |  \-  |  memory order constraints to enforce   
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/atomic/atomic/fetch_add&action=edit&section=2 "Edit section: Return value")] Return value
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/atomic/atomic/fetch_add&action=edit&section=3 "Edit section: Example")] Example
...
Run this code
    ...
    #include <thread>
     
    [std::atomic](../atomic.html)<long long> data{10};
    [std::array](../../container/array.html)<long long, 5> return_values{};
     
    void do_work(int thread_num)
    {
        long long val = data.fetch_add(1, [std::memory_order_relaxed](../memory_order.html));
    ...
    {
        {
            [std::jthread](../../thread/jthread.html) th0{do_work, 0};
            [std::jthread](../../thread/jthread.html) th1{do_work, 1};
            [std::jthread](../../thread/jthread.html) th2{do_work, 2};
            [std::jthread](../../thread/jthread.html) th3{do_work, 3};
            [std::jthread](../../thread/jthread.html) th4{do_work, 4};
        }
     
        [std::cout](../../io/cout.html) << "Result : " << data << '\n';
     
        for (long long val : return_values)
            [std::cout](../../io/cout.html) << "Seen return value : " << val << [std::endl](../../io/manip/endl.html);
    }
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/atomic/atomic/fetch_add&action=edit&section=4 "Edit section: Defect reports")] Defect reports
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/atomic/atomic/fetch_add&action=edit&section=5 "Edit section: See also")] See also

[ atomic_fetch_addatomic_fetch_add_explicit](../atomic_fetch_add.html "cpp/atomic/atomic fetch add")(C++11)(C++11) |  adds a non-atomic value to an atomic object and obtains the previous value of the atomic   
