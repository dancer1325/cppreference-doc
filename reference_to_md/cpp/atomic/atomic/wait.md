[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/atomic/atomic/wait&action=edit)
...
[std::memory_order_seq_cst](../memory_order.html) ) const volatile noexcept; |  (2)  |  (since C++20)  
| |   
  
Performs atomic waiting operations. Behaves as if it repeatedly performs the following steps: 

  * Compare the [value representation](../../language/objects.html "cpp/language/object") of this->load(order) with that of old. 
...
If order is not [std::memory_order_relaxed](../memory_order.html), [std::memory_order_consume](../memory_order.html), [std::memory_order_acquire](../memory_order.html) or [std::memory_order_seq_cst](../memory_order.html), the behavior is undefined. 

## Contents

  * [1 Parameters](wait.html#Parameters)
  * [2 Notes](wait.html#Notes)
  * [3 Example](wait.html#Example)
  * [4 See also](wait.html#See_also)
...
Run this code
    ...
    int main()
    {
        [std::atomic](../atomic.html)<bool> all_tasks_completed{false};
        [std::atomic](../atomic.html)<unsigned> completion_count{};
        [std::future](../../thread/future.html)<void> task_futures[16];
        [std::atomic](../atomic.html)<unsigned> outstanding_task_count{16};
     
        // Spawn several tasks which take different amounts of
        // time, then decrement the outstanding task count.
        for ([std::future](../../thread/future.html)<void>& task_future : task_futures)
            task_future = [std::async](../../thread/async.html)([&]
            {
                // This sleep represents doing real work...
                [std::this_thread::sleep_for](../../thread/sleep_for.html)(50ms);
     
                ++completion_count;
    ...
     
        [std::cout](../../io/cout.html) << "Tasks completed = " << completion_count.load() << '\n';
    }
