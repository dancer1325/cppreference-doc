...
## Contents
  ...
  * [3 Example](exchange.html#Example)
...
Run this code
    ...
    int main()
    {
        ...
        constexpr int sum{5};
     
        [std::atomic](../atomic.html)<int> atom{0};
        [std::atomic](../atomic.html)<int> counter{0};
     
        auto increment_to_sum = [&](const int id)
        {
            for (int next = 0; next < sum;)
            {
                ...
                [std::osyncstream](../../io/basic_osyncstream.html)([std::cout](../../io/cout.html))
                    << "Thread #" << id << " (id=" << [std::this_thread::get_id](../../thread/get_id.html)()
                    << ") wrote " << next << " replacing the old value "
                    << current << ".\n";
                next = [std::max](../../algorithm/max.html)(current, next) + 1;
            }
        };
     
        [std::vector](../../container/vector.html)<[std::thread](../../thread/thread.html)> v;
        for ([std::size_t](../../types/size_t.html) i = 0; i < thread_count; ++i)
            v.emplace_back(increment_to_sum, i);
     
        for (auto& tr : v)
            tr.join();
     
        [std::cout](../../io/cout.html) << thread_count << " threads take "
