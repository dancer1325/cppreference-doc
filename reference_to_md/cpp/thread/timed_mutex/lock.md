...
If `lock` is called by a thread that already owns the `mutex`, the behavior is undefined: for example, the program _may_ deadlock. An implementation that can detect the invalid usage is encouraged to throw a [std::system_error](../../error/system_error.html "cpp/error/system error") with error condition `resource_deadlock_would_occur` instead of deadlocking. 

Prior [unlock()](unlock.html "cpp/thread/timed mutex/unlock") operations on the same mutex _synchronize-with_ (as defined in [std::memory_order](../../atomic/memory_order.html "cpp/atomic/memory order")) this operation. 
