...
Member functions  
---  
[counting_semaphore::counting_semaphore](counting_semaphore.html "cpp/thread/counting semaphore/counting semaphore")  
[counting_semaphore::~counting_semaphore](~counting_semaphore.html "cpp/thread/counting semaphore/~counting semaphore")  
Operations  
[counting_semaphore::release](release.html "cpp/thread/counting semaphore/release")  
**counting_semaphore::acquire**  
[counting_semaphore::try_acquire](try_acquire.html "cpp/thread/counting semaphore/try acquire")  
[counting_semaphore::try_acquire_for](try_acquire_for.html "cpp/thread/counting semaphore/try acquire for")  
[counting_semaphore::try_acquire_until](try_acquire_until.html "cpp/thread/counting semaphore/try acquire until")  
Constants  
...
## Contents
  ...
  * [1 Preconditions](acquire.html#Preconditions)
  * [2 Parameters](acquire.html#Parameters)
  * [3 Exceptions](acquire.html#Exceptions)
  * [4 Example](acquire.html#Example)
  * [5 See also](acquire.html#See_also)

  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/counting_semaphore/acquire&action=edit&section=1 "Edit section: Preconditions")] Preconditions

(none) 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/counting_semaphore/acquire&action=edit&section=2 "Edit section: Parameters")] Parameters

(none) 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/counting_semaphore/acquire&action=edit&section=3 "Edit section: Exceptions")] Exceptions

May throw [std::system_error](../../error/system_error.html "cpp/error/system error"). 

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/counting_semaphore/acquire&action=edit&section=4 "Edit section: Example")] Example

The example visualizes a concurrent work of several randomized threads when no more than N (N is the semaphore _desired_ value) of the thread-functions are active, while the other might wait on the semaphore.
...
Run this code
    ...
     
    constexpr [std::size_t](../../types/size_t.html) max_threads{10U}; // change and see the effect
    constexpr [std::ptrdiff_t](../../types/ptrdiff_t.html) max_sema_threads{3}; // {1} for binary semaphore
    [std::counting_semaphore](../counting_semaphore.html) semaphore{max_sema_threads};
    constexpr auto time_tick{10ms};
     
    unsigned rnd()
    {
        static [std::uniform_int_distribution](../../numeric/random/uniform_int_distribution.html)<unsigned> distribution{2U, 9U}; // [delays]
        static [std::random_device](../../numeric/random/random_device.html) engine;
        static [std::mt19937](../../numeric/random/mersenne_twister_engine.html) noise{engine()};
        return distribution(noise);
    }
     
    class alignas([std::hardware_destructive_interference_size](../hardware_destructive_interference_size.html)) Guide
    {
        inline static [std::mutex](../mutex.html) cout_mutex;
        inline static [std::chrono::time_point](../../chrono/time_point.html)<[std::chrono::high_resolution_clock](../../chrono/high_resolution_clock.html)> started_at;
        unsigned delay{rnd()}, occupy{rnd()}, wait_on_sema{};
     
    public:
        static void start_time() { started_at = [std::chrono::high_resolution_clock::now](../../chrono/high_resolution_clock/now.html)(); }
     
        void initial_delay() { [std::this_thread::sleep_for](../sleep_for.html)(delay * time_tick); }
     
        void occupy_sema()
        {
            wait_on_sema =
                static_cast<unsigned>([std::chrono::duration_cast](../../chrono/duration/duration_cast.html)<[std::chrono::milliseconds](../../chrono/duration.html)>(
                    [std::chrono::high_resolution_clock::now](../../chrono/high_resolution_clock/now.html)() - started_at -
                    delay * time_tick).count() / time_tick.count());
            [std::this_thread::sleep_for](../sleep_for.html)(occupy * time_tick);
        }
     
        void visualize(unsigned id, unsigned x_scale = 2) const
        {
            ...
            {
                for (n *= x_scale; n-- > 0; [std::cout](../../io/cout.html) << str)
                    ;
            };
            [std::lock_guard](../lock_guard.html) lk{cout_mutex};
            [std::cout](../../io/cout.html) << '#' << [std::setw](../../io/manip/setw.html)(2) << id << ' ';
            cout_n("░", delay);
            cout_n("▒", wait_on_sema);
            cout_n("█", occupy);
    ...
     
    [std::array](../../container/array.html)<Guide, max_threads> guides;
     
    ...
     
        for (auto id{0U}; id != max_threads; ++id)
            threads.push_back([std::jthread](../jthread.html)(workerThread, id));
    }
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/counting_semaphore/acquire&action=edit&section=5 "Edit section: See also")] See also
