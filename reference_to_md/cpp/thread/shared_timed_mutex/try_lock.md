...
Prior [unlock()](unlock.html "cpp/thread/shared timed mutex/unlock") operation on the same mutex _synchronizes-with_ (as defined in [std::memory_order](../../atomic/memory_order.html "cpp/atomic/memory order")) this operation if it returns true. Note that prior [lock()](lock.html "cpp/thread/shared timed mutex/lock") does not synchronize with this operation if it returns false. 
...
[[edit]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/shared_timed_mutex/example_try_lock&action=edit)
