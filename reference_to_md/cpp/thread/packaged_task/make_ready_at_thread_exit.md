...
Calls the stored task as if by [`_INVOKE <R>_`](../../functional.html "cpp/utility/functional")(f, args...), where f is the stored task. The return value of the task or any exception thrown by it is stored in the shared state of *this. 
...
## Contents
  ...
  * [1 Parameters](make_ready_at_thread_exit.html#Parameters)
  * [2 Return value](make_ready_at_thread_exit.html#Return_value)
  * [3 Exceptions](make_ready_at_thread_exit.html#Exceptions)
  * [4 Example](make_ready_at_thread_exit.html#Example)
  * [5 See also](make_ready_at_thread_exit.html#See_also)
...
Run this code
    ...
    struct ProgramState
    {
        [std::packaged_task](../packaged_task.html)<void()> task;
        [std::future](../future.html)<void> future;
        [std::thread](../thread.html) worker;
    };
     
    static void worker([std::shared_ptr](../../memory/shared_ptr.html)<ProgramState> state)
    {
        state->task.make_ready_at_thread_exit(); // execute task right away
     
        auto status = state->future.wait_for([std::chrono::seconds](../../chrono/duration.html)(0));
        if (status == [std::future_status::timeout](../future_status.html))
            [std::cout](../../io/cout.html) << "worker: future is not ready yet\n";
        else
            [std::cout](../../io/cout.html) << "worker: future is ready\n";
     
        [std::cout](../../io/cout.html) << "worker: exit\n";
    }
     
    static [std::shared_ptr](../../memory/shared_ptr.html)<ProgramState> create_state()
    {
        auto state = [std::make_shared](../../memory/shared_ptr/make_shared.html)<ProgramState>();
        state->task = [std::packaged_task](../packaged_task.html)<void()>{[]
        {
            [std::cout](../../io/cout.html) << "task: executed\n";
        }};
        state->future = state->task.get_future();
        state->worker = [std::thread](../thread.html){worker, state};
        return state;
    }
     
    int main()
    ...
     
        state->worker.join();
        [std::cout](../../io/cout.html) << "main: worker finished\n";
     
        auto status = state->future.wait_for([std::chrono::seconds](../../chrono/duration.html)(0));
        if (status == [std::future_status::timeout](../future_status.html))
            [std::cout](../../io/cout.html) << "main: future is not ready yet\n";
        else
            [std::cout](../../io/cout.html) << "main: future is ready\n";
    }
