...
explicit packaged_task( [std::allocator_arg_t](../../memory/allocator_arg_t.html), const Allocator& a, F&& f ); |  (3)  |  (since C++11)   
(until C++17)  
...
2,3) Constructs a `std::packaged_task` object with a stored task of type [std::decay](../../types/decay.html)<F>::type and a shared state. The stored task is initialized with [std::forward](../../utility/forward.html)<F>(f).  These overloads participate in overload resolution only if [std::decay](../../types/decay.html)<F>::type is not the same type as [std::packaged_task](../packaged_task.html)<R(Args...)>. Let t1, t2, ..., tN be values of the corresponding types in `Args`, if [`_INVOKE <R>_`](../../functional.html "cpp/utility/functional")(f, t1, t2, ..., tN) is not a valid expression, the program is ill-formed.  | (until C++20)  
---|---  
This overload participates in overload resolution only if [std::remove_cvref_t](../../types/remove_cvref.html)<F> is not the same type as [std::packaged_task](../packaged_task.html)<R(Args...)>. If [std::is_invocable_r_v](../../types/is_invocable.html)<R, [std::decay_t](../../types/decay.html)<F>&, Args...> is false, the program is ill-formed.  | (since C++20)  
...
## Contents
  ...
  * [1 Parameters](packaged_task.html#Parameters)
  * [2 Exceptions](packaged_task.html#Exceptions)
  * [3 Example](packaged_task.html#Example)
  * [4 Defect reports](packaged_task.html#Defect_reports)
...
Run this code
    ...
    int main()
    {
        [std::packaged_task](../packaged_task.html)<int(int)> fib_task(&fib); 
     
        [std::cout](../../io/cout.html) << "Starting task\n";
        auto result = fib_task.get_future();
        [std::thread](../thread.html) t(std::move(fib_task), 42);
     
        [std::cout](../../io/cout.html) << "Waiting for task to finish..." << [std::endl](../../io/manip/endl.html);
        [std::cout](../../io/cout.html) << result.get() << '\n';
     
        [std::cout](../../io/cout.html) << "Task complete\n";
        t.join();
    }
...
---|---|---|---  
[LWG 2067](https://cplusplus.github.io/LWG/issue2067) | C++11  | the parameter type of the copy constructor was `packaged_task&` | added const  
[LWG 2097](https://cplusplus.github.io/LWG/issue2097) | C++11  | for overloads (2,3), `F` could be `std::packaged_task<R(Args...)>` | `F` is constrained   
[LWG 4154](https://cplusplus.github.io/LWG/issue4154) | C++11  | overloads (2,3) did not consider decaying  | consider decaying 
