[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/mutex/unlock&action=edit)
...
Unlocks the mutex if its level of ownership is 1 (there was exactly one more call to [lock()](lock.html "cpp/thread/recursive timed mutex/lock") than there were calls to `unlock()` made by this thread), reduces the level of ownership by 1 otherwise. The mutex must be locked by the current thread of execution, otherwise, the behavior is undefined. 
