[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/mutex/try_lock_until&action=edit)
...
[recursive_timed_mutex::try_lock](try_lock.html "cpp/thread/recursive timed mutex/try lock")  
...
bool try_lock_until( const [std::chrono::time_point](../../chrono/time_point.html)<Clock, Duration>& timeout_time ); |  |  (since C++11)  
---|---|---  
| |   
...
If timeout_time has already passed, this function behaves like [try_lock()](try_lock.html "cpp/thread/recursive timed mutex/try lock"). 

`Clock` must meet the [Clock](../../named_req/Clock.html "cpp/named req/Clock") requirements. The program is ill-formed if [std::chrono::is_clock_v](../../chrono/is_clock.html)<Clock> is false.(since C++20)
...
As with [try_lock()](try_lock.html "cpp/thread/recursive timed mutex/try lock"), this function is allowed to fail spuriously and return false even if the mutex was not locked by any other thread at some point before timeout_time. 

Prior [unlock()](unlock.html "cpp/thread/recursive timed mutex/unlock") operation on the same mutex _synchronizes-with_ (as defined in [std::memory_order](../../atomic/memory_order.html "cpp/atomic/memory order")) this operation if it returns true. 

A thread may call `try_lock_until` on a recursive mutex repeatedly. Successful calls to `try_lock_until` increment the ownership count: the mutex will only be released after the thread makes a matching number of calls to [unlock](unlock.html "cpp/thread/recursive timed mutex/unlock"). 
...
## Contents
  ...
  * [5 Defect reports](try_lock_until.html#Defect_reports)
  * [6 See also](try_lock_until.html#See_also)
...
---|---|---|---  
[LWG 2093](https://cplusplus.github.io/LWG/issue2093) | C++11  | `try_lock_until` threw nothing  | throws timeout-related exceptions   
