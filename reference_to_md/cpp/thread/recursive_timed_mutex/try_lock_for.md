[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/mutex/try_lock_for&action=edit)
...
If timeout_duration is less or equal timeout_duration.zero(), the function behaves like [try_lock()](try_lock.html "cpp/thread/recursive timed mutex/try lock"). 
...
As with [try_lock()](try_lock.html "cpp/thread/recursive timed mutex/try lock"), this function is allowed to fail spuriously and return false even if the mutex was not locked by any other thread at some point during timeout_duration. 
...
A thread may call `try_lock_for` on a recursive mutex repeatedly. Successful calls to `try_lock_for` increment the ownership count: the mutex will only be released after the thread makes a matching number of calls to [unlock()](unlock.html "cpp/thread/recursive timed mutex/unlock"). 
...
## Contents
  ...
  * [1 Parameters](try_lock_for.html#Parameters)
  * [2 Return value](try_lock_for.html#Return_value)
  * [3 Exceptions](try_lock_for.html#Exceptions)
  * [4 Example](try_lock_for.html#Example)
  * [5 Defect reports](try_lock_for.html#Defect_reports)
  * [6 See also](try_lock_for.html#See_also)
...
Run this code
    ...
     
    [std::mutex](../mutex.html) cout_mutex; // control access to std::cout
    [std::timed_mutex](../timed_mutex.html) mutex;
     
    void job(int id)
    {
        [std::ostringstream](../../io/basic_ostringstream.html) stream;
     
        for (int i = 0; i < 3; ++i)
        {
            ...
            {
                stream << "success ";
                [std::this_thread::sleep_for](../sleep_for.html)(100ms);
                mutex.unlock();
            }
            else
                stream << "failed ";
    ...
     
        [std::lock_guard](../lock_guard.html)<[std::mutex](../mutex.html)> lock{cout_mutex};
        [std::cout](../../io/cout.html) << '[' << id << "] " << stream.str() << '\n';
    }
     
    int main()
    {
