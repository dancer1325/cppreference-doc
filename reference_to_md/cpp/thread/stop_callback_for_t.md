...
using stop_callback_for_t = T::template callback_type<CallbackFn>; |  |  (since C++26)  
| |   
...
The corresponding `stop_callback_for_t` for standard stop token types and any valid `CallbackFn` are: 
  ...
  * specialization of [std::stop_callback](stop_callback.html) for [std::stop_token](stop_token.html), 
