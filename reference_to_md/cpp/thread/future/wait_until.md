[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/future/wait_until&action=edit)
...
Member functions  
---  
[future::future](future.html "cpp/thread/future/future")  
[future::~future](~future.html "cpp/thread/future/~future")  
[future::operator=](operator=.html "cpp/thread/future/operator=")  
[future::share](share.html "cpp/thread/future/share")  
Getting the result  
[future::get](get.html "cpp/thread/future/get")  
State  
[future::valid](valid.html "cpp/thread/future/valid")  
[future::wait](wait.html "cpp/thread/future/wait")  
...
If the future is the result of a call to [async](../async.html "cpp/thread/async") that used lazy evaluation, this function returns immediately without waiting. 

The behavior is undefined if [valid()](valid.html "cpp/thread/future/valid") is false before the call to this function, or `Clock` does not meet the [Clock](../../named_req/Clock.html "cpp/named req/Clock") requirements. The programs is ill-formed if [std::chrono::is_clock_v](../../chrono/is_clock.html)<Clock> is false.(since C++20)
...
## Contents
  ...
  * [6 See also](wait_until.html#See_also)
...
---|---  
[`future_status::deferred`](../future_status.html "cpp/thread/future status") |  The shared state contains a deferred function using lazy evaluation, so the result will be computed only when explicitly requested   
...
The standard recommends that the clock tied to timeout_time be used to measure time; that clock is not required to be a monotonic clock. There are no guarantees regarding the behavior of this function if the clock is adjusted discontinuously, but the existing implementations convert timeout_time from `Clock` to [std::chrono::system_clock](../../chrono/system_clock.html "cpp/chrono/system clock") and delegate to POSIX [`pthread_cond_timedwait`](https://pubs.opengroup.org/onlinepubs/9699919799/functions/pthread_cond_timedwait.html) so that the wait honors adjustments to the system clock, but not to the user-provided `Clock`. In any case, the function also may wait for longer than until after timeout_time has been reached due to scheduling or resource contention delays. 
...
Run this code
    ...
    int main()
    {
        std::chrono::system_clock::time_point two_seconds_passed
            = [std::chrono::system_clock::now](../../chrono/system_clock/now.html)() + [std::chrono::seconds](../../chrono/duration.html)(2);
     
        // Make a future that takes 1 second to complete
        [std::promise](../promise.html)<int> p1;
        [std::future](../future.html)<int> f_completes = p1.get_future();
        [std::thread](../thread.html)([]([std::promise](../promise.html)<int> p1)
                    { 
    ...
     
        ...
        [std::promise](../promise.html)<int> p2;
        [std::future](../future.html)<int> f_times_out = p2.get_future();
        [std::thread](../thread.html)([]([std::promise](../promise.html)<int> p2)
                    { 
                        [std::this_thread::sleep_for](../sleep_for.html)([std::chrono::seconds](../../chrono/duration.html)(5)); 
                        p2.set_value_at_thread_exit(8); 
                    }, 
                    std::move(p2)
        ).detach();
     
        [std::cout](../../io/cout.html) << "Waiting for 2 seconds..." << [std::endl](../../io/manip/endl.html);
     
        if ([std::future_status::ready](../future_status.html) == f_completes.wait_until(two_seconds_passed))
            [std::cout](../../io/cout.html) << "f_completes: " << f_completes.get() << "\n";
        else
            [std::cout](../../io/cout.html) << "f_completes did not complete!\n";
     
        if ([std::future_status::ready](../future_status.html) == f_times_out.wait_until(two_seconds_passed))
            [std::cout](../../io/cout.html) << "f_times_out: " << f_times_out.get() << "\n";
        else
            [std::cout](../../io/cout.html) << "f_times_out did not complete!\n";
     
        [std::cout](../../io/cout.html) << "Done!\n";
    }
...
[ wait](wait.html "cpp/thread/future/wait") |  waits for the result to become available   
