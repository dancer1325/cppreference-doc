...
The class `std::once_flag` is a helper structure for [std::call_once](call_once.html "cpp/thread/call once"). 

An object of type `std::once_flag` that is passed to multiple calls to [std::call_once](call_once.html "cpp/thread/call once") allows those calls to coordinate with each other such that only one of the calls will actually run to completion. 

`std::once_flag` is neither copyable nor movable. 

## Contents

  * [1 Member functions](once_flag.html#Member_functions)
  * [2 std::once_flag::once_flag](once_flag.html#std::once_flag::once_flag)
    * [2.1 Parameters](once_flag.html#Parameters)
    * [2.2 See also](once_flag.html#See_also)
