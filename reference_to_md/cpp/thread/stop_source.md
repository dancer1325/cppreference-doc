...
Member functions  
---  
[stop_source::stop_source](stop_source/stop_source.html "cpp/thread/stop source/stop source")  
[stop_source::~stop_source](stop_source/~stop_source.html "cpp/thread/stop source/~stop source")  
[stop_source::operator=](stop_source/operator=.html "cpp/thread/stop source/operator=")  
Modifiers  
[stop_source::request_stop](stop_source/request_stop.html "cpp/thread/stop source/request stop")  
[stop_source::swap](stop_source/swap.html "cpp/thread/stop source/swap")  
Observers  
[stop_source::get_token](stop_source/get_token.html "cpp/thread/stop source/get token")  
[stop_source::stop_requested](stop_source/stop_requested.html "cpp/thread/stop source/stop requested")  
[stop_source::stop_possible](stop_source/stop_possible.html "cpp/thread/stop source/stop possible")  
Non-member functions  
[swap(std::stop_source)](stop_source/swap2.html "cpp/thread/stop source/swap2")  
[operator==](stop_source/operator_cmp.html "cpp/thread/stop source/operator cmp")  
Helper tags  
...
The `stop_source` class provides the means to issue a stop request, such as for [std::jthread](jthread.html "cpp/thread/jthread") cancellation. A stop request made for one `stop_source` object is visible to all `stop_source`s and [std::stop_token](stop_token.html "cpp/thread/stop token")s of the same associated stop-state; any [std::stop_callback](stop_callback.html "cpp/thread/stop callback")(s) registered for associated [std::stop_token](stop_token.html "cpp/thread/stop token")(s) will be invoked, and any [std::condition_variable_any](condition_variable_any.html "cpp/thread/condition variable any") objects waiting on associated [std::stop_token](stop_token.html "cpp/thread/stop token")(s) will be awoken. 

Once a stop is requested, it cannot be withdrawn. Additional stop requests have no effect. 

## Contents

  * [1 Member functions](stop_source.html#Member_functions)
    * [1.1 Modifiers](stop_source.html#Modifiers)
    * [1.2 Observers](stop_source.html#Observers)
  * [2 Non-member functions](stop_source.html#Non-member_functions)
  * [3 Helper tags](stop_source.html#Helper_tags)
  * [4 Notes](stop_source.html#Notes)
  * [5 Example](stop_source.html#Example)
...
Run this code
    ...
     
    void worker_fun(int id, [std::stop_token](stop_token.html) stoken)
    { 
        for (int i = 10; i; --i)
        {
            ...
            {
                [std::printf](../io/c/fprintf.html)("  worker%d is requested to stop\n", id);
                return;
            }
            [std::printf](../io/c/fprintf.html)("  worker%d goes back to sleep\n", id);
        }
    }
     
    int main()
    {
        [std::jthread](jthread.html) threads[4];
        [std::cout](../io/cout.html) << [std::boolalpha](../io/manip/boolalpha.html);
        auto print = [](const std::stop_source& source)
        {
            [std::printf](../io/c/fprintf.html)("stop_source stop_possible = %s, stop_requested = %s\n",
    ...
     
        ...
        for (int i = 0; i < 4; ++i)
            threads[i] = [std::jthread](jthread.html)(worker_fun, i + 1, stop_source.get_token());
     
        [std::this_thread::sleep_for](sleep_for.html)(500ms);
     
        [std::puts](../io/c/puts.html)("Request stop");
