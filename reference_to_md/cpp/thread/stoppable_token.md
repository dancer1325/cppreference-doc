...
{ tok.stop_requested() } noexcept -> [std::same_as](../concepts/same_as.html)<bool>;  
{ tok.stop_possible() } noexcept -> [std::same_as](../concepts/same_as.html)<bool>;  
{ Token(tok) } noexcept; // see implicit expression variations  
} &&  
[std::copyable](../concepts/copyable.html)<Token> &&  
[std::equality_comparable](../concepts/equality_comparable.html)<Token>; |  |  (since C++26)  
Helper templates |  |   
template< template<class> class >  
struct /*check-type-alias-exists*/; // not defined |  |  (exposition only*)  
| |   
