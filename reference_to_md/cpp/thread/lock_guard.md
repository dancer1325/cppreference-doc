...
The class `lock_guard` is a mutex wrapper that provides a convenient [RAII-style](../language/raii.html "cpp/language/raii") mechanism for owning a mutex for the duration of a scoped block. 
...
## Contents
  ...
  * [1 Template parameters](lock_guard.html#Template_parameters)
  * [2 Member types](lock_guard.html#Member_types)
  * [3 Member functions](lock_guard.html#Member_functions)
  * [4 Notes](lock_guard.html#Notes)
  * [5 Example](lock_guard.html#Example)
  * [6 Defect reports](lock_guard.html#Defect_reports)
  * [7 See also](lock_guard.html#See_also)
...
Run this code
    ...
     
    volatile int g_i = 0;
    [std::mutex](mutex.html) g_i_mutex;  // protects g_i
     
    void safe_increment(int iterations)
    {
        const std::lock_guard<[std::mutex](mutex.html)> lock(g_i_mutex);
        while (iterations-- > 0)
            g_i = g_i + 1;
        [std::cout](../io/cout.html) << "thread #" << [std::this_thread::get_id](get_id.html)() << ", g_i: " << g_i << '\n';
     
    ...
    void unsafe_increment(int iterations)
    {
        while (iterations-- > 0)
            g_i = g_i + 1;
        [std::osyncstream](../io/basic_osyncstream.html)([std::cout](../io/cout.html)) << "thread #" << [std::this_thread::get_id](get_id.html)()
                                    << ", g_i: " << g_i << '\n';
    }
     
    int main()
    {
        auto test = []([std::string_view](../string/basic_string_view.html) fun_name, auto fun)
        {
            g_i = 0;
            [std::cout](../io/cout.html) << fun_name << ":\nbefore, g_i: " << g_i << '\n';
            {
                [std::jthread](jthread.html) t1(fun, 1'000'000);
                [std::jthread](jthread.html) t2(fun, 1'000'000);
            }
            [std::cout](../io/cout.html) << "after, g_i: " << g_i << "\n\n";
        };
