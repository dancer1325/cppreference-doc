...
Member functions  
---  
[recursive_mutex::recursive_mutex](recursive_mutex.html "cpp/thread/recursive mutex/recursive mutex")  
[recursive_mutex::~recursive_mutex](~recursive_mutex.html "cpp/thread/recursive mutex/~recursive mutex")  
Locking  
[recursive_mutex::lock](lock.html "cpp/thread/recursive mutex/lock")  
**recursive_mutex::try_lock**  
[recursive_mutex::unlock](unlock.html "cpp/thread/recursive mutex/unlock")  
Native handle  
...
A thread may call `try_lock` on a recursive mutex repeatedly. Successful calls to `try_lock` increment the ownership count: the mutex will only be released after the thread makes a matching number of calls to [unlock](unlock.html "cpp/thread/recursive mutex/unlock"). 

The maximum number of levels of ownership is unspecified. A call to `try_lock` will return false if this number is exceeded. 

Prior [unlock()](unlock.html "cpp/thread/recursive mutex/unlock") operation on the same mutex _synchronizes-with_ (as defined in [std::memory_order](../../atomic/memory_order.html "cpp/atomic/memory order")) this operation if it returns true. Note that prior [lock()](lock.html "cpp/thread/recursive mutex/lock") does not synchronize with this operation if it returns false. 
...
[[edit]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/recursive_mutex/example_try_lock&action=edit)
...
Run this code
    ...
    int main()
    {
        [std::recursive_mutex](../recursive_mutex.html) test;
        if (test.try_lock())
        {
            [std::cout](../../io/cout.html) << "lock acquired\n";
            test.unlock();
        }
        else
            [std::cout](../../io/cout.html) << "lock not acquired\n";
     
        test.lock();
...
---|---  
[ unlock](unlock.html "cpp/thread/recursive mutex/unlock") |  unlocks the mutex   
