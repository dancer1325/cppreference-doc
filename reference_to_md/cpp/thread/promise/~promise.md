...
Abandons the shared state: 
  ...
  * if the shared state is ready, [releases](../promise.html "cpp/thread/promise") it. 
  * if the shared state is not ready, stores an exception object of type [std::future_error](../future_error.html "cpp/thread/future error") with an error condition [std::future_errc::broken_promise](../future_errc.html "cpp/thread/future errc"), makes the shared state ready and releases it. 
