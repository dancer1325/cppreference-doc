...
promise& operator=( promise&& other ) noexcept; |  (1)  |  (since C++11)  
---|---|---  
promise& operator=( const promise& rhs ) = delete; |  (2)  |  (since C++11)  
| |   
  
Assigns the contents. 

1) Move assignment operator. First, abandons the shared state (as in ~promise()), then assigns the shared state of other as if by executing [std::promise](../promise.html)(std::move(other)).swap(*this).

2) `promise` is not copy-assignable.

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/promise/operator%3D&action=edit&section=1 "Edit section: Parameters")] Parameters

other  |  \-  |  another `promise` to acquire the state from   
---|---|---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/promise/operator%3D&action=edit&section=2 "Edit section: Return value")] Return value
