...
[std::system_error](../../error/system_error.html "cpp/error/system error") is thrown if there is no associated mutex or if the mutex is already locked by this [std::unique_lock](../unique_lock.html "cpp/thread/unique lock"). 
...
### Exceptions
  ...
  * If the mutex is already locked by this `std::unique_lock`, [std::system_error](../../error/system_error.html "cpp/error/system error") with an error code of [std::errc::resource_deadlock_would_occur](../../error/errc.html "cpp/error/errc"). 
...
Run this code
    ...
    int main()
    {
        ...
        using Id = int;
     
        auto worker_task = [&](Id id, [std::chrono::seconds](../../chrono/duration.html) wait, [std::chrono::seconds](../../chrono/duration.html) acquire)
        {
            // wait for a few seconds before acquiring lock.
            [std::this_thread::sleep_for](../sleep_for.html)(wait);
     
            [std::unique_lock](../unique_lock.html)<[std::mutex](../mutex.html)> lock(counter_mutex, [std::defer_lock](../lock_tag_t.html));
            if (lock.try_lock())
                [std::cout](../../io/cout.html) << '#' << id << ", lock acquired.\n";
            else
            {
                [std::cout](../../io/cout.html) << '#' << id << ", failed acquiring lock.\n";
                return;
            }
     
            // keep the lock for a while.
            [std::this_thread::sleep_for](../sleep_for.html)(acquire);
     
            [std::cout](../../io/cout.html) << '#' << id << ", releasing lock (via destructor).\n";
        };
     
