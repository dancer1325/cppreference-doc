...
Member functions  
---  
[stop_token::stop_token](stop_token.html "cpp/thread/stop token/stop token")  
[stop_token::~stop_token](~stop_token.html "cpp/thread/stop token/~stop token")  
[stop_token::operator=](operator=.html "cpp/thread/stop token/operator=")  
Modifiers  
[stop_token::swap](swap.html "cpp/thread/stop token/swap")  
Observers  
[stop_token::stop_requested](stop_requested.html "cpp/thread/stop token/stop requested")  
...
Checks if the `stop_token` object has associated stop-state, and that state either has already had a stop requested or it has associated [std::stop_source](../stop_source.html "cpp/thread/stop source") object(s). 

A default constructed stop_token has no associated stop-state, and thus cannot be stopped; the associated stop-state for which no [std::stop_source](../stop_source.html "cpp/thread/stop source") object(s) exist can also not be stopped if such a request has not already been made. 

## Contents

  * [1 Parameters](stop_possible.html#Parameters)
  * [2 Return value](stop_possible.html#Return_value)
  * [3 Notes](stop_possible.html#Notes)
  * [4 Example](stop_possible.html#Example)
...
Run this code
    ...
    int main()
    {
        ...
        auto print = []([std::string_view](../../string/basic_string_view.html) name, const [std::stop_token](../stop_token.html)& token)
        {
            [std::cout](../../io/cout.html) << [std::format](../../utility/format/format.html)("{}: stop_possible = {:s}, stop_requested = {:s}\n", 
                name, token.stop_possible(), token.stop_requested()
            );
        };
     
        // A worker thread that will listen to stop requests
        auto stop_worker = [std::jthread](../jthread.html)([]([std::stop_token](../stop_token.html) stoken)
        {
            for (int i = 10; i; --i)
            {
                ...
                {
                    [std::cout](../../io/cout.html) << "  Sleepy worker is requested to stop\n";
                    return;
                }
                [std::cout](../../io/cout.html) << "  Sleepy worker goes back to sleep\n";
            }
        });
     
        // A worker thread that will only stop when completed
        auto inf_worker = [std::jthread](../jthread.html)([]()
        {
            for (int i = 5; i; --i)
            {
                [std::this_thread::sleep_for](../sleep_for.html)(300ms);
                [std::cout](../../io/cout.html) << "  Run as long as we want\n";
            }
        });
     
        [std::stop_token](../stop_token.html) def_token;
        [std::stop_token](../stop_token.html) stop_token = stop_worker.get_stop_token();
        [std::stop_token](../stop_token.html) inf_token = inf_worker.get_stop_token();
