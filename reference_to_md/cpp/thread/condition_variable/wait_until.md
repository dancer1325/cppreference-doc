...
Member functions  
---  
[condition_variable::condition_variable](condition_variable.html "cpp/thread/condition variable/condition variable")  
[condition_variable::~condition_variable](~condition_variable.html "cpp/thread/condition variable/~condition variable")  
Notification  
[condition_variable::notify_one](notify_one.html "cpp/thread/condition variable/notify one")  
[condition_variable::notify_all](notify_all.html "cpp/thread/condition variable/notify all")  
Waiting  
[condition_variable::wait](wait.html "cpp/thread/condition variable/wait")  
[condition_variable::wait_for](wait_for.html "cpp/thread/condition variable/wait for")  
...
[std::cv_status](../cv_status.html)  
wait_until( [std::unique_lock](../unique_lock.html)<[std::mutex](../mutex.html)>& lock,  
const [std::chrono::time_point](../../chrono/time_point.html)<Clock, Duration>& abs_time ); |  (1)  |  (since C++11)  
---|---|---  
template< class Clock, class Duration, class Predicate >  
bool wait_until( [std::unique_lock](../unique_lock.html)<[std::mutex](../mutex.html)>& lock,  
const [std::chrono::time_point](../../chrono/time_point.html)<Clock, Duration>& abs_time,  
...
The thread will be unblocked when [notify_all()](notify_all.html "cpp/thread/condition variable/notify all") or [notify_one()](notify_one.html "cpp/thread/condition variable/notify one") is executed, or abs_time is reached. It may also be unblocked spuriously.

When unblocked, calls lock.lock() (possibly blocking on the lock), then returns.

2) Equivalent to while (!pred())  
if (wait_until(lock, abs_time) == [std::cv_status::timeout](../cv_status.html))  
return pred();  
return true;.

This overload may be used to ignore spurious awakenings while waiting for a specific condition to become true.

Right after `wait_until` returns, lock.owns_lock() is true, and lock.mutex() is locked by the calling thread. If these postconditions cannot be satisfied[[1]](wait_until.html#cite_note-1), calls [std::terminate](../../error/terminate.html "cpp/error/terminate"). 

If any of the following conditions is satisfied, the behavior is undefined: 

  * lock.owns_lock() is false. 
  * lock.mutex() is not locked by the calling thread. 
  * If some other threads are also waiting on *this, lock.mutex() is different from the mutex unlocked by the waiting functions ([wait](wait.html "cpp/thread/condition variable/wait"), [wait_for](wait_for.html "cpp/thread/condition variable/wait for") and `wait_until`) called on *this by those threads. 


  1. [↑](wait_until.html#cite_ref-1) This can happen if the re-locking of the mutex throws an exception.



## Contents

  * [1 Parameters](wait_until.html#Parameters)
  * [2 Return value](wait_until.html#Return_value)
  * [3 Exceptions](wait_until.html#Exceptions)
  * [4 Notes](wait_until.html#Notes)
  * [5 Example](wait_until.html#Example)
  * [6 Defect reports](wait_until.html#Defect_reports)
  * [7 See also](wait_until.html#See_also)

  
---  
  
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_until&action=edit&section=1 "Edit section: Parameters")] Parameters
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_until&action=edit&section=2 "Edit section: Return value")] Return value

1) [std::cv_status::timeout](../cv_status.html) if abs_time has been reached, otherwise [std::cv_status::no_timeout](../cv_status.html).

2) The latest result of pred() before returning to the caller.

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_until&action=edit&section=3 "Edit section: Exceptions")] Exceptions

1) Timeout-related exceptions.

2) Timeout-related exceptions, and any exception thrown by pred.

### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_until&action=edit&section=4 "Edit section: Notes")] Notes

The standard recommends that the clock tied to abs_time be used to measure time; that clock is not required to be a monotonic clock. There are no guarantees regarding the behavior of this function if the clock is adjusted discontinuously, but the existing implementations convert abs_time from `Clock` to [std::chrono::system_clock](../../chrono/system_clock.html "cpp/chrono/system clock") and delegate to POSIX [`pthread_cond_timedwait`](https://pubs.opengroup.org/onlinepubs/9699919799/functions/pthread_cond_timedwait.html) so that the wait honors adjustments to the system clock, but not to the user-provided `Clock`. In any case, the function also may wait for longer than until after abs_time has been reached due to scheduling or resource contention delays. 

Even if the clock in use is [std::chrono::steady_clock](../../chrono/steady_clock.html "cpp/chrono/steady clock") or another monotonic clock, a system clock adjustment may induce a spurious wakeup. 
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_until&action=edit&section=5 "Edit section: Example")] Example
...
Run this code
    ...
    #include <thread>
     
    [std::condition_variable](../condition_variable.html) cv;
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_until&action=edit&section=6 "Edit section: Defect reports")] Defect reports
...
### [[edit](https://en.cppreference.com/mwiki/index.php?title=cpp/thread/condition_variable/wait_until&action=edit&section=7 "Edit section: See also")] See also
...
---|---  
[ wait_for](wait_for.html "cpp/thread/condition variable/wait for") |  blocks the current thread until the condition variable is awakened or after the specified timeout duration   
