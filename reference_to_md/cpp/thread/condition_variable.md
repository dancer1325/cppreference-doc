...
Member functions  
---  
[condition_variable::condition_variable](condition_variable/condition_variable.html "cpp/thread/condition variable/condition variable")  
[condition_variable::~condition_variable](condition_variable/~condition_variable.html "cpp/thread/condition variable/~condition variable")  
Notification  
[condition_variable::notify_one](condition_variable/notify_one.html "cpp/thread/condition variable/notify one")  
[condition_variable::notify_all](condition_variable/notify_all.html "cpp/thread/condition variable/notify all")  
Waiting  
[condition_variable::wait](condition_variable/wait.html "cpp/thread/condition variable/wait")  
[condition_variable::wait_for](condition_variable/wait_for.html "cpp/thread/condition variable/wait for")  
[condition_variable::wait_until](condition_variable/wait_until.html "cpp/thread/condition variable/wait until")  
Native handle  
...
`std::condition_variable` is a synchronization primitive used with a [std::mutex](mutex.html "cpp/thread/mutex") to block one or more threads until another thread both modifies a shared variable (the _condition_) and notifies the `std::condition_variable`. 

The thread that intends to modify the shared variable must: 

  1. Acquire a [std::mutex](mutex.html "cpp/thread/mutex") (typically via [std::lock_guard](lock_guard.html "cpp/thread/lock guard")). 
  2. Modify the shared variable while the lock is owned. 
  3. Call [notify_one](condition_variable/notify_one.html "cpp/thread/condition variable/notify one") or [notify_all](condition_variable/notify_all.html "cpp/thread/condition variable/notify all") on the `std::condition_variable` (can be done after releasing the lock). 
...
Any thread that intends to wait on a `std::condition_variable` must: 
  ...
  1. Acquire a [std::unique_lock](unique_lock.html)<[std::mutex](mutex.html)> on the mutex used to protect the shared variable. 
  ...
  2. Call [wait](condition_variable/wait.html "cpp/thread/condition variable/wait"), [wait_for](condition_variable/wait_for.html "cpp/thread/condition variable/wait for"), or [wait_until](condition_variable/wait_until.html "cpp/thread/condition variable/wait until") on the `std::condition_variable` (atomically releases the mutex and suspends thread execution until the condition variable is notified, a timeout expires, or a [spurious wakeup](https://en.wikipedia.org/wiki/Spurious_wakeup "enwiki:Spurious wakeup") occurs, then atomically acquires the mutex before returning). 
  ...
  1. Use the predicated overload of [wait](condition_variable/wait.html "cpp/thread/condition variable/wait"), [wait_for](condition_variable/wait_for.html "cpp/thread/condition variable/wait for"), and [wait_until](condition_variable/wait_until.html "cpp/thread/condition variable/wait until"), which performs the same three steps. 



`std::condition_variable` works only with [std::unique_lock](unique_lock.html)<[std::mutex](mutex.html)>, which allows for maximal efficiency on some platforms. [std::condition_variable_any](condition_variable_any.html "cpp/thread/condition variable any") provides a condition variable that works with any [BasicLockable](../named_req/BasicLockable.html "cpp/named req/BasicLockable") object, such as [std::shared_lock](shared_lock.html "cpp/thread/shared lock"). 

Condition variables permit concurrent invocation of the [wait](condition_variable/wait.html "cpp/thread/condition variable/wait"), [wait_for](condition_variable/wait_for.html "cpp/thread/condition variable/wait for"), [wait_until](condition_variable/wait_until.html "cpp/thread/condition variable/wait until"), [notify_one](condition_variable/notify_one.html "cpp/thread/condition variable/notify one") and [notify_all](condition_variable/notify_all.html "cpp/thread/condition variable/notify all") member functions. 

The class `std::condition_variable` is a [StandardLayoutType](../named_req/StandardLayoutType.html "cpp/named req/StandardLayoutType"). It is not [CopyConstructible](../named_req/CopyConstructible.html "cpp/named req/CopyConstructible"), [MoveConstructible](../named_req/MoveConstructible.html "cpp/named req/MoveConstructible"), [CopyAssignable](../named_req/CopyAssignable.html "cpp/named req/CopyAssignable"), or [MoveAssignable](../named_req/MoveAssignable.html "cpp/named req/MoveAssignable"). 

## Contents

  * [1 Nested types](condition_variable.html#Nested_types)
  * [2 Member functions](condition_variable.html#Member_functions)
    * [2.1 Notification](condition_variable.html#Notification)
    * [2.2 Waiting](condition_variable.html#Waiting)
    * [2.3 Native handle](condition_variable.html#Native_handle)
  * [3 Example](condition_variable.html#Example)
  * [4 See also](condition_variable.html#See_also)
...
Run this code
    ...
    void worker_thread()
    {
        ...
        [std::unique_lock](unique_lock.html) lk(m);
        cv.wait(lk, []{ return ready; });
     
        // after the wait, we own the lock
        [std::cout](../io/cout.html) << "Worker thread is processing data\n";
        data += " after processing";
     
        // send data back to main()
        processed = true;
        [std::cout](../io/cout.html) << "Worker thread signals data processing completed\n";
     
    ...
    int main()
    {
        [std::thread](thread.html) worker(worker_thread);
     
        data = "Example data";
        // send data to the worker thread
        {
            [std::lock_guard](lock_guard.html) lk(m);
            ready = true;
            [std::cout](../io/cout.html) << "main() signals data ready for processing\n";
        }
        cv.notify_one();
     
        // wait for the worker
        ...
        [std::cout](../io/cout.html) << "Back in main(), data = " << data << '\n';
     
        worker.join();
    }
