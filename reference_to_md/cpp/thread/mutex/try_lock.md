...
[`std::mutex`](../mutex.html "cpp/thread/mutex")

Member functions  
---  
[mutex::mutex](mutex.html "cpp/thread/mutex/mutex")  
[mutex::~mutex](~mutex.html "cpp/thread/mutex/~mutex")  
Locking  
[mutex::lock](lock.html "cpp/thread/mutex/lock")  
**mutex::try_lock**  
[mutex::unlock](unlock.html "cpp/thread/mutex/unlock")  
Native handle  
[mutex::native_handle](native_handle.html "cpp/thread/mutex/native handle")  
...
Prior [unlock()](unlock.html "cpp/thread/mutex/unlock") operation on the same mutex _synchronizes-with_ (as defined in [std::memory_order](../../atomic/memory_order.html "cpp/atomic/memory order")) this operation if it returns true. Note that prior [lock()](lock.html "cpp/thread/mutex/lock") does not synchronize with this operation if it returns false. 
...
[[edit]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/mutex/example_try_lock&action=edit)
...
Run this code
    ...
    #include <thread>
     
    [std::chrono::milliseconds](../../chrono/duration.html) interval(100);
     
    ...
    void job_1() 
    {
        [std::this_thread::sleep_for](../sleep_for.html)(interval); // let 'job_2' take a lock
     
        while (true)
        {
            ...
            if (mutex.try_lock())
            {
                [std::cout](../../io/cout.html) << "job shared (" << job_shared << ")\n";
                mutex.unlock();
                return;
            }
            else
            {
                ...
                ++job_exclusive;
                [std::cout](../../io/cout.html) << "job exclusive (" << job_exclusive << ")\n";
                [std::this_thread::sleep_for](../sleep_for.html)(interval);
            }
        }
    }
     
    ...
    {
        mutex.lock();
        [std::this_thread::sleep_for](../sleep_for.html)(5 * interval);
        ++job_shared;
        mutex.unlock();
    }
     
    int main() 
    {
        [std::thread](../thread.html) thread_1(job_1);
        [std::thread](../thread.html) thread_2(job_2);
     
        thread_1.join();
        thread_2.join();
    }
...
[ lock](lock.html "cpp/thread/mutex/lock") |  locks the mutex, blocks if the mutex is not available   
...
---|---  
[ unlock](unlock.html "cpp/thread/mutex/unlock") |  unlocks the mutex   
