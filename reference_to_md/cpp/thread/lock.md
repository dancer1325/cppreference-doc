...
Locks the given [Lockable](../named_req/Lockable.html "cpp/named req/Lockable") objects lock1, lock2, `...`, lockn using a deadlock avoidance algorithm to avoid deadlock. 
...
Run this code
    ...
    struct Employee
    {
        ...
        [std::string](../string/basic_string.html) output() const
        {
    ...
    void send_mail(Employee&, Employee&)
    {
        ...
        [std::this_thread::sleep_for](sleep_for.html)([std::chrono::milliseconds](../chrono/duration.html)(696));
    }
     
    void assign_lunch_partner(Employee& e1, Employee& e2)
    {
        static [std::mutex](mutex.html) io_mutex;
        {
            [std::lock_guard](lock_guard.html)<[std::mutex](mutex.html)> lk(io_mutex);
            [std::cout](../io/cout.html) << e1.id << " and " << e2.id << " are waiting for locks" << [std::endl](../io/manip/endl.html);
        }
     
        ...
        {
            std::lock(e1.m, e2.m);
            [std::lock_guard](lock_guard.html)<[std::mutex](mutex.html)> lk1(e1.m, [std::adopt_lock](lock_tag_t.html));
            [std::lock_guard](lock_guard.html)<[std::mutex](mutex.html)> lk2(e2.m, [std::adopt_lock](lock_tag_t.html));
        ...
        //  std::scoped_lock lk(e1.m, e2.m);
            {
                [std::lock_guard](lock_guard.html)<[std::mutex](mutex.html)> lk(io_mutex);
                [std::cout](../io/cout.html) << e1.id << " and " << e2.id << " got locks" << [std::endl](../io/manip/endl.html);
            }
    ...
     
        for (auto& thread : threads)
            thread.join();
     
        [std::cout](../io/cout.html) << alice.output() << '\n'
                  << bob.output() << '\n'
                  << christina.output() << '\n'
                  << dave.output() << '\n';
    }
