...
Member functions  
---  
[future::future](future/future.html "cpp/thread/future/future")  
[future::~future](future/~future.html "cpp/thread/future/~future")  
[future::operator=](future/operator=.html "cpp/thread/future/operator=")  
[future::share](future/share.html "cpp/thread/future/share")  
Getting the result  
[future::get](future/get.html "cpp/thread/future/get")  
State  
[future::valid](future/valid.html "cpp/thread/future/valid")  
[future::wait](future/wait.html "cpp/thread/future/wait")  
...
The class template `std::future` provides a mechanism to access the result of asynchronous operations: 
  ...
  * An asynchronous operation (created via [std::async](async.html "cpp/thread/async"), [std::packaged_task](packaged_task.html "cpp/thread/packaged task"), or [std::promise](promise.html "cpp/thread/promise")) can provide a `std::future` object to the creator of that asynchronous operation. 
  ...
  * When the asynchronous operation is ready to send a result to the creator, it can do so by modifying _shared state_ (e.g. [std::promise::set_value](promise/set_value.html "cpp/thread/promise/set value")) that is linked to the creator's `std::future`. 



Note that `std::future` references shared state that is not shared with any other asynchronous return objects (as opposed to [std::shared_future](shared_future.html "cpp/thread/shared future")). 

## Contents

  * [1 Member functions](future.html#Member_functions)
    * [1.1 Getting the result](future.html#Getting_the_result)
    * [1.2 State](future.html#State)
  * [2 Examples](future.html#Examples)
    * [2.1 Example with exceptions](future.html#Example_with_exceptions)
  * [3 See also](future.html#See_also)
...
Run this code
    ...
    int main()
    {
        ...
        [std::packaged_task](packaged_task.html)<int()> task([]{ return 7; }); // wrap the function
        std::future<int> f1 = task.get_future(); // get a future
        [std::thread](thread.html) t(std::move(task)); // launch on a thread
     
        // future from an async()
        std::future<int> f2 = [std::async](async.html)([std::launch::async](launch.html), []{ return 8; });
     
        // future from a promise
        [std::promise](promise.html)<int> p;
        std::future<int> f3 = p.get_future();
        [std::thread](thread.html)([&p]{ p.set_value_at_thread_exit(9); }).detach();
     
