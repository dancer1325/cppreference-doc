[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/thread/thread/joinable&action=edit)
...
Checks if the `std::thread` object identifies an active thread of execution. Specifically, returns true if get_id() != [std::thread::id](id.html)(). So a default constructed `thread` is not joinable. 

A thread that has finished executing code, but has not yet been joined is still considered an active thread of execution and is therefore joinable. 

## Contents

  * [1 Parameters](joinable.html#Parameters)
  * [2 Return value](joinable.html#Return_value)
  * [3 Example](joinable.html#Example)
  * [4 References](joinable.html#References)
  * [5 See also](joinable.html#See_also)
...
Run this code
    ...
     
        [std::thread](../thread.html) t;
        [std::cout](../../io/cout.html) << "before starting, joinable: " << t.joinable() << '\n';
     
        t = [std::thread](../thread.html){foo};
        [std::cout](../../io/cout.html) << "after starting, joinable: " << t.joinable() << '\n';
     
        t.join();
        [std::cout](../../io/cout.html) << "after joining, joinable: " << t.joinable() << '\n';
     
        t = [std::thread](../thread.html){foo};
        t.detach();
        [std::cout](../../io/cout.html) << "after detaching, joinable: " << t.joinable() << '\n';
        [std::this_thread::sleep_for](../sleep_for.html)(1500ms);
    }
