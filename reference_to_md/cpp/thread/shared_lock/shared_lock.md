...
shared_lock( mutex_type& m, [std::defer_lock_t](../lock_tag_t.html) t ) noexcept; |  (4)  |  (since C++14)  
shared_lock( mutex_type& m, [std::try_to_lock_t](../lock_tag_t.html) t ); |  (5)  |  (since C++14)  
shared_lock( mutex_type& m, [std::adopt_lock_t](../lock_tag_t.html) t ); |  (6)  |  (since C++14)  
template< class Rep, class Period >  
shared_lock( mutex_type& m,   
const [std::chrono::duration](../../chrono/duration.html)<Rep,Period>& timeout_duration ); |  (7)  |  (since C++14)  
template< class Clock, class Duration >  
shared_lock( mutex_type& m,   
const [std::chrono::time_point](../../chrono/time_point.html)<Clock,Duration>& timeout_time ); |  (8)  |  (since C++14)  
| |   
...
7) Tries to lock the associated mutex in shared mode by calling m.try_lock_shared_for(timeout_duration), which blocks until specified timeout_duration has elapsed or the lock is acquired, whichever comes first. May block for longer than timeout_duration. The behavior is undefined if `Mutex` does not meet the [SharedTimedLockable](../../named_req/SharedTimedLockable.html "cpp/named req/SharedTimedLockable") requirements.
...
Run this code
    ...
    #include <thread>
     
    [std::shared_timed_mutex](../shared_timed_mutex.html) m;
    int i = 10;
     
    void read_shared_var(int id)
    {
         // both the threads get access to the integer i
         [std::shared_lock](../shared_lock.html)<[std::shared_timed_mutex](../shared_timed_mutex.html)> slk(m);
         const int ii = i; // reads global i
     
         [std::osyncstream](../../io/basic_osyncstream.html)([std::cout](../../io/cout.html)) << '#' << id << " read i as " << ii << "...\n";
         [std::this_thread::sleep_for](../sleep_for.html)([std::chrono::milliseconds](../../chrono/duration.html)(10));
         [std::osyncstream](../../io/basic_osyncstream.html)([std::cout](../../io/cout.html)) << '#' << id << " woke up..." << [std::endl](../../io/manip/endl.html);
    }
     
    int main()
    {
         [std::thread](../thread.html) r1{read_shared_var, 1};
         [std::thread](../thread.html) r2{read_shared_var, 2};
     
         r1.join();
         r2.join();
    }
