...
## Contents
  ...
  * [1 Parameters](lock.html#Parameters)
  * [2 Return value](lock.html#Return_value)
  * [3 Exceptions](lock.html#Exceptions)
  * [4 Example](lock.html#Example)
...
### Exceptions
  ...
  * If the associated mutex is already locked by this `shared_lock` (that is, [owns_lock](owns_lock.html "cpp/thread/shared lock/owns lock") returns true), [std::system_error](../../error/system_error.html "cpp/error/system error") with an error code of [std::errc::resource_deadlock_would_occur](../../error/errc.html "cpp/error/errc"). 
...
Run this code
    ...
    #include <thread>
     
    [std::string](../../string/basic_string.html) file = "Original content."; // Simulates a file
    [std::mutex](../mutex.html) output_mutex; // mutex that protects output operations.
    [std::shared_mutex](../shared_mutex.html) file_mutex; // reader/writer mutex
     
    void read_content(int id)
    {
        [std::string](../../string/basic_string.html) content;
        {
            [std::shared_lock](../shared_lock.html) lock(file_mutex, [std::defer_lock](../lock_tag_t.html)); // Do not lock it first.
            lock.lock(); // Lock it here.
            content = file;
        }
        [std::lock_guard](../lock_guard.html) lock(output_mutex);
        [std::cout](../../io/cout.html) << "Contents read by reader #" << id << ": " << content << '\n';
    }
     
    void write_content()
    {
        {
            [std::lock_guard](../lock_guard.html) file_lock(file_mutex);
            file = "New content";
        }
        [std::lock_guard](../lock_guard.html) output_lock(output_mutex);
        [std::cout](../../io/cout.html) << "New content saved.\n";
    }
     
    int main()
    {
        [std::cout](../../io/cout.html) << "Two readers reading from file.\n"
                  << "A writer competes with them.\n";
        [std::thread](../thread.html) reader1{read_content, 1};
        [std::thread](../thread.html) reader2{read_content, 2};
        [std::thread](../thread.html) writer{write_content};
        reader1.join();
        reader2.join();
        writer.join();
        [std::cout](../../io/cout.html) << "The first few operations to file are done.\n";
        reader1 = [std::thread](../thread.html){read_content, 3};
        reader1.join();
    }
