...
requires [std::bool_constant](../types/integral_constant.html)<(!tok.stop_possible())>::value;  
}; |  |  (since C++26)  
| |   
  
The concept `unstoppable_token<Token>` specifies an additional requirement for a [`stoppable_token`](stoppable_token.html "cpp/thread/stoppable token") where the type does not allow stopping. That is, the expression tok.stop_possible() must be usable in constant expression and returns false. 
