...
class stop_callback; |  |  (since C++20)  
| |   
  
The `stop_callback` class template provides an RAII object type that registers a callback function for an associated [`std::stop_token`](stop_token.html "cpp/thread/stop token") object, such that the callback function will be invoked when the [`std::stop_token`](stop_token.html "cpp/thread/stop token")'s associated [`std::stop_source`](stop_source.html "cpp/thread/stop source") is requested to stop. 

Callback functions registered via `stop_callback`'s constructor are invoked either in the same thread that successfully invokes request_stop() for a [`std::stop_source`](stop_source.html "cpp/thread/stop source") of the `stop_callback`'s associated [`std::stop_token`](stop_token.html "cpp/thread/stop token"); or if stop has already been requested prior to the constructor's registration, then the callback is invoked in the thread constructing the `stop_callback`. 

More than one `stop_callback` can be created for the same [`std::stop_token`](stop_token.html "cpp/thread/stop token"), from the same or different threads concurrently. No guarantee is provided for the order in which they will be executed, but they will be invoked synchronously; except for `stop_callback`(s) constructed after stop has already been requested for the [`std::stop_token`](stop_token.html "cpp/thread/stop token"), as described previously. 

If an invocation of a callback exits via an exception then [std::terminate](../error/terminate.html "cpp/error/terminate") is called. 

`std::stop_callback` is not [CopyConstructible](../named_req/CopyConstructible.html "cpp/named req/CopyConstructible"), [CopyAssignable](../named_req/CopyAssignable.html "cpp/named req/CopyAssignable"), [MoveConstructible](../named_req/MoveConstructible.html "cpp/named req/MoveConstructible"), nor [MoveAssignable](../named_req/MoveAssignable.html "cpp/named req/MoveAssignable"). 

The template param `Callback` type must be both [`invocable`](../concepts/invocable.html "cpp/concepts/invocable") and [`destructible`](../concepts/destructible.html "cpp/concepts/destructible"). Any return value is ignored. 

## Contents

  * [1 Member types](stop_callback.html#Member_types)
  * [2 Member functions](stop_callback.html#Member_functions)
  * [3 Deduction guides](stop_callback.html#Deduction_guides)
  * [4 Example](stop_callback.html#Example)
...
Run this code
    ...
    class Writer
    {
        [std::ostringstream](../io/basic_ostringstream.html) buffer;
    public:
        ~Writer()
        {
            [std::cout](../io/cout.html) << buffer.str();
        }
    ...
    int main()
    {
        ...
        [std::jthread](jthread.html) worker([] ([std::stop_token](stop_token.html) stoken)
        {
            Writer() << "Worker thread's id: " << [std::this_thread::get_id](get_id.html)() << '\n';
            [std::mutex](mutex.html) mutex;
            [std::unique_lock](unique_lock.html) lock(mutex);
            [std::condition_variable_any](condition_variable_any.html)().wait(lock, stoken,
    ...
     
        ...
        std::stop_callback callback(worker.get_stop_token(), []
        {
            Writer() << "Stop callback executed by thread: "
                << [std::this_thread::get_id](get_id.html)() << '\n';
        });
     
    ...
     
        ...
        [std::jthread](jthread.html) stopper1(stopper_func);
        [std::jthread](jthread.html) stopper2(stopper_func);
        stopper1.join();
        stopper2.join();
     
        // After a stop has already been requested,
        // a new stop_callback executes immediately.
        Writer() << "Main thread: " << [std::this_thread::get_id](get_id.html)() << '\n';
