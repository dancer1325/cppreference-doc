...
This is the case only for futures that were not default-constructed or moved from. Unlike [std::future](../future.html "cpp/thread/future"), `std::shared_future`'s shared state is not invalidated when `get()` is called. 

The behavior is undefined if any member function other than the destructor, the copy-assignment operator, the move-assignment operator, or `valid` is called on a `shared_future ` that does not refer to shared state (although implementations are encouraged to throw [std::future_error](../future_error.html "cpp/thread/future error") indicating `no_state` in this case). It is valid to move or copy from a shared_future object for which `valid()` is false. 
...
Run this code
    ...
    int main()
    {
        ...
        [std::shared_future](../shared_future.html)<void> f = p.get_future();
     
