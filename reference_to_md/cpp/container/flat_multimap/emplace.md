...
template< class... Args >  
iterator emplace( Args&&... args ); |  | (since C++23)   
...
Initializes an object t of type [std::pair](../../utility/pair.html)<key_type, mapped_type> with [std::forward](../../utility/forward.html)<Args>(args)...; if the map already contains an element whose key is equivalent to t.first, *this is unchanged. Otherwise, equivalent to: 
    
    
    auto key_it = [ranges::upper_bound](../../algorithm/ranges/upper_bound.html)(c.keys, t.first, compare);
    auto value_it = c.values.begin() + [std::distance](../../iterator/distance.html)(c.keys.begin(), key_it);
    c.keys.insert(key_it, std::move(t.first));
    c.values.insert(value_it, std::move(t.second));

This overload participates in overload resolution only if [std::is_constructible_v](../../types/is_constructible.html)<[std::pair](../../utility/pair.html)<key_type, mapped_type>, Args...> is true. 

If `value_type` is not [EmplaceConstructible](../../named_req/EmplaceConstructible.html "cpp/named req/EmplaceConstructible") into `flat_multimap` from args, the behavior is undefined. 
...
Run this code
    ...
    int main()
    {
        [std::flat_multimap](../flat_multimap.html)<[std::string](../../string/basic_string.html), [std::string](../../string/basic_string.html)> m;
     
