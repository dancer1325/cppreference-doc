...
First, initializes an object t of type `value_type` with [std::forward](../../utility/forward.html)<Args>(args)..., then inserts t as if by 
    
    
    auto it = [ranges::upper_bound](../../algorithm/ranges/upper_bound.html)(c, t, compare);
    c.insert(it, std::move(t));

This overload participates in overload resolution only if [std::is_constructible_v](../../types/is_constructible.html)<value_type, Args...> is true. 

If `value_type` is not [EmplaceConstructible](../../named_req/EmplaceConstructible.html "cpp/named req/EmplaceConstructible") into `flat_multiset` from args, the behavior is undefined. 
