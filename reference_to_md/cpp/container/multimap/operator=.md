...
multimap& operator=( const multimap& other ); |  (1) | (constexpr since C++26)  
---|---|---  
| (2) |   
multimap& operator=( multimap&& other ); |  | (since C++11)   
(until C++17)  
multimap& operator=( multimap&& other )  
noexcept(/* see below */); |  | (since C++17)   
(constexpr since C++26)  
multimap& operator=( [std::initializer_list](../../utility/initializer_list.html)<value_type> ilist ); |  (3) | (since C++11)   
(constexpr since C++26)  
| |   
...
3) \\(\scriptsize O(N \cdot \log(N))\\)O(N·log(N)) in general, where \\(\scriptsize N\\)N is size() + ilist.size(). Linear if ilist is sorted with respect to [value_comp()](value_comp.html "cpp/container/multimap/value comp").
...
The following code uses operator= to assign one [std::multimap](../multimap.html "cpp/container/multimap") to another:
...
Run this code
    ...
    int main()
    {
        [std::multimap](../multimap.html)<int, int> x{{1,1}, {2,2}, {3,3}}, y, z;
...
[ (constructor)](multimap.html "cpp/container/multimap/multimap") |  constructs the `multimap`   
