...
set& operator=( const set& other ); |  (1) | (constexpr since C++26)  
---|---|---  
| (2) |   
set& operator=( set&& other ); |  | (since C++11)   
(until C++17)  
set& operator=( set&& other )  
noexcept(/* see below */); |  | (since C++17)   
(constexpr since C++26)  
set& operator=( [std::initializer_list](../../utility/initializer_list.html)<value_type> ilist ); |  (3) | (since C++11)   
(constexpr since C++26)  
| |   
...
3) \\(\scriptsize O(N \cdot \log(N))\\)O(N·log(N)) in general, where \\(\scriptsize N\\)N is size() + ilist.size(). Linear if ilist is sorted with respect to [value_comp()](value_comp.html "cpp/container/set/value comp").
...
The following code uses operator= to assign one [std::set](../set.html "cpp/container/set") to another:
...
Run this code
    ...
    int main()
    {
        [std::set](../set.html)<int> x{1, 2, 3}, y, z;
...
[ (constructor)](set.html "cpp/container/set/set") |  constructs the `set`   
