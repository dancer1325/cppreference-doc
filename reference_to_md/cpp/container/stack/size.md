...
Returns the number of elements in the container adaptor. Equivalent to: return` `[`c`](../stack.html#Member_objects "cpp/container/stack").size(). 
...
Run this code
    ...
    int main()
    {
        ...
        [assert](../../error/assert.html)(stack.size() == 0);
     
        const int count = 8;
        for (int i = 0; i != count; ++i)
            stack.push(i);
        [assert](../../error/assert.html)(stack.size() == count);
    }

### See also

[ empty](empty.html "cpp/container/stack/empty") |  checks whether the container adaptor is empty   
