...
Run this code
    ...
    {
        [std::map](../map.html)<unsigned, unsigned> open_brackets, close_brackets;
        [std::array](../array.html)<[std::uint8_t](../../types/integer.html), 32768> data_{0};
        unsigned program_{0};
        int pos_{0};
     
        void collect_brackets(const [std::string_view](../../string/basic_string_view.html) program)
        {
            [std::stack](../stack.html)<unsigned> brackets_stack;
     
            for (auto pos{0U}; pos != program.length(); ++pos)
            {
                ...
                else if (']' == c)
                {
                    if (brackets_stack.empty())
                        throw [std::runtime_error](../../error/runtime_error.html)("Brackets [] do not match!");
                    else
                    {
    ...
     
        void check_data_pos(int pos)
        {
            if (pos < 0 or static_cast<int>(data_.size()) <= pos)
                throw [std::out_of_range](../../error/out_of_range.html){"Data pointer out of bound!"};
        }
     
    public:
        BrainHackInterpreter(const [std::string_view](../../string/basic_string_view.html) program)
        {
            for (collect_brackets(program); program_ < program.length(); ++program_)
                switch (program[program_])
                {
                    ...
                    case '+':
                        ...
                        break;
                    case '.':
                        [std::cout](../../io/cout.html) << data_[pos_];
                        break;
                    case ',':
                        [std::cin](../../io/cin.html) >> data_[pos_];
                        break;
                    case '[':
                        if (data_[pos_] == 0)
