...
bool operator==( const [std::unordered_multiset](../unordered_multiset.html)<Key, Hash, KeyEqual, Alloc>& lhs,  
const [std::unordered_multiset](../unordered_multiset.html)<Key, Hash, KeyEqual, Alloc>& rhs ); |  (1)  |   
---|---|---  
template< class Key, class Hash, class KeyEqual, class Alloc >  
bool operator!=( const [std::unordered_multiset](../unordered_multiset.html)<Key, Hash, KeyEqual, Alloc>& lhs,  
const [std::unordered_multiset](../unordered_multiset.html)<Key, Hash, KeyEqual, Alloc>& rhs ); |  (2)  |  (until C++20)  
| |   
...
The behavior is undefined if `Key` is not [EqualityComparable](../../named_req/EqualityComparable.html "cpp/named req/EqualityComparable"). 
...
Proportional to _ΣS i2_ calls to operator== on `value_type`, calls to the predicate returned by [`key_eq`](key_eq.html "cpp/container/unordered multiset/key eq"), and calls to the hasher returned by [`hash_function`](hash_function.html "cpp/container/unordered multiset/hash function") in the average case, where _S_ is the size of the _i_ th equivalent key group. Proportional to _N 2_ in the worst case, where _N_ is the size of the container. Average case becomes proportional to _N_ if the elements within each equivalent key group are arranged in the same order (happens when the containers are copies of each other). 
