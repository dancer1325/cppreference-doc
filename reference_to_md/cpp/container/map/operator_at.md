[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/container/operator_at_assoc&action=edit)
...
[map::insert](insert.html "cpp/container/map/insert")  
...
1) Inserts value_type(key, T()) if the key does not exist.  |  -`key_type` must meet the requirements of [CopyConstructible](../../named_req/CopyConstructible.html "cpp/named req/CopyConstructible").   
---  
-`mapped_type` must meet the requirements of [CopyConstructible](../../named_req/CopyConstructible.html "cpp/named req/CopyConstructible") and [DefaultConstructible](../../named_req/DefaultConstructible.html "cpp/named req/DefaultConstructible").   
If an insertion is performed, the mapped value is [value-initialized](../../language/value_initialization.html "cpp/language/value initialization") (default-constructed for class types, zero-initialized otherwise) and a reference to it is returned.

(until C++11)  
1) Inserts a [`value_type`](../map.html#Member_types "cpp/container/map") object constructed in-place from [std::piecewise_construct](../../utility/piecewise_construct.html), [std::forward_as_tuple](../../utility/tuple/forward_as_tuple.html)(key), [std::tuple](../../utility/tuple.html)<>() if the key does not exist. Equivalent to return this->try_emplace(key).first->second;.(since C++17)When the default allocator is used, this results in the key being copy constructed from `key` and the mapped value being [value-initialized](../../language/value_initialization.html "cpp/language/value initialization").  |  -`value_type` must be [EmplaceConstructible](../../named_req/EmplaceConstructible.html "cpp/named req/EmplaceConstructible") from [std::piecewise_construct](../../utility/piecewise_construct.html), [std::forward_as_tuple](../../utility/tuple/forward_as_tuple.html)(key), [std::tuple](../../utility/tuple.html)<>(). When the default allocator is used, this means that [`key_type`](../map.html#Member_types "cpp/container/map") must be [CopyConstructible](../../named_req/CopyConstructible.html "cpp/named req/CopyConstructible") and `mapped_type` must be [DefaultConstructible](../../named_req/DefaultConstructible.html "cpp/named req/DefaultConstructible").   
---  
  
2) Inserts a [`value_type`](../map.html#Member_types "cpp/container/map") object constructed in-place from [std::piecewise_construct](../../utility/piecewise_construct.html), [std::forward_as_tuple](../../utility/tuple/forward_as_tuple.html)(std::move(key)), [std::tuple](../../utility/tuple.html)<>() if the key does not exist.

Equivalent to return this->try_emplace(std::move(key)).first->second;.(since C++17)  
When the default allocator is used, this results in the key being move constructed from `key` and the mapped value being [value-initialized](../../language/value_initialization.html "cpp/language/value initialization").  -`value_type` must be [EmplaceConstructible](../../named_req/EmplaceConstructible.html "cpp/named req/EmplaceConstructible") from [std::piecewise_construct](../../utility/piecewise_construct.html), [std::forward_as_tuple](../../utility/tuple/forward_as_tuple.html)(std::move(key)), [std::tuple](../../utility/tuple.html)<>(). When the default allocator is used, this means that [`key_type`](../map.html#Member_types "cpp/container/map") must be [MoveConstructible](../../named_req/MoveConstructible.html "cpp/named req/MoveConstructible") and `mapped_type` must be [DefaultConstructible](../../named_req/DefaultConstructible.html "cpp/named req/DefaultConstructible").   
---  
  
(since C++11)  
  
3) Inserts a [`value_type`](../map.html#Member_types "cpp/container/map") object constructed in-place if there is no key that transparently compares _equivalent_ to the value x.

Equivalent to return this->try_emplace([std::forward](../../utility/forward.html)<K>(x)).first->second;. This overload participates in overload resolution only if `Compare` is [transparent](../../functional.html#Transparent_function_objects "cpp/utility/functional"). It allows calling this function without constructing an instance of `Key`.
...
## Contents
  ...
  * [4 Complexity](operator_at.html#Complexity)
  * [5 Notes](operator_at.html#Notes)
  * [6 Example](operator_at.html#Example)
  * [7 Defect reports](operator_at.html#Defect_reports)
  * [8 See also](operator_at.html#See_also)
...
However, one implementation (libc++) is known to construct the [`key_type`](../map.html#Member_types "cpp/container/map") and `mapped_type` objects via two separate allocator `construct()` calls, as arguably required by the standards as published, rather than emplacing a [`value_type`](../map.html#Member_types "cpp/container/map") object. 

operator[] is non-const because it inserts the key if it doesn't exist. If this behavior is undesirable or if the container is const, [`at`](at.html "cpp/container/map/at") may be used. 

[`insert_or_assign`](insert_or_assign.html "cpp/container/map/insert or assign") returns more information than operator[] and does not require default-constructibility of the mapped type.  | (since C++17)  
---|---  
...
Run this code
    ...
    void println(auto const comment, auto const& map)
    {
        ...
        for (const auto& pair : map)
            [std::cout](../../io/cout.html) << '{' << pair.first << ": " << pair.second << '}';
        [std::cout](../../io/cout.html) << "}\n";
    }
     
    int main()
    {
        [std::map](../map.html)<char, int> letter_counts{{'a', 27}, {'b', 3}, {'c', 1}};
     
    ...
     
        ...
        [std::map](../map.html)<[std::string](../../string/basic_string.html), int>  word_map;
    ...
     
        for (const auto& [word, count] : word_map)
            [std::cout](../../io/cout.html) << count << " occurrence(s) of word '" << word << "'\n";
    }
...
---|---|---|---  
[LWG 334](https://cplusplus.github.io/LWG/issue334) | C++98  | the effect of overload (1) was simply returning  
