...
The constructor of the new element (i.e. [std::pair](../../utility/pair.html)<const Key, T>) is called with exactly the same arguments as supplied to `emplace`, forwarded via [std::forward](../../utility/forward.html)<Args>(args).... The element may be constructed even if there already is an element with the key in the container, in which case the newly constructed element will be destroyed immediately (see [`try_emplace()`](try_emplace.html "cpp/container/map/try emplace") if this behavior is undesirable). 

If `value_type` is not [EmplaceConstructible](../../named_req/EmplaceConstructible.html "cpp/named req/EmplaceConstructible") into `map` from args, the behavior is undefined. 
