...
map& operator=( const map& other ); |  (1) | (constexpr since C++26)  
---|---|---  
| (2) |   
...
map& operator=( [std::initializer_list](../../utility/initializer_list.html)<value_type> ilist ); |  (3) | (since C++11)   
(constexpr since C++26)  
| |   
...
3) \\(\scriptsize O(N \cdot \log(N))\\)O(N·log(N)) in general, where \\(\scriptsize N\\)N is size() + ilist.size(). Linear if ilist is sorted with respect to [value_comp()](value_comp.html "cpp/container/map/value comp").
...
The following code uses operator= to assign one [std::map](../map.html "cpp/container/map") to another:
...
Run this code
    ...
    int main()
    {
        [std::map](../map.html)<int, int> x{{1,1}, {2,2}, {3,3}}, y, z;
...
[ (constructor)](map.html "cpp/container/map/map") |  constructs the `map`   
