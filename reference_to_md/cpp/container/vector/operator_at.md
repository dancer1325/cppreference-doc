...
The following code uses operator[] to read from and write to a [std::vector](../vector.html)<int>:
...
Run this code
    ...
    int main()
    {
        [std::vector](../vector.html)<int> numbers{2, 4, 6, 8};
     
    ...
    consteval auto sum_of_all_primes_up_to(unsigned N)
    {
        if (N < 2)
            return 0ULL;
     
        [std::vector](../vector.html)<bool> is_prime(N, true);
...
[ at](at.html "cpp/container/vector/at") |  access specified element with bounds checking   
