[[edit template]](https://en.cppreference.com/mwiki/index.php?title=Template:cpp/container/at_assoc&action=edit)
...
template< class K >  
T& at( const K& x ); |  (3) | (since C++23)  
template< class K >  
const T& at( const K& x ) const; |  (4)  |  (since C++23)  
| |   
  
Returns a reference to the mapped value of the element with specified key. If no such element exists, an exception of type [std::out_of_range](../../error/out_of_range.html "cpp/error/out of range") is thrown. 

1,2) The key is equivalent to key.

3,4) The key compares _equivalent_ to the value x. The reference to the mapped value is obtained as if by expression this->find(x)->second.

The expression this->find(x) must be well-formed and have well-defined behavior, otherwise the behavior is undefined.

These overloads participate in overload resolution only if `Compare` is [transparent](../../functional.html#Transparent_function_objects "cpp/utility/functional"). It allows calling this function without constructing an instance of `Key`.
...
Run this code
    ...
    int main()
    {
        [std::flat_map](../flat_map.html)<int, char> map{{1, 'a'}, {2, 'b'}};
        [assert](../../error/assert.html)(map.at(1) == 'a');
        [assert](../../error/assert.html)(map.at(2) == 'b');
        try
        {
            map.at(13);
        }
        ...
        {
            [std::cout](../../io/cout.html) << "1) out_of_range::what(): " << ex.what() << '\n';
        }
     
    #ifdef __cpp_lib_associative_heterogeneous_insertion
        // Transparent comparison demo.
        [std::flat_map](../flat_map.html)<HeavyKey, char, [std::less](../../utility/functional/less.html)<>> map2{{{1}, 'a'}, {{2}, 'b'}};
        [assert](../../error/assert.html)(map2.at(LightKey{1}) == 'a');
        [assert](../../error/assert.html)(map2.at(LightKey{2}) == 'b');
        try
        {
            map2.at(LightKey{13});
        }
        ...
        {
            [std::cout](../../io/cout.html) << "2) out_of_range::what(): " << ex.what() << '\n';
        }
    #endif
    }
