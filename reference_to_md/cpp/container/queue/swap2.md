...
void swap( [std::queue](../queue.html)<T, Container>& lhs,  
[std::queue](../queue.html)<T, Container>& rhs ); |  |  (since C++11)   
(until C++17)  
template< class T, class Container >  
void swap( [std::queue](../queue.html)<T, Container>& lhs,  
[std::queue](../queue.html)<T, Container>& rhs )  
noexcept(/* see below */); |  | (since C++17)   
(constexpr since C++26)  
| |   
Specializes the [std::swap](../../utility/swap.html "cpp/algorithm/swap") algorithm for [std::queue](../queue.html "cpp/container/queue"). Swaps the contents of lhs and rhs. Calls lhs.swap(rhs). This overload participates in overload resolution only if [std::is_swappable_v](../../types/is_swappable.html)<Container> is true.  | (since C++17)  
---|---  
...
Run this code
    ...
    int main()
    {
        [std::queue](../queue.html)<int> alice;
        [std::queue](../queue.html)<int> bob;
     
        auto print = [](const auto& title, const auto& cont)
        {
            [std::cout](../../io/cout.html) << title << " size=" << cont.size();
            [std::cout](../../io/cout.html) << " front=" << cont.front();
            [std::cout](../../io/cout.html) << " back=" << cont.back() << '\n';
        };
     
        for (int i = 1; i < 4; ++i)
            alice.push(i);
...
[ swap](swap.html "cpp/container/queue/swap")(C++11) |  swaps the contents   
