...
Causes abnormal program termination unless [SIGABRT](SIG_types.html "cpp/utility/program/SIG types") is being caught by a signal handler passed to [std::signal](signal.html "cpp/utility/program/signal") and the handler does not return. 

Destructors of variables with automatic, thread local(since C++11) and static [storage durations](../../language/storage_duration.html "cpp/language/storage duration") are not called. Functions registered with [std::atexit()](atexit.html "cpp/utility/program/atexit") and [std::at_quick_exit](at_quick_exit.html "cpp/utility/program/at quick exit")(since C++11) are also not called. Whether open resources such as files are closed is implementation defined. An implementation defined status is returned to the host environment that indicates unsuccessful execution. 

## Contents

  * [1 Parameters](abort.html#Parameters)
  * [2 Return value](abort.html#Return_value)
  * [3 Exceptions](abort.html#Exceptions)
  * [4 Notes](abort.html#Notes)
  * [5 Example](abort.html#Example)
...
Run this code
    ...
    {
    public:
        Tester()  { [std::cout](../../io/cout.html) << "Tester ctor\n"; }
        ~Tester() { [std::cout](../../io/cout.html) << "Tester dtor\n"; }
    };
     
    ...
    {
        if (signal == [SIGABRT](SIG_types.html))
            [std::cerr](../../io/cerr.html) << "SIGABRT received\n";
        else
            [std::cerr](../../io/cerr.html) << "Unexpected signal " << signal << " received\n";
        [std::_Exit](_Exit.html)([EXIT_FAILURE](EXIT_status.html));
    }
     
    int main()
    {
    ...
     
        // Setup handler
        auto previous_handler = [std::signal](signal.html)([SIGABRT](SIG_types.html), signal_handler);
        if (previous_handler == [SIG_ERR](SIG_ERR.html))
        {
            [std::cerr](../../io/cerr.html) << "Setup failed\n";
            return [EXIT_FAILURE](EXIT_status.html);
        }
     
        std::abort(); // Raise SIGABRT
        [std::cout](../../io/cout.html) << "This code is unreachable\n";
    }
