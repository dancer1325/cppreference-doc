...
[va_start](variadic/va_start.html "cpp/utility/variadic/va start")  
---  
[va_arg](variadic/va_arg.html "cpp/utility/variadic/va arg")  
[va_copy](variadic/va_copy.html "cpp/utility/variadic/va copy")(C++11)  
...
To declare a variadic function, an ellipsis appears after the list of parameters, e.g. int printf(const char* format...);, which may be preceded by an optional comma. See [Variadic arguments](../language/variadic_arguments.html "cpp/language/variadic arguments") for additional detail on the syntax, automatic argument conversions and the alternatives. 
...
Run this code
    ...
    {
        va_list args;
        [va_start](variadic/va_start.html)(args, fmt);
     
        while (*fmt != '\0')
        {
            if (*fmt == 'd')
            {
                int i = [va_arg](variadic/va_arg.html)(args, int);
                [std::cout](../io/cout.html) << i << '\n';
            }
            else if (*fmt == 'c')
            {
                // note automatic conversion to integral type
                int c = [va_arg](variadic/va_arg.html)(args, int);
                [std::cout](../io/cout.html) << static_cast<char>(c) << '\n';
            }
            else if (*fmt == 'f')
            {
                double d = [va_arg](variadic/va_arg.html)(args, double);
                [std::cout](../io/cout.html) << d << '\n';
            }
            ++fmt;
        }
     
        [va_end](variadic/va_end.html)(args);
    }
     
    int main()
    {
