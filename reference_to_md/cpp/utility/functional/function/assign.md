...
Initializes the _target_ with `f`. The `alloc` is used to allocate memory for any internal data structures that the `function` might use. 

Equivalent to function([std::allocator_arg](../../../memory/allocator_arg.html), alloc, [std::forward](../../forward.html)<F>(f)).swap(*this);. 
