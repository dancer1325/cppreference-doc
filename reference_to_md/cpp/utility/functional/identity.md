...
Run this code
    ...
    struct Pair
    {
        ...
        friend [std::ostream](../../io/basic_ostream.html)& operator<<([std::ostream](../../io/basic_ostream.html)& os, const Pair& p)
        {
            return os << '{' << p.n << ", " << p.s << '}';
        }
    };
    ...
    template<std::[ranges::input_range](../../ranges/input_range.html) R,
             typename Projection = std::identity> //<- Notice the default projection
    void print([std::string_view](../../string/basic_string_view.html) const rem, R&& range, Projection projection = {})
    {
        [std::cout](../../io/cout.html) << rem << '{';
        std::[ranges::for_each](../../algorithm/ranges/for_each.html)(
            range,
            [O = 0](const auto& o) mutable { [std::cout](../../io/cout.html) << (O++ ? ", " : "") << o; },
            projection
        );
    ...
     
        ...
        print("Print using custom closure as a projection: ", v,
            [](Pair const& p) { return [std::to_string](../../string/basic_string/to_string.html)(p.n) + ':' + p.s; });
    }
