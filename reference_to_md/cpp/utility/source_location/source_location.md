...
Run this code
    ...
    #if (__GNUG__ >= 11)
    template<typename T>
    auto type_name_helper(const [std::source_location](../source_location.html) s = std::source_location::current())
    {
        using std::operator""sv;
        const [std::string_view](../../string/basic_string_view.html) fun_name{s.function_name()};
        constexpr auto prefix{"[with T = "sv};
        const auto type_name_begin{fun_name.find(prefix)};
        if (""sv.npos == type_name_begin)
            return ""sv;
        const [std::size_t](../../types/size_t.html) first{type_name_begin + prefix.length()};
        return [std::string_view](../../string/basic_string_view.html){fun_name.cbegin() + first, fun_name.cend() - 1};
    }
     
    template<typename T>
    ...
    #endif
     
    void print([std::string_view](../../string/basic_string_view.html) const comment, [std::source_location](../source_location.html) const l)
    {
        [std::cout](../../io/cout.html) << comment << ":\n"
                  << "  file_name     : " << [std::quoted](../../io/manip/quoted.html)(l.file_name()) << '\n'
                  << "  function_name : " << [std::quoted](../../io/manip/quoted.html)(l.function_name()) << '\n'
                  << "  line          : " << l.line() << '\n'
                  << "  column        : " << l.column() << '\n';
    }
     
    int main()
    {
        constexpr [std::source_location](../source_location.html) default_constructed;
        print("default constructed", default_constructed);
     
        constexpr [std::source_location](../source_location.html) current = std::source_location::current();
        print("current", current);
     
    #if (__GNUG__ >= 11)
        const [std::vector](../../container/vector.html)<[std::vector](../../container/vector.html)<int>> v{{1,2}, {3,4,5}, {6}};
        auto jv = std::[ranges::join_view](../../ranges/join_view.html)(v);
